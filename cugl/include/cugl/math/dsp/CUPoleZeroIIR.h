 ...
 *
 * This filter is the preferred way to create a first-order highpass filter,
 * and so we provide the method {@link setHighpass} for setting the highpass
 * frequency.  In addition, there are methods for creating an allpass filter,
 * as well as a DC blocking filter.
 *
...
#pragma mark Specialized Attributes
     ...
     *
     * The resulting filter is typically considered the simplest effective
     * highpass filter.  However, filters are not intended to be model classes,
     * and so it does not save the defining frequency.
     *
