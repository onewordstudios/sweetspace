...
class socket_streambuf_buffers {
   protected:
	socket_streambuf_buffers() : get_buffer_(buffer_size), put_buffer_(buffer_size) {}
...
template <typename Protocol, typename Clock = chrono::steady_clock,
...
#endif // defined(ASIO_NO_DEPRECATED) || defined(GENERATING_DOCUMENTATION)
{
   ...
   protected:
	int_type underflow() {
		...
		for (;;) {
			...
			if (!socket().native_non_blocking()) socket().native_non_blocking(true, ec_);
   ...
   private:
	...
	basic_socket_streambuf& operator=(const basic_socket_streambuf&) ASIO_DELETED;
	...
	void connect_to_endpoints(EndpointIterator begin, EndpointIterator end) {
		...
		for (EndpointIterator i = begin; i != end; ++i) {
			...
			// Operation failed.
			if (ec_ != asio::error::in_progress && ec_ != asio::error::would_block) continue;
			...
			if (detail::socket_ops::getsockopt(socket().native_handle(), 0, SOL_SOCKET, SO_ERROR,
