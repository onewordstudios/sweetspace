...
class deadline_timer_service
   ...
   public:
	...
	void move_assign(implementation_type& impl, deadline_timer_service& other_service,
	...
	std::size_t cancel_one(implementation_type& impl, asio::error_code& ec) {
		...
		if (count == 0) impl.might_have_pending_waits = false;
		ec = asio::error_code();
		return count;
	}
	...
	std::size_t expires_after(implementation_type& impl, const duration_type& expiry_time,
							  asio::error_code& ec) {
	...
	void wait(implementation_type& impl, asio::error_code& ec) {
		...
		while (Time_Traits::less_than(now, impl.expiry) && !ec) {
			this->do_wait(Time_Traits::to_posix_duration(Time_Traits::subtract(impl.expiry, now)),
						  ec);
			now = Time_Traits::now();
		}
	}
	...
	void async_wait(implementation_type& impl, Handler& handler, const IoExecutor& io_ex) {
		...
		ASIO_HANDLER_CREATION(
			(scheduler_.context(), *p.p, "deadline_timer", &impl, 0, "async_wait"));
