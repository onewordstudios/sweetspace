...
class io_object_executor {
   public:
	io_object_executor(const Executor& ex, bool native_implementation) ASIO_NOEXCEPT
		: executor_(ex),
	...
	io_object_executor(const io_object_executor<Executor1>& other) ASIO_NOEXCEPT
	...
	const Executor& inner_executor() const ASIO_NOEXCEPT { return executor_; }

	bool has_native_implementation() const ASIO_NOEXCEPT { return has_native_impl_; }

	execution_context& context() const ASIO_NOEXCEPT { return executor_.context(); }

	void on_work_started() const ASIO_NOEXCEPT {
		if (is_same<Executor, io_context::executor_type>::value || has_native_impl_) {
	...
	friend bool operator==(const io_object_executor& a, const io_object_executor& b) ASIO_NOEXCEPT {
		return a.executor_ == b.executor_ && a.has_native_impl_ == b.has_native_impl_;
	}
