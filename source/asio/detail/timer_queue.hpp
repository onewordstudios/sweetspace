...
class timer_queue : public timer_queue_base {
   public:
	...
	class per_timer_data {
	   public:
		per_timer_data()
			: heap_index_((std::numeric_limits<std::size_t>::max)()), next_(0), prev_(0) {}
	...
	virtual long wait_duration_msec(long max_duration) const {
		...
		return this->to_msec(Time_Traits::to_posix_duration(
	...
	virtual long wait_duration_usec(long max_duration) const {
		...
		return this->to_usec(Time_Traits::to_posix_duration(
								 Time_Traits::subtract(heap_[0].time_, Time_Traits::now())),
							 max_duration);
	}
	...
	void move_timer(per_timer_data& target, per_timer_data& source) {
		...
		if (target.heap_index_ < heap_.size()) heap_[target.heap_index_].timer_ = &target;

		if (timers_ == &source) timers_ = &target;
		if (source.prev_) source.prev_->next_ = &target;
		if (source.next_) source.next_->prev_ = &target;
   ...
   private:
	...
	void up_heap(std::size_t index) {
		while (index > 0) {
			...
			if (!Time_Traits::less_than(heap_[index].time_, heap_[parent].time_)) break;
	...
	void down_heap(std::size_t index) {
		...
		while (child < heap_.size()) {
			std::size_t min_child =
				(child + 1 == heap_.size() ||
				 Time_Traits::less_than(heap_[child].time_, heap_[child + 1].time_))
					? child
					: child + 1;
			if (Time_Traits::less_than(heap_[index].time_, heap_[min_child].time_)) break;
	...
	void remove_timer(per_timer_data& timer) {
		...
		if (!heap_.empty() && index < heap_.size()) {
			if (index == heap_.size() - 1) {
				...
				heap_.pop_back();
				if (index > 0 &&
					Time_Traits::less_than(heap_[index].time_, heap_[(index - 1) / 2].time_))
					up_heap(index);
				else
					down_heap(index);
			}
	...
	long to_usec(const Duration& d, long max_duration) const {
		...
		if (usec == 0) return 1;
		if (usec > max_duration) return max_duration;
