...
class resolve_query_op : public resolve_op {
   public:
	...
	resolve_query_op(socket_ops::weak_cancel_token_type cancel_token, const query_type& query,
	...
	~resolve_query_op() {
		if (addrinfo_) socket_ops::freeaddrinfo(addrinfo_);
	}
	...
	static void do_complete(void* owner, operation* base, const asio::error_code& /*ec*/,
		...
		if (owner && owner != &o->scheduler_) {
			...
			socket_ops::background_getaddrinfo(o->cancel_token_, o->query_.host_name().c_str(),
											   o->query_.service_name().c_str(), o->query_.hints(),
											   &o->addrinfo_, o->ec_);
			...
			detail::binder2<Handler, asio::error_code, results_type> handler(o->handler_, o->ec_,
																			 results_type());
			p.h = asio::detail::addressof(handler.handler_);
			if (o->addrinfo_) {
				handler.arg2_ = results_type::create(o->addrinfo_, o->query_.host_name(),
