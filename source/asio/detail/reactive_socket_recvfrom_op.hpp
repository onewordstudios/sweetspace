...
class reactive_socket_recvfrom_op_base : public reactor_op {
   public:
	...
	static status do_perform(reactor_op* base) {
		...
		status result = socket_ops::non_blocking_recvfrom(o->socket_, bufs.buffers(), bufs.count(),
														  o->flags_, o->sender_endpoint_.data(),
														  &addr_len, o->ec_, o->bytes_transferred_)
							? done
							: not_done;

		if (result && !o->ec_) o->sender_endpoint_.resize(addr_len);
