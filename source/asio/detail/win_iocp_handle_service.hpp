...
class win_iocp_handle_service : public execution_context_service_base<win_iocp_handle_service> {
   public:
	...
	class implementation_type {
	   public:
		// Default constructor.
		implementation_type()
			: handle_(INVALID_HANDLE_VALUE), safe_cancellation_thread_id_(0), next_(0), prev_(0) {}
	...
	ASIO_DECL void move_assign(implementation_type& impl, win_iocp_handle_service& other_service,
	...
	ASIO_DECL asio::error_code assign(implementation_type& impl, const native_handle_type& handle,
									  asio::error_code& ec);
	...
	size_t write_some(implementation_type& impl, const ConstBufferSequence& buffers,
					  asio::error_code& ec) {
	...
	void async_write_some(implementation_type& impl, const ConstBufferSequence& buffers,
		...
		start_write_op(
			impl, 0,
			buffer_sequence_adapter<asio::const_buffer, ConstBufferSequence>::first(buffers), p.p);
		p.v = p.p = 0;
	}
	...
	size_t read_some(implementation_type& impl, const MutableBufferSequence& buffers,
					 asio::error_code& ec) {
	...
	void async_read_some(implementation_type& impl, const MutableBufferSequence& buffers,
						 Handler& handler, const IoExecutor& io_ex) {
		// Allocate and construct an operation to wrap the handler.
   ...
   private:
	...
	size_t write_some_at(implementation_type& impl, uint64_t offset, const null_buffers& buffers,
						 asio::error_code& ec);
	template <typename Handler, typename IoExecutor>
	void async_write_some(implementation_type& impl, const null_buffers& buffers, Handler& handler,
						  const IoExecutor& io_ex);
	template <typename Handler, typename IoExecutor>
	void async_write_some_at(implementation_type& impl, uint64_t offset,
	...
	size_t read_some_at(implementation_type& impl, uint64_t offset, const null_buffers& buffers,
						asio::error_code& ec);
	template <typename Handler, typename IoExecutor>
	void async_read_some(implementation_type& impl, const null_buffers& buffers, Handler& handler,
						 const IoExecutor& io_ex);
	template <typename Handler, typename IoExecutor>
	void async_read_some_at(implementation_type& impl, uint64_t offset, const null_buffers& buffers,
							Handler& handler, const IoExecutor& io_ex);
	...
	ASIO_DECL size_t do_write(implementation_type& impl, uint64_t offset,
							  const asio::const_buffer& buffer, asio::error_code& ec);

	// Helper function to start a write operation.
	ASIO_DECL void start_write_op(implementation_type& impl, uint64_t offset,
								  const asio::const_buffer& buffer, operation* op);

	// Helper function to perform a synchronous write operation.
	ASIO_DECL size_t do_read(implementation_type& impl, uint64_t offset,
							 const asio::mutable_buffer& buffer, asio::error_code& ec);

	// Helper function to start a read operation.
	ASIO_DECL void start_read_op(implementation_type& impl, uint64_t offset,
