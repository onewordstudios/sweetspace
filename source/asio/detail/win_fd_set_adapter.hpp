...
class win_fd_set_adapter : noncopyable {
   public:
	...
	win_fd_set_adapter() : capacity_(default_fd_set_size), max_descriptor_(invalid_socket) {
	...
	bool set(socket_type descriptor) {
		for (u_int i = 0; i < fd_set_->fd_count; ++i)
			if (fd_set_->fd_array[i] == descriptor) return true;
   ...
   private:
	...
	void reserve(u_int n) {
		...
		if (new_capacity < n) new_capacity = n;
