...
class reactive_socket_recv_op_base : public reactor_op {
   public:
	reactive_socket_recv_op_base(socket_type socket, socket_ops::state_type state,
	...
	static status do_perform(reactor_op* base) {
		...
		status result =
			socket_ops::non_blocking_recv(o->socket_, bufs.buffers(), bufs.count(), o->flags_,
		...
		if (result == done)
			if ((o->state_ & socket_ops::stream_oriented) != 0)
				if (o->bytes_transferred_ == 0) result = done_and_exhausted;
   ...
   public:
	...
	reactive_socket_recv_op(socket_type socket, socket_ops::state_type state,
		...
		: reactive_socket_recv_op_base<MutableBufferSequence>(
			  socket, state, buffers, flags, &reactive_socket_recv_op::do_complete),
