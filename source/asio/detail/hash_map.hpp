   ...
   public:
	...
	iterator find(const K& k) {
		if (num_buckets_) {
			...
			if (it == values_.end()) return values_.end();
	...
	std::pair<iterator, bool> insert(const value_type& v) {
		if (size_ + 1 >= num_buckets_) rehash(hash_size(size_ + 1));
		...
		++end_it;
		while (it != end_it) {
			if (it->first == v.first) return std::pair<iterator, bool>(it, false);
			++it;
		}
   ...
   private:
	...
	static std::size_t hash_size(std::size_t num_elems) {
		...
		for (std::size_t i = 0; i < nth_size; ++i)
			if (num_elems < sizes[i]) return sizes[i];
		return sizes[nth_size];
	}

	// Re-initialise the hash from the values already contained in the list.
	void rehash(std::size_t num_buckets) {
		if (num_buckets == num_buckets_) return;
