...
class buffered_read_stream : private noncopyable {
   public:
	...
	explicit buffered_read_stream(Arg& a) : next_layer_(a), storage_(default_buffer_size) {}

	/// Construct, passing the specified argument to initialise the next layer.
	template <typename Arg>
	buffered_read_stream(Arg& a, std::size_t buffer_size) : next_layer_(a), storage_(buffer_size) {}
	...
	async_write_some(const ConstBufferSequence& buffers,
		...
		return next_layer_.async_write_some(buffers, ASIO_MOVE_CAST(WriteHandler)(handler));
	}
	...
	async_fill(ASIO_MOVE_ARG(ReadHandler) handler ASIO_DEFAULT_COMPLETION_TOKEN(executor_type));
