...
class basic_stream_descriptor : public basic_descriptor<Executor> {
   public:
	 ...
	 * descriptor.
	 */
	explicit basic_stream_descriptor(const executor_type& ex) : basic_descriptor<Executor>(ex) {}
	...
	basic_stream_descriptor(
		ExecutionContext& context, const native_handle_type& native_descriptor,
		typename enable_if<is_convertible<ExecutionContext&, execution_context&>::value>::type* = 0)
	...
	std::size_t write_some(const ConstBufferSequence& buffers) {
		asio::error_code ec;
		std::size_t s =
			this->impl_.get_service().write_some(this->impl_.get_implementation(), buffers, ec);
	...
	std::size_t read_some(const MutableBufferSequence& buffers) {
		asio::error_code ec;
		std::size_t s =
			this->impl_.get_service().read_some(this->impl_.get_implementation(), buffers, ec);
   ...
   private:
	class initiate_async_write_some {
	   public:
		...
		explicit initiate_async_write_some(basic_stream_descriptor* self) : self_(self) {}
		...
		void operator()(ASIO_MOVE_ARG(WriteHandler) handler,
			...
			self_->impl_.get_service().async_write_some(self_->impl_.get_implementation(), buffers,
														handler2.value,
	...
	class initiate_async_read_some {
	   public:
		...
		explicit initiate_async_read_some(basic_stream_descriptor* self) : self_(self) {}
		...
		void operator()(ASIO_MOVE_ARG(ReadHandler) handler,
			...
			self_->impl_.get_service().async_read_some(self_->impl_.get_implementation(), buffers,
													   handler2.value,
													   self_->impl_.get_implementation_executor());
		}
