...
class basic_overlapped_handle {
   public:
	 ...
	 * overlapped handle.
	 */
	explicit basic_overlapped_handle(const executor_type& ex) : impl_(ex) {}
	...
	explicit basic_overlapped_handle(
		...
		typename enable_if<is_convertible<ExecutionContext&, execution_context&>::value,
						   basic_overlapped_handle>::type* = 0)
		: impl_(context) {}
	 ...
	 * constructor.
	 */
	basic_overlapped_handle(basic_overlapped_handle&& other) : impl_(std::move(other.impl_)) {}
   ...
   private:
	...
	basic_overlapped_handle& operator=(const basic_overlapped_handle&) ASIO_DELETED;
};
