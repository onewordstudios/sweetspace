...
class buffered_write_stream : private noncopyable {
   public:
	...
	explicit buffered_write_stream(Arg& a) : next_layer_(a), storage_(default_buffer_size) {}
	...
	async_flush(ASIO_MOVE_ARG(WriteHandler) handler ASIO_DEFAULT_COMPLETION_TOKEN(executor_type));
	...
	async_read_some(const MutableBufferSequence& buffers,
					ASIO_MOVE_ARG(ReadHandler)
						handler ASIO_DEFAULT_COMPLETION_TOKEN(executor_type)) {
		return next_layer_.async_read_some(buffers, ASIO_MOVE_CAST(ReadHandler)(handler));
	}
