...
class basic_socket : public socket_base {
   public:
	 ...
	 */
	explicit basic_socket(const executor_type& ex) : impl_(ex) {}
	 ...
	 */
	basic_socket(const executor_type& ex, const protocol_type& protocol) : impl_(ex) {
		asio::error_code ec;
	 ...
	 * @throws asio::system_error Thrown on failure.
	 */
	basic_socket(const executor_type& ex, const endpoint_type& endpoint) : impl_(ex) {
		asio::error_code ec;
	 ...
	 */
	basic_socket(basic_socket&& other) ASIO_NOEXCEPT : impl_(std::move(other.impl_)) {}
	 ...
	 * @param ec Set to indicate what error occurred, if any.
	 */
	ASIO_SYNC_OP_VOID assign(const protocol_type& protocol, const native_handle_type& native_socket,
							 asio::error_code& ec) {
		impl_.get_service().assign(impl_.get_implementation(), protocol, native_socket, ec);
		ASIO_SYNC_OP_VOID_RETURN(ec);
	}
	...
	(!defined(_WIN32_WINNT) || _WIN32_WINNT < 0x0600) && !defined(ASIO_ENABLE_CANCELIO)
	__declspec(
		deprecated("By default, this function always fails with "
	...
	ASIO_INITFN_AUTO_RESULT_TYPE(ConnectHandler, void(asio::error_code))
	async_connect(const endpoint_type& peer_endpoint,
				  ASIO_MOVE_ARG(ConnectHandler)
   ...
   private:
	...
	class initiate_async_connect {
	   public:
		...
		explicit initiate_async_connect(basic_socket* self) : self_(self) {}

		executor_type get_executor() const ASIO_NOEXCEPT { return self_->get_executor(); }

		template <typename ConnectHandler>
		void operator()(ASIO_MOVE_ARG(ConnectHandler) handler, const endpoint_type& peer_endpoint,
			...
			if (open_ec) {
				asio::post(
					...
					asio::detail::bind_handler(ASIO_MOVE_CAST(ConnectHandler)(handler), open_ec));
			} else {
				detail::non_const_lvalue<ConnectHandler> handler2(handler);
				self_->impl_.get_service().async_connect(
					self_->impl_.get_implementation(), peer_endpoint, handler2.value,
	...
	class initiate_async_wait {
	   public:
		...
		explicit initiate_async_wait(basic_socket* self) : self_(self) {}
