...
class basic_resolver : public resolver_base {
   public:
	 ...
	 * resolver.
	 */
	explicit basic_resolver(const executor_type& ex) : impl_(ex) {}
	 ...
	 */
	basic_resolver(basic_resolver&& other) : impl_(std::move(other.impl_)) {}
	 ...
	 * may use additional locations when resolving service names.
	 */
	results_type resolve(const protocol_type& protocol, ASIO_STRING_VIEW_PARAM host,
	...
	results_type resolve(const protocol_type& protocol, ASIO_STRING_VIEW_PARAM host,
		...
		asio::error_code ec;
		basic_resolver_query<protocol_type> q(protocol, static_cast<std::string>(host),
											  static_cast<std::string>(service), resolve_flags);
		results_type r = impl_.get_service().resolve(impl_.get_implementation(), q, ec);
		asio::detail::throw_error(ec, "resolve");
		return r;
	...
	template <ASIO_COMPLETION_TOKEN_FOR(void(asio::error_code, results_type))
				  ResolveHandler ASIO_DEFAULT_COMPLETION_TOKEN_TYPE(executor_type)>
	ASIO_INITFN_AUTO_RESULT_TYPE(ResolveHandler, void(asio::error_code, results_type))
	async_resolve(const query& q, ASIO_MOVE_ARG(ResolveHandler)
									  handler ASIO_DEFAULT_COMPLETION_TOKEN(executor_type)) {
		return asio::async_initiate<ResolveHandler, void(asio::error_code, results_type)>(
	...
	template <ASIO_COMPLETION_TOKEN_FOR(void(asio::error_code, results_type))
				  ResolveHandler ASIO_DEFAULT_COMPLETION_TOKEN_TYPE(executor_type)>
	ASIO_INITFN_AUTO_RESULT_TYPE(ResolveHandler, void(asio::error_code, results_type))
	async_resolve(ASIO_STRING_VIEW_PARAM host, ASIO_STRING_VIEW_PARAM service,
				  ASIO_MOVE_ARG(ResolveHandler)
					  handler ASIO_DEFAULT_COMPLETION_TOKEN(executor_type)) {
	...
	template <ASIO_COMPLETION_TOKEN_FOR(void(asio::error_code, results_type))
				  ResolveHandler ASIO_DEFAULT_COMPLETION_TOKEN_TYPE(executor_type)>
	ASIO_INITFN_AUTO_RESULT_TYPE(ResolveHandler, void(asio::error_code, results_type))
	async_resolve(const protocol_type& protocol, ASIO_STRING_VIEW_PARAM host,
				  ASIO_STRING_VIEW_PARAM service,
				  ASIO_MOVE_ARG(ResolveHandler)
					  handler ASIO_DEFAULT_COMPLETION_TOKEN(executor_type)) {
   ...
   private:
	...
	class initiate_async_resolve {
	   public:
		...
		explicit initiate_async_resolve(basic_resolver* self) : self_(self) {}

		executor_type get_executor() const ASIO_NOEXCEPT { return self_->get_executor(); }

		template <typename ResolveHandler, typename Query>
		void operator()(ASIO_MOVE_ARG(ResolveHandler) handler, const Query& q) const {
			...
			self_->impl_.get_service().async_resolve(self_->impl_.get_implementation(), q,
													 handler2.value,
...
#if defined(ASIO_WINDOWS_RUNTIME)
	asio::detail::io_object_impl<asio::detail::winrt_resolver_service<InternetProtocol>, Executor>
		impl_;
#else
