...
#endif // !defined(ASIO_NO_DEPRECATED)
{
   public:
	...
	basic_resolver_results(basic_resolver_results&& other)
		: basic_resolver_iterator<InternetProtocol>(ASIO_MOVE_CAST(basic_resolver_results)(other)) {
	}
	...
	static basic_resolver_results create(asio::detail::addrinfo_type* address_info,
										 const std::string& host_name,
										 const std::string& service_name) {
		basic_resolver_results results;
		if (!address_info) return results;

		std::string actual_host_name = host_name;
		if (address_info->ai_canonname) actual_host_name = address_info->ai_canonname;
	...
	static basic_resolver_results create(const endpoint_type& endpoint,
		...
		results.values_->push_back(
			basic_resolver_entry<InternetProtocol>(endpoint, host_name, service_name));
		return results;
	}

	// Create results from a sequence of endpoints, host and service name.
	template <typename EndpointIterator>
	static basic_resolver_results create(EndpointIterator begin, EndpointIterator end,
										 const std::string& host_name,
										 const std::string& service_name) {
		basic_resolver_results results;
		if (begin != end) {
			...
			for (EndpointIterator ep_iter = begin; ep_iter != end; ++ep_iter) {
				results.values_->push_back(
					basic_resolver_entry<InternetProtocol>(*ep_iter, host_name, service_name));
			}
		}
		return results;
	}
			...
			for (unsigned int i = 0; i < endpoints->Size; ++i) {
				...
				results.values_->push_back(basic_resolver_entry<InternetProtocol>(
					typename InternetProtocol::endpoint(
						ip::make_address(
							asio::detail::winrt_utils::string(pair->RemoteHostName->CanonicalName)),
	...
	size_type size() const ASIO_NOEXCEPT { return this->values_ ? this->values_->size() : 0; }
	...
	bool empty() const ASIO_NOEXCEPT { return this->values_ ? this->values_->empty() : true; }
