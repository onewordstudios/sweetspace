...
class basic_yield_context {
   public:
	 ...
	 * function.
	 */
	basic_yield_context(const detail::weak_ptr<callee_type>& coro, caller_type& ca,
						Handler& handler)
	...
	basic_yield_context(const basic_yield_context<OtherHandler>& other)
		: coro_(other.coro_), ca_(other.ca_), handler_(other.handler_), ec_(other.ec_) {}
...
typedef basic_yield_context<executor_binder<void (*)(), executor> > yield_context;
#endif // defined(GENERATING_DOCUMENTATION)
...
void spawn(const Executor& ex, ASIO_MOVE_ARG(Function) function,
		   const boost::coroutines::attributes& attributes = boost::coroutines::attributes(),
 ...
 */
template <typename Function, typename Executor>
void spawn(const strand<Executor>& ex, ASIO_MOVE_ARG(Function) function,
		   const boost::coroutines::attributes& attributes = boost::coroutines::attributes());
 ...
 */
template <typename Function>
void spawn(const asio::io_context::strand& s, ASIO_MOVE_ARG(Function) function,
		   const boost::coroutines::attributes& attributes = boost::coroutines::attributes());
