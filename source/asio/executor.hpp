...
class executor {
   public:
	...
	/// Move constructor.
	executor(executor&& other) ASIO_NOEXCEPT : impl_(other.impl_) { other.impl_ = 0; }
	...
	execution_context& context() const ASIO_NOEXCEPT { return get_impl()->context(); }

	/// Inform the executor that it has some outstanding work to do.
	void on_work_started() const ASIO_NOEXCEPT { get_impl()->on_work_started(); }

	/// Inform the executor that some work is no longer outstanding.
	void on_work_finished() const ASIO_NOEXCEPT { get_impl()->on_work_finished(); }
	...
	const void* target_type() const ASIO_NOEXCEPT { return impl_ ? impl_->target_type() : 0; }
	...
	friend bool operator==(const executor& a, const executor& b) ASIO_NOEXCEPT {
		...
		if (!a.impl_ || !b.impl_) return false;
		return a.impl_->equals(b.impl_);
	}

	/// Compare two executors for inequality.
	friend bool operator!=(const executor& a, const executor& b) ASIO_NOEXCEPT { return !(a == b); }
   ...
   private:
	...
	impl_base* clone() const ASIO_NOEXCEPT { return impl_ ? impl_->clone() : 0; }
