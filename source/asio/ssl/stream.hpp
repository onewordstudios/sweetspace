...
class stream : public stream_base, private noncopyable {
   public:
	 ...
	 */
	executor_type get_executor() ASIO_NOEXCEPT { return next_layer_.lowest_layer().get_executor(); }
	...
	ASIO_SYNC_OP_VOID set_verify_callback(VerifyCallback callback, asio::error_code& ec) {
		core_.engine_.set_verify_callback(new detail::verify_callback<VerifyCallback>(callback),
										  ec);
		ASIO_SYNC_OP_VOID_RETURN(ec);
	}
	 ...
	 */
	template <typename ConstBufferSequence>
	ASIO_SYNC_OP_VOID handshake(handshake_type type, const ConstBufferSequence& buffers,
								asio::error_code& ec) {
	...
	template <ASIO_COMPLETION_TOKEN_FOR(void(asio::error_code))
				  HandshakeHandler ASIO_DEFAULT_COMPLETION_TOKEN_TYPE(executor_type)>
	ASIO_INITFN_AUTO_RESULT_TYPE(HandshakeHandler, void(asio::error_code))
	async_handshake(handshake_type type, ASIO_MOVE_ARG(HandshakeHandler)
											 handler ASIO_DEFAULT_COMPLETION_TOKEN(executor_type)) {
		return async_initiate<HandshakeHandler, void(asio::error_code)>(
	...
	template <typename ConstBufferSequence,
			  ASIO_COMPLETION_TOKEN_FOR(void(asio::error_code, std::size_t))
				  BufferedHandshakeHandler ASIO_DEFAULT_COMPLETION_TOKEN_TYPE(executor_type)>
	ASIO_INITFN_AUTO_RESULT_TYPE(BufferedHandshakeHandler, void(asio::error_code, std::size_t))
	async_handshake(handshake_type type, const ConstBufferSequence& buffers,
					ASIO_MOVE_ARG(BufferedHandshakeHandler)
						handler ASIO_DEFAULT_COMPLETION_TOKEN(executor_type)) {
		return async_initiate<BufferedHandshakeHandler, void(asio::error_code, std::size_t)>(
	 ...
	 */
	template <ASIO_COMPLETION_TOKEN_FOR(void(asio::error_code))
				  ShutdownHandler ASIO_DEFAULT_COMPLETION_TOKEN_TYPE(executor_type)>
	ASIO_INITFN_AUTO_RESULT_TYPE(ShutdownHandler, void(asio::error_code))
	async_shutdown(ASIO_MOVE_ARG(ShutdownHandler)
					   handler ASIO_DEFAULT_COMPLETION_TOKEN(executor_type)) {
		return async_initiate<ShutdownHandler, void(asio::error_code)>(
   ...
   private:
	class initiate_async_handshake {
	   public:
		...
		void operator()(ASIO_MOVE_ARG(HandshakeHandler) handler, handshake_type type) const {
			...
			detail::async_io(self_->next_layer_, self_->core_, detail::handshake_op(type),
							 handler2.value);
		}
	...
	class initiate_async_buffered_handshake {
	   public:
		...
		void operator()(ASIO_MOVE_ARG(BufferedHandshakeHandler) handler, handshake_type type,
