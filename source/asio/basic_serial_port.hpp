...
class basic_serial_port : public serial_port_base {
   public:
	 ...
	 * serial port.
	 */
	explicit basic_serial_port(const executor_type& ex) : impl_(ex) {}
	 ...
	 * port.
	 */
	basic_serial_port(const executor_type& ex, const char* device) : impl_(ex) {
		asio::error_code ec;
	 ...
	 * port.
	 */
	basic_serial_port(const executor_type& ex, const std::string& device) : impl_(ex) {
		asio::error_code ec;
		impl_.get_service().open(impl_.get_implementation(), device, ec);
		asio::detail::throw_error(ec, "open");
	}
	 ...
	 * constructor.
	 */
	basic_serial_port(basic_serial_port&& other) : impl_(std::move(other.impl_)) {}
