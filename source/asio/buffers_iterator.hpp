...
struct buffers_iterator_types {
	enum {
		is_mutable = is_convertible<typename BufferSequence::value_type, mutable_buffer>::value
	};
...
class buffers_iterator {
   private:
	typedef
		typename detail::buffers_iterator_types<BufferSequence, ByteType>::buffer_type buffer_type;

	typedef typename detail::buffers_iterator_types<BufferSequence, ByteType>::const_iterator
   ...
   public:
...
#endif // defined(__GNUC__) && (__GNUC__ == 4) && (__GNUC_MINOR__ == 3)
	{
		...
		while (new_iter.current_ != new_iter.end_) {
			...
			if (new_iter.current_buffer_.size() > 0) break;
			++new_iter.current_;
		}
		return new_iter;
	}
   ...
   private:
	...
	reference dereference() const {
		return static_cast<pointer>(current_buffer_.data())[current_buffer_position_];
	}
	...
	void increment() {
		...
		if (current_buffer_position_ != current_buffer_.size()) return;
		...
		while (current_ != end_) {
			...
			if (current_buffer_.size() > 0) return;
			++current_;
		}
	}
...
inline buffers_iterator<BufferSequence> buffers_begin(const BufferSequence& buffers) {
	return buffers_iterator<BufferSequence>::begin(buffers);
}

/// Construct an iterator representing the end of the buffers' data.
template <typename BufferSequence>
inline buffers_iterator<BufferSequence> buffers_end(const BufferSequence& buffers) {
