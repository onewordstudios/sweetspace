...
#define ASIO_PRIVATE_ASYNC_RESULT_DEF(n)                                                    \
	...
	class async_result<std::packaged_task<Result(ASIO_VARIADIC_TARGS(n))>, Signature> {     \
	   public:                                                                              \
		typedef std::packaged_task<Result(ASIO_VARIADIC_TARGS(n))> completion_handler_type; \
                                                                                            \
		typedef std::future<Result> return_type;                                            \
                                                                                            \
		explicit async_result(completion_handler_type& h) : future_(h.get_future()) {}      \
