...
class initiate_dispatch_with_executor {
   public:
	...
	explicit initiate_dispatch_with_executor(const Executor& ex) : ex_(ex) {}
...
dispatch(ASIO_MOVE_ARG(CompletionToken) token) {
	return async_initiate<CompletionToken, void()>(detail::initiate_dispatch(), token);
}

template <typename Executor, ASIO_COMPLETION_TOKEN_FOR(void()) CompletionToken>
ASIO_INITFN_AUTO_RESULT_TYPE(CompletionToken, void())
dispatch(const Executor& ex, ASIO_MOVE_ARG(CompletionToken) token,
...
inline ASIO_INITFN_AUTO_RESULT_TYPE(CompletionToken, void()) dispatch(
	...
	return (dispatch)(ctx.get_executor(), ASIO_MOVE_CAST(CompletionToken)(token));
}
