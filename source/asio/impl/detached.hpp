...
struct async_result<detached_t, Signature> {
	...
	static return_type initiate(ASIO_MOVE_ARG(Initiation) initiation,
								ASIO_MOVE_ARG(RawCompletionToken), ASIO_MOVE_ARG(Args)... args) {
		ASIO_MOVE_CAST(Initiation)
		(initiation)(detail::detached_handler(detached_t()), ASIO_MOVE_CAST(Args)(args)...);
	}
	...
	static return_type initiate(ASIO_MOVE_ARG(Initiation) initiation,
		...
		ASIO_MOVE_CAST(Initiation)(initiation)(detail::detached_handler(detached_t()));
	}

#define ASIO_PRIVATE_INITIATE_DEF(n)                                                               \
	template <typename Initiation, typename RawCompletionToken, ASIO_VARIADIC_TPARAMS(n)>          \
	static return_type initiate(ASIO_MOVE_ARG(Initiation) initiation,                              \
								ASIO_MOVE_ARG(RawCompletionToken), ASIO_VARIADIC_MOVE_PARAMS(n)) { \
		ASIO_MOVE_CAST(Initiation)                                                                 \
		(initiation)(detail::detached_handler(detached_t()), ASIO_VARIADIC_MOVE_ARGS(n));          \
	}                                                                                              \
	/**/
