...
struct composed_io_executors<void()> {
	composed_io_executors() ASIO_NOEXCEPT : head_(system_executor()) {}
...
struct composed_io_executors<void(Head)> {
	explicit composed_io_executors(const Head& ex) ASIO_NOEXCEPT : head_(ex) {}
...
template <typename Head>
inline composed_io_executors<void(Head)> make_composed_io_executors(const Head& head) {
...
struct composed_io_executors<void(Head, Tail...)> {
	explicit composed_io_executors(const Head& head, const Tail&... tail) ASIO_NOEXCEPT
		: head_(head),
		  tail_(tail...) {}
...
template <typename Head, typename... Tail>
inline composed_io_executors<void(Head, Tail...)> make_composed_io_executors(const Head& head,
																			 const Tail&... tail) {
...
#define ASIO_PRIVATE_COMPOSED_IO_EXECUTORS_DEF(n)                                                \
	...
	struct composed_io_executors<void(Head, ASIO_VARIADIC_TARGS(n))> {                           \
		...
		composed_io_executors<void(ASIO_VARIADIC_TARGS(n))> tail_;                               \
                                                                                                 ...
                                                                                                 \
	template <typename Head, ASIO_VARIADIC_TPARAMS(n)>                                           \
	inline composed_io_executors<void(Head, ASIO_VARIADIC_TARGS(n))> make_composed_io_executors( \
		const Head& head, ASIO_VARIADIC_CONSTREF_PARAMS(n)) {                                    \
		return composed_io_executors<void(Head, ASIO_VARIADIC_TARGS(n))>(                        \
...
struct composed_work<void()> {
	...
	composed_work(const executors_type&) ASIO_NOEXCEPT : head_(system_executor()) {}
...
struct composed_work<void(Head)> {
	...
	explicit composed_work(const executors_type& ex) ASIO_NOEXCEPT : head_(ex.head_) {}
...
#define ASIO_PRIVATE_COMPOSED_WORK_DEF(n)                                                   \
	...
	struct composed_work<void(Head, ASIO_VARIADIC_TARGS(n))> {                              \
		typedef composed_io_executors<void(Head, ASIO_VARIADIC_TARGS(n))> executors_type;   \
                                                                                            \
		explicit composed_work(const executors_type& ex) ASIO_NOEXCEPT : head_(ex.head_),   \
																		 tail_(ex.tail_) {} \
                                                                                            \
...
{
   public:
	composed_op(ASIO_MOVE_ARG(Impl) impl, ASIO_MOVE_ARG(Work) work, ASIO_MOVE_ARG(Handler) handler)
	...
	typedef typename associated_executor<Handler, typename Work::head_type>::type executor_type;

	executor_type get_executor() const ASIO_NOEXCEPT {
		return (get_associated_executor)(handler_, work_.head_.get_executor());
	}

	typedef typename associated_allocator<Handler, std::allocator<void> >::type allocator_type;
	...
	void operator()(ASIO_MOVE_ARG(T)... t) {
		if (invocations_ < ~unsigned(0)) ++invocations_;
...
class initiate_composed_op {
   public:
	...
	executor_type get_executor() const ASIO_NOEXCEPT { return executors_.head_; }

	template <typename Handler, typename Impl>
	void operator()(ASIO_MOVE_ARG(Handler) handler, ASIO_MOVE_ARG(Impl) impl) const {
		composed_op<typename decay<Impl>::type, composed_work<Executors>,
					typename decay<Handler>::type, Signature>(ASIO_MOVE_CAST(Impl)(impl),
...
inline typename IoObject::executor_type get_composed_io_executor(
	IoObject& io_object, typename enable_if<!is_executor<IoObject>::value>::type* = 0) {
	return io_object.get_executor();
}

template <typename Executor>
inline const Executor& get_composed_io_executor(
	const Executor& ex, typename enable_if<is_executor<Executor>::value>::type* = 0) {
	return ex;
}
} // namespace detail
	...
	return async_initiate<CompletionToken, Signature>(
		detail::make_initiate_composed_op<Signature>(
			detail::make_composed_io_executors(detail::get_composed_io_executor(
				ASIO_MOVE_CAST(IoObjectsOrExecutors)(io_objects_or_executors))...)),
...
async_compose(ASIO_MOVE_ARG(Implementation) implementation,
			  ASIO_NONDEDUCED_MOVE_ARG(CompletionToken) token) {
	return async_initiate<CompletionToken, Signature>(
		detail::make_initiate_composed_op<Signature>(detail::make_composed_io_executors()), token,
		ASIO_MOVE_CAST(Implementation)(implementation));
}

#define ASIO_PRIVATE_GET_COMPOSED_IO_EXECUTOR(n) ASIO_PRIVATE_GET_COMPOSED_IO_EXECUTOR_##n
...
#define ASIO_PRIVATE_ASYNC_COMPOSE_DEF(n)                                                          \
	...
	async_compose(ASIO_MOVE_ARG(Implementation) implementation,                                    \
		...
		return async_initiate<CompletionToken, Signature>(                                         \
			detail::make_initiate_composed_op<Signature>(                                          \
				detail::make_composed_io_executors(ASIO_PRIVATE_GET_COMPOSED_IO_EXECUTOR(n))),     \
			token, ASIO_MOVE_CAST(Implementation)(implementation));                                \
	}                                                                                              \
	/**/
