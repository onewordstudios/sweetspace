...
class redirect_error_handler {
   public:
	...
	redirect_error_handler(asio::error_code& ec, ASIO_MOVE_ARG(RedirectedHandler) h)
		: ec_(ec), handler_(ASIO_MOVE_CAST(RedirectedHandler)(h)) {}
	...
	typename enable_if<!is_same<typename decay<Arg>::type, asio::error_code>::value>::type
...
template <typename Handler>
inline bool asio_handler_is_continuation(redirect_error_handler<Handler>* this_handler) {
	return asio_handler_cont_helpers::is_continuation(this_handler->handler_);
}

template <typename Function, typename Handler>
inline void asio_handler_invoke(Function& function, redirect_error_handler<Handler>* this_handler) {
...
#define ASIO_PRIVATE_REDIRECT_ERROR_DEF(n)                                                \
	...
	struct redirect_error_signature<R(asio::error_code, ASIO_VARIADIC_TARGS(n))> {        \
		typedef R type(ASIO_VARIADIC_TARGS(n));                                           \
	};                                                                                    \
                                                                                          \
	template <typename R, ASIO_VARIADIC_TPARAMS(n)>                                       \
	struct redirect_error_signature<R(const asio::error_code&, ASIO_VARIADIC_TARGS(n))> { \
		typedef R type(ASIO_VARIADIC_TARGS(n));                                           \
	};                                                                                    \
	/**/
ASIO_VARIADIC_GENERATE(ASIO_PRIVATE_REDIRECT_ERROR_DEF)
...
struct async_result<redirect_error_t<CompletionToken>, Signature> {
	typedef typename async_result<
		CompletionToken, typename detail::redirect_error_signature<Signature>::type>::return_type
		return_type;
	...
	struct init_wrapper {
		...
		init_wrapper(asio::error_code& ec, ASIO_MOVE_ARG(Init) init)
			: ec_(ec), initiation_(ASIO_MOVE_CAST(Init)(init)) {}

#if defined(ASIO_HAS_VARIADIC_TEMPLATES)

		template <typename Handler, typename... Args>
		void operator()(ASIO_MOVE_ARG(Handler) handler, ASIO_MOVE_ARG(Args)... args) {
			ASIO_MOVE_CAST(Initiation)
			(initiation_)(detail::redirect_error_handler<typename decay<Handler>::type>(
...
#define ASIO_PRIVATE_INIT_WRAPPER_DEF(n)                                             \
	...
	void operator()(ASIO_MOVE_ARG(Handler) handler, ASIO_VARIADIC_MOVE_PARAMS(n)) {  \
		ASIO_MOVE_CAST(Initiation)                                                   \
		(initiation_)(detail::redirect_error_handler<typename decay<Handler>::type>( \
						  ec_, ASIO_MOVE_CAST(Handler)(handler)),                    \
...
#undef ASIO_PRIVATE_INIT_WRAPPER_DEF
	...
	static return_type initiate(ASIO_MOVE_ARG(Initiation) initiation,
		...
		return async_initiate<CompletionToken,
			...
			init_wrapper<typename decay<Initiation>::type>(token.ec_,
...
#define ASIO_PRIVATE_INITIATE_DEF(n)                                                       \
	template <typename Initiation, typename RawCompletionToken, ASIO_VARIADIC_TPARAMS(n)>  \
	static return_type initiate(ASIO_MOVE_ARG(Initiation) initiation,                      \
								ASIO_MOVE_ARG(RawCompletionToken) token,                   \
