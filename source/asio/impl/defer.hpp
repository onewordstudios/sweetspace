...
class initiate_defer {
   public:
	...
	void operator()(ASIO_MOVE_ARG(CompletionHandler) handler) const {
		...
		typename associated_executor<DecayedHandler>::type ex((get_associated_executor)(handler));
...
class initiate_defer_with_executor {
   public:
	...
	explicit initiate_defer_with_executor(const Executor& ex) : ex_(ex) {}
	...
	void operator()(ASIO_MOVE_ARG(CompletionHandler) handler) const {
		...
		ex_.defer(
			detail::work_dispatcher<DecayedHandler>(ASIO_MOVE_CAST(CompletionHandler)(handler)),
			alloc);
	}
...
defer(ASIO_MOVE_ARG(CompletionToken) token) {
	return async_initiate<CompletionToken, void()>(detail::initiate_defer(), token);
}

template <typename Executor, ASIO_COMPLETION_TOKEN_FOR(void()) CompletionToken>
ASIO_INITFN_AUTO_RESULT_TYPE(CompletionToken, void())
defer(const Executor& ex, ASIO_MOVE_ARG(CompletionToken) token,
...
inline ASIO_INITFN_AUTO_RESULT_TYPE(CompletionToken, void())
	defer(ExecutionContext& ctx, ASIO_MOVE_ARG(CompletionToken) token,
