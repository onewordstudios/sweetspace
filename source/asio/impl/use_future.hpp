...
inline void promise_invoke_and_set(std::promise<T>& p, F& f, ASIO_MOVE_ARG(Args)... args) {
...
inline void promise_invoke_and_set(std::promise<void>& p, F& f, ASIO_MOVE_ARG(Args)... args) {
#if !defined(ASIO_NO_EXCEPTIONS)
	try
#endif // !defined(ASIO_NO_EXCEPTIONS)
	{
...
#define ASIO_PRIVATE_PROMISE_INVOKE_DEF(n)                                                       \
	...
	inline void promise_invoke_and_set(std::promise<T>& p, F& f, ASIO_VARIADIC_MOVE_PARAMS(n)) { \
...
class promise_executor {
   public:
	...
	execution_context& context() const ASIO_NOEXCEPT { return system_executor().context(); }
	...
	void post(ASIO_MOVE_ARG(F) f, const A& a) const {
		system_executor().post(promise_invoker<T, F>(p_, ASIO_MOVE_CAST(F)(f)), a);
	}

	template <typename F, typename A>
	void defer(ASIO_MOVE_ARG(F) f, const A& a) const {
		system_executor().defer(promise_invoker<T, F>(p_, ASIO_MOVE_CAST(F)(f)), a);
	}

	friend bool operator==(const promise_executor& a, const promise_executor& b) ASIO_NOEXCEPT {
		return a.p_ == b.p_;
	}
...
class promise_creator {
   public:
	...
	executor_type get_executor() const ASIO_NOEXCEPT { return executor_type(p_); }
...
class promise_handler_n : public promise_creator<T> {
   public:
	...
	void operator()(ASIO_MOVE_ARG(Args)... args) {
		this->p_->set_value(std::forward_as_tuple(ASIO_MOVE_CAST(Args)(args)...));
	}
...
class promise_handler_ex_n : public promise_creator<T> {
   public:
	...
	template <typename... Args>
	void operator()(const std::exception_ptr& ex, ASIO_MOVE_ARG(Args)... args) {
		if (ex)
			this->p_->set_exception(ex);
		else {
			this->p_->set_value(std::forward_as_tuple(ASIO_MOVE_CAST(Args)(args)...));
...
#define ASIO_PRIVATE_CALL_OP_DEF(n)                                                 \
	template <ASIO_VARIADIC_TPARAMS(n)>                                             \
	void operator()(const std::exception_ptr& ex, ASIO_VARIADIC_MOVE_PARAMS(n)) {   \
		if (ex)                                                                     \
			this->p_->set_exception(ex);                                            \
		else {                                                                      \
			this->p_->set_value(std::forward_as_tuple(ASIO_VARIADIC_MOVE_ARGS(n))); \
...
template <>
class promise_handler_selector<void(asio::error_code)> : public promise_handler_ec_0 {};

template <>
class promise_handler_selector<void(std::exception_ptr)> : public promise_handler_ex_0 {};
...
#define ASIO_PRIVATE_PROMISE_SELECTOR_DEF(n)                                              \
	...
	class promise_handler_selector<void(Arg, ASIO_VARIADIC_TARGS(n))>                     \
		: public promise_handler_n<std::tuple<Arg, ASIO_VARIADIC_TARGS(n)>> {};           \
                                                                                          \
	template <typename Arg, ASIO_VARIADIC_TPARAMS(n)>                                     \
	class promise_handler_selector<void(asio::error_code, Arg, ASIO_VARIADIC_TARGS(n))>   \
		: public promise_handler_ec_n<std::tuple<Arg, ASIO_VARIADIC_TARGS(n)>> {};        \
                                                                                          \
	template <typename Arg, ASIO_VARIADIC_TPARAMS(n)>                                     \
	class promise_handler_selector<void(std::exception_ptr, Arg, ASIO_VARIADIC_TARGS(n))> \
...
class promise_handler : public promise_handler_selector<Signature> {
   public:
	...
	allocator_type get_allocator() const ASIO_NOEXCEPT { return allocator_; }
...
inline void asio_handler_invoke(Function& f, promise_handler<Signature, Allocator>* h) {
	typename promise_handler<Signature, Allocator>::executor_type ex(h->get_executor());
	ex.dispatch(ASIO_MOVE_CAST(Function)(f), std::allocator<void>());
}

template <typename Function, typename Signature, typename Allocator>
inline void asio_handler_invoke(const Function& f, promise_handler<Signature, Allocator>* h) {
	typename promise_handler<Signature, Allocator>::executor_type ex(h->get_executor());
...
class promise_async_result {
   public:
	...
	explicit promise_async_result(completion_handler_type& h) : future_(h.get_future()) {}

	return_type get() { return ASIO_MOVE_CAST(return_type)(future_); }
...
class packaged_handler : public promise_creator<Result> {
   public:
	...
	packaged_handler(packaged_token<Function, Allocator> t)
		: function_(ASIO_MOVE_CAST(Function)(t.function_)), allocator_(t.allocator_) {
		this->create_promise(allocator_);
	}
	...
	template <typename... Args>
	void operator()(ASIO_MOVE_ARG(Args)... args) {
		(promise_invoke_and_set)(*this->p_, function_, ASIO_MOVE_CAST(Args)(args)...);
	}

#else // defined(ASIO_HAS_VARIADIC_TEMPLATES)

	void operator()() { (promise_invoke_and_set)(*this->p_, function_); }

#define ASIO_PRIVATE_CALL_OP_DEF(n)                                                 \
	template <ASIO_VARIADIC_TPARAMS(n)>                                             \
	void operator()(ASIO_VARIADIC_MOVE_PARAMS(n)) {                                 \
		(promise_invoke_and_set)(*this->p_, function_, ASIO_VARIADIC_MOVE_ARGS(n)); \
	}                                                                               \
	/**/
	ASIO_VARIADIC_GENERATE(ASIO_PRIVATE_CALL_OP_DEF)
#undef ASIO_PRIVATE_CALL_OP_DEF
...
template <typename Function, typename Function1, typename Allocator, typename Result>
inline void asio_handler_invoke(Function& f, packaged_handler<Function1, Allocator, Result>* h) {
	typename packaged_handler<Function1, Allocator, Result>::executor_type ex(h->get_executor());
	ex.dispatch(ASIO_MOVE_CAST(Function)(f), std::allocator<void>());
}
...
class packaged_async_result {
   public:
	...
	explicit packaged_async_result(completion_handler_type& h) : future_(h.get_future()) {}
...
class async_result<use_future_t<Allocator>, Result(Args...)>
	: public detail::promise_async_result<void(typename decay<Args>::type...), Allocator> {
   public:
	explicit async_result(
		typename detail::promise_async_result<void(typename decay<Args>::type...),
											  Allocator>::completion_handler_type& h)
		: detail::promise_async_result<void(typename decay<Args>::type...), Allocator>(h) {}
};
...
class async_result<use_future_t<Allocator>, Result()>
   ...
   public:
	explicit async_result(
		typename detail::promise_async_result<void(), Allocator>::completion_handler_type& h)
   ...
   public:
	explicit async_result(
		typename detail::packaged_async_result<
			Function, Allocator, typename result_of<Function()>::type>::completion_handler_type& h)
		: detail::packaged_async_result<Function, Allocator, typename result_of<Function()>::type>(
			  h) {}
};
...
#define ASIO_PRIVATE_ASYNC_RESULT_DEF(n)                                                           \
	...
	class async_result<use_future_t<Allocator>, Result(ASIO_VARIADIC_TARGS(n))>                    \
		: public detail::promise_async_result<void(ASIO_VARIADIC_DECAY(n)), Allocator> {           \
	   public:                                                                                     \
		explicit async_result(                                                                     \
			typename detail::promise_async_result<void(ASIO_VARIADIC_DECAY(n)),                    \
												  Allocator>::completion_handler_type& h)          \
			: detail::promise_async_result<void(ASIO_VARIADIC_DECAY(n)), Allocator>(h) {}          \
	};                                                                                             \
                                                                                                   \
	template <typename Function, typename Allocator, typename Result, ASIO_VARIADIC_TPARAMS(n)>    \
	class async_result<detail::packaged_token<Function, Allocator>,                                \
		...
		: public detail::packaged_async_result<                                                    \
			  Function, Allocator, typename result_of<Function(ASIO_VARIADIC_TARGS(n))>::type> {   \
	   public:                                                                                     \
		explicit async_result(                                                                     \
			typename detail::packaged_async_result<                                                \
				Function, Allocator, typename result_of<Function(ASIO_VARIADIC_TARGS(n))>::type>:: \
