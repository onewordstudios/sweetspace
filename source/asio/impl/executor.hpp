...
class executor::function {
   public:
	...
	explicit function(F f, const Alloc& a) {
		...
		typename func_type::ptr p = {detail::addressof(a), func_type::ptr::allocate(a), 0};
		func_ = new (p.v) func_type(ASIO_MOVE_CAST(F)(f), a);
		p.v = 0;
	}

	function(function&& other) ASIO_NOEXCEPT : func_(other.func_) { other.func_ = 0; }

	~function() {
		if (func_) func_->destroy();
	}
...
class executor::function {
   public:
	...
	void operator()() { impl_->invoke_(impl_.get()); }
   ...
   private:
	...
	struct impl : impl_base {
		impl(const F& f) : function_(f) { invoke_ = &function::invoke<F>; }
...
class executor::impl : public executor::impl_base {
   public:
	...
	impl(const Executor& e, const Allocator& a) ASIO_NOEXCEPT : impl_base(false),
																ref_count_(1),
																executor_(e),
																allocator_(a) {}
	...
	void on_work_started() ASIO_NOEXCEPT { executor_.on_work_started(); }

	void on_work_finished() ASIO_NOEXCEPT { executor_.on_work_finished(); }

	execution_context& context() ASIO_NOEXCEPT { return executor_.context(); }
	...
	type_id_result_type target_type() const ASIO_NOEXCEPT { return type_id<Executor>(); }

	void* target() ASIO_NOEXCEPT { return &executor_; }

	const void* target() const ASIO_NOEXCEPT { return &executor_; }

	bool equals(const impl_base* e) const ASIO_NOEXCEPT {
		if (this == e) return true;
		if (target_type() != e->target_type()) return false;
   ...
   private:
	...
	struct raw_mem {
		...
		~raw_mem() {
			if (ptr_) allocator_.deallocate(ptr_, 1);
		}
...
class executor::impl<system_executor, Allocator> : public executor::impl_base {
   public:
	...
	type_id_result_type target_type() const ASIO_NOEXCEPT { return type_id<system_executor>(); }

	void* target() ASIO_NOEXCEPT { return &executor_; }

	const void* target() const ASIO_NOEXCEPT { return &executor_; }

	bool equals(const impl_base* e) const ASIO_NOEXCEPT { return this == e; }
...
executor::executor(Executor e) : impl_(impl<Executor, std::allocator<void> >::create(e)) {}
