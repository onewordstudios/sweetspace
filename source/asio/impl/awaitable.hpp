   ...
   public:
	...
	void* operator new(std::size_t size) {
		return asio::detail::thread_info_base::allocate(
			...
			asio::detail::thread_context::thread_call_stack::top(), size);
	}

	void operator delete(void* pointer, std::size_t size) {
		asio::detail::thread_info_base::deallocate(
			asio::detail::thread_info_base::awaitable_frame_tag(),
			asio::detail::thread_context::thread_call_stack::top(), pointer, size);
	}
	...
	auto final_suspend() noexcept {
		struct result {
			...
			void await_suspend(coroutine_handle<void>) noexcept { this_->pop_frame(); }
	...
	auto await_transform(
		Function f,
		typename enable_if<is_convertible<typename result_of<Function(awaitable_frame_base*)>::type,
										  awaitable_thread<Executor>*>::value>::type* = 0) {
		struct result {
			Function function_;
			awaitable_frame_base* this_;
	...
	void pop_frame() noexcept {
		if (caller_) caller_->attached_thread_ = attached_thread_;
...
class awaitable_frame : public awaitable_frame_base<Executor> {
   public:
	...
	~awaitable_frame() {
		if (has_result_) static_cast<T*>(static_cast<void*>(result_))->~T();
	}
...
class awaitable_thread {
   public:
	...
	awaitable_thread(awaitable<void, Executor> p, const Executor& ex)
		: bottom_of_stack_(std::move(p)), top_of_stack_(bottom_of_stack_.frame_), executor_(ex) {}
