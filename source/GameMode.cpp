...
bool GameMode::init(const std::shared_ptr<cugl::AssetManager>& assets,
	...
	float lvlSize = 360; // TODO level size comes from level file
	ship = ShipModel::alloc(net->getNumPlayers(), MAX_EVENTS, MAX_DOORS, playerID, lvlSize);
	gm.init(ship, net, lvlSize);
