...
void UDPForwarder::RecvFrom(RakNet::TimeMS curTime, ForwardEntry *forwardEntry) {
...
#if RAKNET_SUPPORT_IPV6 == 1
	receivedDataLen =
		recvfrom__(forwardEntry->socket, data, MAXIMUM_MTU_SIZE, flag, sockAddrPtr, socketlenPtr);
#else
	receivedDataLen = recvfrom__(forwardEntry->socket, data, MAXIMUM_MTU_SIZE, flag,
								 (sockaddr *)&sockAddrIn, (socklen_t *)&len2);
#endif
	...
	if (matchConfirmed1 == true ||
		(matchConfirmed2 == false && confirmed1 == false && matchUnconfirmed1 == true)) {
		// Forward to addr2
	...
	do {
		...
		if (forwardTarget.address.addr4.sin_family == AF_INET) {
			...
			do {
				len =
					sendto__(forwardEntry->socket, data, receivedDataLen, 0,
	...
	while (1) {
		...
		if (GetUsedForwardEntries() > maxForwardEntries) {
			...
			if (sfos.result == UDPFORWARDER_RESULT_COUNT) {
				...
				int ret = bind__(fe->socket, (struct sockaddr *)&listenerSocketAddress,
				...
				if (sfos.result == UDPFORWARDER_SUCCESS) {
					...
					sock_opt = 1024 * 256;
					setsockopt__(fe->socket, SOL_SOCKET, SO_RCVBUF, (char *)&sock_opt,
								 sizeof(sock_opt));
					sock_opt = 0;
					setsockopt__(fe->socket, SOL_SOCKET, SO_LINGER, (char *)&sock_opt,
								 sizeof(sock_opt));
#ifdef _WIN32
...
#pragma warning(disable : 4127) // warning C4127: conditional expression is constant
	...
	while (i < forwardListNotUpdated.Size()) {
		if (curTime >
				forwardListNotUpdated[i]->timeLastDatagramForwarded && // Account for timestamp wrap
