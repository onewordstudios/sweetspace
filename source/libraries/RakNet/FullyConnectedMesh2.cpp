...
void FullyConnectedMesh2::AddParticipant(RakNetGUID rakNetGuid) {
	if (rakPeerInterface->GetConnectionState(
			rakPeerInterface->GetSystemAddressFromGuid(rakNetGuid)) != IS_CONNECTED) {
#ifdef DEBUG_FCM2
...
void FullyConnectedMesh2::OnClosedConnection(const SystemAddress &systemAddress,
		...
		if (joinsInProgress[idx]->requester == rakNetGUID) {
			...
			for (unsigned int j = 0; j < joinsInProgress[idx]->vjipMembers.Size(); j++) {
				if (joinsInProgress[idx]->vjipMembers[j].userData != 0) {
					RakNet::OP_DELETE(joinsInProgress[idx]->vjipMembers[j].userData,
									  _FILE_AND_LINE_);
				}
			}
...
void FullyConnectedMesh2::OnNewConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
										  bool isIncoming) {
	(void)isIncoming;
	(void)rakNetGUID;
	(void)systemAddress;
...
void FullyConnectedMesh2::OnFailedConnectionAttempt(
	...
	if (failedConnectionAttemptReason == FCAR_ALREADY_CONNECTED) {
		...
		UpdateVerifiedJoinInProgressMember(packet->systemAddress, UNASSIGNED_RAKNET_GUID,
										   JIPS_FAILED);
	}
}
...
void FullyConnectedMesh2::AssignOurFCMGuid(void) {
	...
	randomNumber ^=
		(unsigned int)(rakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS).g &
					   0xFFFFFFFF);
...
void FullyConnectedMesh2::OnRespondConnectionCount(Packet *packet) {
	...
	*/
	...
	if (wasAssigned == true) {
		...
		for (idx = 0; idx < fcm2ParticipantList.Size(); idx++)
			SendOurFCMGuid(
				rakPeerInterface->GetSystemAddressFromGuid(fcm2ParticipantList[idx]->rakNetGuid));
		CalculateAndPushHost();
	}
}
void FullyConnectedMesh2::OnInformFCMGuid(Packet *packet) {
	...
	if (AddParticipantInternal(packet->guid, theirFCMGuid)) {
		// 1/19/2010 - Relay increased total connection count in case new participant only connects
		// to part of the mesh
		unsigned int idx;
		RakNet::BitStream bsOut;
		...
		for (idx = 0; idx < fcm2ParticipantList.Size(); idx++) {
			if (packet->guid != fcm2ParticipantList[idx]->rakNetGuid)
				rakPeerInterface->Send(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0,
									   fcm2ParticipantList[idx]->rakNetGuid, false);
		}
	}
...
void FullyConnectedMesh2::RespondOnVerifiedJoinCapable(Packet *packet, bool accept,
	...
	if (participatingMembersNotOnClient.Size() > 0) {
		...
		for (i = 0; i < participatingMembersNotOnClient.Size(); i++) {
			...
			bsOut.Write(
				rakPeerInterface->GetSystemAddressFromGuid(participatingMembersNotOnClient[i]));
	...
	if (accept) {
		...
		for (unsigned int i = 0; i < fcm2ParticipantList.Size(); i++)
			SendUnified(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0,
						fcm2ParticipantList[i]->rakNetGuid, false);
...
void FullyConnectedMesh2::GetVerifiedJoinRejectedAdditionalData(Packet *packet,
																BitStream *additionalData) {
	RakNet::BitStream bsIn(packet->data, packet->length, false);
	bsIn.IgnoreBytes(sizeof(MessageID));
	if (additionalData) {
...
void FullyConnectedMesh2::OnVerifiedJoinAccepted(Packet *packet) {
	...
	if (systemToAddGuid == rakPeerInterface->GetMyGUID()) {
		...
		for (unsigned int j = 0; j < joinsInProgress[curIndex]->vjipMembers.Size(); j++) {
			if (joinsInProgress[curIndex]->vjipMembers[j].userData != 0) {
				RakNet::OP_DELETE(joinsInProgress[curIndex]->vjipMembers[j].userData,
								  _FILE_AND_LINE_);
			}
		}
...
void FullyConnectedMesh2::UpdateVerifiedJoinInProgressMember(
	const AddressOrGUID systemIdentifier, RakNetGUID guidToAssign,
...
void FullyConnectedMesh2::WriteVerifiedJoinCapable(RakNet::BitStream *bsOut,
												   VerifiedJoinInProgress *vjip) {
	bsOut->Write((MessageID)ID_FCM2_VERIFIED_JOIN_CAPABLE);
...
unsigned int FullyConnectedMesh2::GetVerifiedJoinInProgressMemberIndex(
	const AddressOrGUID systemIdentifier, VerifiedJoinInProgress *vjip) {
	for (unsigned int j = 0; j < vjip->vjipMembers.Size(); j++) {
...
void FullyConnectedMesh2::CategorizeVJIP(
	...
	for (unsigned int i = 0; i < fcm2ParticipantList.Size(); i++) {
		unsigned int j =
			GetVerifiedJoinInProgressMemberIndex(fcm2ParticipantList[i]->rakNetGuid, vjip);
		if (j == (unsigned int)-1) {
