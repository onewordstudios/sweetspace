...
bool UDPProxyServer::LoginToCoordinator(RakNet::RakString password,
	...
	rakPeerInterface->Send(&outgoingBs, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0, coordinatorAddress,
						   false);
	...
	if (packet->data[0] == ID_UDP_PROXY_GENERAL && packet->length > 1) {
		...
		switch (packet->data[1]) {
			case ID_UDP_PROXY_FORWARDING_REQUEST_FROM_COORDINATOR_TO_SERVER:
				if (loggedInCoordinators.GetIndexFromKey(packet->systemAddress, &objectExists) !=
					(unsigned int)-1) {
			...
			case ID_UDP_PROXY_LOGIN_SUCCESS_FROM_COORDINATOR_TO_SERVER: {
				unsigned int removalIndex =
					loggingInCoordinators.GetIndexFromKey(packet->systemAddress, &objectExists);
				if (objectExists) {
...
void UDPProxyServer::OnForwardingRequestFromCoordinatorToServer(Packet *packet) {
	...
	UDPForwarderResult success = udpForwarder.StartForwarding(
		sourceAddress, targetAddress, timeoutOnNoDataMS, 0, socketFamily, &forwardingPort, 0);
	RakNet::BitStream outgoingBs;
	outgoingBs.Write((MessageID)ID_UDP_PROXY_GENERAL);
