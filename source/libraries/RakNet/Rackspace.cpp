...
SystemAddress Rackspace::Authenticate(TCPInterface *_tcpInterface, const char *_authenticationURL,
	...
	tcpInterface->Send(command.C_String(), (unsigned int)command.GetLength(), ro.connectionAddress,
					   false);
...
void Rackspace::AddOperation(RackspaceOperationType type, RakNet::RakString httpCommand,
...
void Rackspace::CreateServer(RakNet::RakString name, RakNet::RakString imageId,
	...
	RakNet::RakString xml(
		...
		"<server xmlns=\"http://docs.rackspacecloud.com/servers/api/v1.0\" name=\"%s\" "
		"imageId=\"%s\" flavorId=\"%s\">"
		"</server>",
...
void Rackspace::DeleteServerAddress(RakNet::RakString serverId, RakNet::RakString ipAddress) {
	AddOperation(
		RO_DELETE_SERVER_ADDRESS, "DELETE",
		RakNet::RakString("servers/%s/ips/public/%s", serverId.C_String(), ipAddress.C_String()),
		"");
}
	...
	while (operationsIndex < operations.Size()) {
		if (operations[operationsIndex].isPendingAuthentication == false &&
			...
			if (result != 0) {
				...
				for (i = 0; i < sizeof(resultCodeStr) - 1 && result[i] && result[i] >= '0' &&
							result[i] <= '9';
					 i++)
			...
			switch (ro.type) {
				...
				case RO_LIST_SERVERS_WITH_DETAILS: {
					for (i = 0; i < eventCallbacks.Size(); i++)
						eventCallbacks[i]->OnListServersWithDetailsResult(rackspaceEventType,
																		  (const char *)packetData);
					break;
				}
				...
				case RO_UPDATE_SERVER_NAME_OR_PASSWORD: {
					for (i = 0; i < eventCallbacks.Size(); i++)
						eventCallbacks[i]->OnUpdateServerNameOrPassword(rackspaceEventType,
																		(const char *)packetData);
					break;
				}
				...
				case RO_GET_SHARED_IP_GROUP_DETAILS: {
					for (i = 0; i < eventCallbacks.Size(); i++)
						eventCallbacks[i]->OnGetSharedIPGroupDetails(rackspaceEventType,
																	 (const char *)packetData);
					break;
				}
