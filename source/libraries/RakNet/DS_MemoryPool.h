...
class RAK_DLL_EXPORT MemoryPool {
   ...
   protected:
	...
	// which we do not return new blocks. Pages move from the head of unavailablePages to the tail
	// of availablePages, and from the head of availablePages to the tail of unavailablePages
...
template <class MemoryBlockType>
bool MemoryPool<MemoryBlockType>::InitPage(Page *page, Page *prev, const char *file,
										   unsigned int line) {
	int i = 0;
