...
class RAK_DLL_EXPORT LogCommandParser : public CommandParserInterface {
   public:
	...
	/// All logs must be associated with a channel.  This is a filter so that remote clients only
	/// get logs for a system they care about.
	// If you call Log with a channel that is unknown, that channel will automatically be added
	/// \param[in] channelName A persistent string naming the channel.  Don't deallocate this
	/// string.
   ...
   protected:
	...
	struct SystemAddressAndChannel {
		...
		/// Bitwise representations of the channels subscribed to.  If bit 0 is set, then we
		/// subscribe to channelNames[0] and so on.
		unsigned channels;
	};
