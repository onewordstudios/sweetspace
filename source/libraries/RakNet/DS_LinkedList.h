...
CircularLinkedList<CircularLinkedListType>::FindPointer(const CircularLinkedListType& input) {
	node* current;
	...
	// Search for the item starting from the root node and incrementing the pointer after every
	// check If you wind up pointing at the root again you looped around the list so didn't find the
	// item, in which case return 0
	do {
...
inline void CircularLinkedList<CircularLinkedListType>::Concatenate(
	...
	for (counter = 0; counter < L.list_size; counter++) {
		...
		// Update pointers.  Moving ptr keeps the current pointer at the end of the list since the
		// add function does not move the pointer
		ptr = ptr->next;
...
template <class CircularLinkedListType>
CircularLinkedList<CircularLinkedListType> CircularLinkedList<CircularLinkedListType>::Merge(
