...
RNS2BindResult RNS2_NativeClient::Bind(NativeClientBindParameters *bindParameters, const char *file,
	...
	if (Module::Get()->GetBrowserInterface(PPB_UDPSOCKET_PRIVATE_INTERFACE_0_4)) {
		rns2Socket = ((PPB_UDPSocket_Private_0_4 *)Module::Get()->GetBrowserInterface(
		...
		if (bindParameters->forceHostAddress) {
			...
			if (bindParameters->is_ipv6) {
				...
				NetAddressPrivate::CreateFromIPv6Address(ipv6, 0, bindParameters->port,
														 &client_addr);
			} else {
...
void RNS2_NativeClient::SendImmediate(RNS2_SendParameters_NativeClient *sp) {
	...
	NetAddressPrivate::CreateFromIPv4Address(
		(const uint8_t *)&sp->systemAddress.address.addr4.sin_addr, sp->systemAddress.GetPort(),
		&client_addr);
#endif
	...
	if (((PPB_UDPSocket_Private_0_4 *)Module::Get()->GetBrowserInterface(
		...
		if (family == PP_NETADDRESSFAMILY_IPV4) {
			ok = NetAddressPrivate::GetAddress(
				addr, &recvStruct->systemAddress.address.addr4.sin_addr, sizeof(in_addr));
		}
#if RAKNET_SUPPORT_IPV6 == 1
		else {
			ok = NetAddressPrivate::GetAddress(
				addr, &recvStruct->systemAddress.address.addr6.sin6_addr, sizeof(in6_addr));
		}
#endif
	}
