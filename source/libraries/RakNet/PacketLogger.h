...
class RAK_DLL_EXPORT PacketLogger : public PluginInterface2 {
   public:
	...
	// messageIdentifier
	virtual void FormatLine(char* into, const char* dir, const char* type,
							unsigned int reliableMessageNumber, unsigned int frame,
							unsigned char messageIdentifier, const BitSize_t bitLen,
	...
	virtual void FormatLine(char* into, const char* dir, const char* type,
							unsigned int reliableMessageNumber, unsigned int frame,
							const char* idToPrint, const BitSize_t bitLen, unsigned long long time,
	...
	/// Events on low level sends and receives.  These functions may be called from different
	...
	virtual void OnInternalPacket(InternalPacket* internalPacket, unsigned frameNumber,
								  ...
								  int isSend);
	virtual void OnAck(unsigned int messageNumber, SystemAddress remoteSystemAddress,
					   RakNet::TimeMS time);
