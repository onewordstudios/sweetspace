...
#else
	if (key < data->splitPacketList[0]->splitPacketId) return -1;
	if (key == data->splitPacketList[0]->splitPacketId) return 0;
#endif
	return 1;
}
...
bool ReliabilityLayer::HandleSocketReceiveFromConnectedPlayer(
	...
	if (length <= 2 ||
		buffer == 0) // Length of 1 is a connection request resend that we just ignore
	{
	...
	if (dhf.isACK) {
...
#endif
		{
			// Sanity check. This could happen due to type overflow, especially since I only send
			// the low 4 bytes to reduce bandwidth
		...
		if (incomingAcks.Deserialize(&socketData) == false) {
			for (unsigned int messageHandlerIndex = 0;
				 messageHandlerIndex < messageHandlerList.Size(); messageHandlerIndex++)
				messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
			...
			for (datagramNumber = incomingAcks.ranges[i].minIndex;
				...
				if (messageNumberNode) {
...
#if INCLUDE_TIMESTAMP_WITH_DATAGRAMS == 1
					congestionManager.OnAck(timeRead, rtt, dhf.hasBAndAS, 0, dhf.AS,
											totalUserDataBytesAcked, bandwidthExceededStatistic,
											datagramNumber);
#else
					CCTimeType ping;
					if (timeRead > whenSent)
						...
						ping = 0;
					congestionManager.OnAck(timeRead, ping, dhf.hasBAndAS, 0, dhf.AS,
											totalUserDataBytesAcked, bandwidthExceededStatistic,
											datagramNumber);
#endif
					while (messageNumberNode) {
						...
						RemovePacketFromResendListAndDeleteOlderReliableSequenced(
							messageNumberNode->messageNumber, timeRead, messageHandlerList,
							systemAddress);
		...
		for (i = 0; i < incomingNAKs.ranges.Size(); i++) {
			if (incomingNAKs.ranges[i].minIndex > incomingNAKs.ranges[i].maxIndex) {
				...
				for (unsigned int messageHandlerIndex = 0;
					 messageHandlerIndex < messageHandlerList.Size(); messageHandlerIndex++)
					messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
						"incomingNAKs minIndex>maxIndex", BYTES_TO_BITS(length), systemAddress,
						true);
			...
			for (messageNumber = incomingNAKs.ranges[i].minIndex;
				...
				while (messageNumberNode) {
					...
					InternalPacket *internalPacket =
						resendBuffer[messageNumberNode->messageNumber &
		...
		if (!congestionManager.OnGotPacket(dhf.datagramNumber, dhf.isContinuousSend, timeRead,
										   length, &skippedMessageCount)) {
			for (unsigned int messageHandlerIndex = 0;
				 messageHandlerIndex < messageHandlerList.Size(); messageHandlerIndex++)
				messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
					"congestionManager.OnGotPacket failed", BYTES_TO_BITS(length), systemAddress,
					true);
		...
		if (internalPacket == 0) {
			for (unsigned int messageHandlerIndex = 0;
				 messageHandlerIndex < messageHandlerList.Size(); messageHandlerIndex++)
				messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
					"CreateInternalPacketFromBitStream failed", BYTES_TO_BITS(length),
					systemAddress, true);
		...
		while (internalPacket) {
...

			{
				...
				// packetloss this would mean you'd eventually exceed the hole count because
				// unreliable messages were never resent, and you'd stop getting messages
				if (internalPacket->reliability == RELIABLE ||
					...
					holeCount = (DatagramSequenceNumberType)(internalPacket->reliableMessageNumber -
															 receivedPacketsBaseIndex);
					const DatagramSequenceNumberType typeRange =
						(DatagramSequenceNumberType)(const uint32_t)-1;

					// TESTING1
					// 					printf("waiting on reliableMessageNumber=%i holeCount=%i
					...
					if (holeCount == (DatagramSequenceNumberType)0) {
						...
						// Got a higher count out of order packet that was missing in the sequence
						// or we already got
							...
							false) // non-zero means this is a hole
						{
							...
							for (unsigned int messageHandlerIndex = 0;
								 ...
								 messageHandlerIndex++)
								messageHandlerList[messageHandlerIndex]
									->OnReliabilityLayerNotification("Duplicate packet ignored",
					...
					} else // holeCount>=receivedPackets.Size()
					{
						if (holeCount > (DatagramSequenceNumberType)1000000) {
							...
							for (unsigned int messageHandlerIndex = 0;
								 ...
								 messageHandlerIndex++)
								messageHandlerList[messageHandlerIndex]
									->OnReliabilityLayerNotification("holeCount > 1000000",
																	 BYTES_TO_BITS(length),
																	 systemAddress, true);
						...
						// Fix - sending on a higher priority gives us a very very high received
						// packets base index if we formerly had pre-split a lot of messages and
						// used that as the message number.  Because of this, a lot of time is spent
						// in this linear loop and the timeout time expires because not all of the
						// message is sent in time. Fixed by late assigning message IDs on the
						// sender

						// Add 0 times to the queue until (reliableMessageNumber - baseIndex) <
						// queue size.
						while ((unsigned int)(holeCount) > hasReceivedPacketQueue.Size())
							hasReceivedPacketQueue.Push(
								true, _FILE_AND_LINE_); // time+(CCTimeType)60 * (CCTimeType)1000 *
														// (CCTimeType)1000); // Didn't get this
														// packet - set the time to give up waiting
...
internalPacket->splitPacketId, timeRead, s, systemAddress, rnr, remotePortRakNetWasStartedOn_PS3,
extraSocketOptions);

							if ( internalPacket )
							{
								// Update our index to the newest packet
								waitingForSequencedPacketReadIndex[ internalPacket->orderingChannel
] = internalPacket->orderingIndex + (OrderingIndexType)1;
...
USER_MESSAGE_BYTES_RECEIVED_IGNORED].Push1(timeRead,BITS_TO_BYTES(internalPacket->dataBitLength));
				...
				if ( internalPacket->splitPacketCount > 0 )
				{
					...
					if ( internalPacket->reliability != RELIABLE_ORDERED )
						internalPacket->orderingChannel = 255; // Use 255 to designate not sequenced
and not ordered
...
internalPacket->orderingIndex )
					{
						...
						outputQueue.Push( internalPacket, _FILE_AND_LINE_  );
						internalPacket = 0; // Don't reference this any longer since other threads
access it
...
orderingChannelCopy );
						...
						if ( orderingListAtOrderingStream != 0)
						{
							while ( orderingListAtOrderingStream->Size() > 0 )
							{
								...
								while (count++ < size)
								{
									if ( orderingListAtOrderingStream->Peek()->orderingIndex ==
waitingForOrderedPacketReadIndex[ orderingChannelCopy ] )
									{
										outputQueue.Push( orderingListAtOrderingStream->Pop(),
_FILE_AND_LINE_  ); waitingForOrderedPacketReadIndex[ orderingChannelCopy ]++; indexFound=true;
									}
									else
...
future use AddToOrderingList( internalPacket );
				...
				*/
				...
				if (internalPacket->splitPacketCount > 0) {
					...
					internalPacket =
						BuildPacketFromSplitPacketList(internalPacket->splitPacketId, timeRead, s,
				...
				if (internalPacket->reliability == RELIABLE_SEQUENCED ||
					...
					// ___________________
					BitStream bitStream(internalPacket->data,
										BITS_TO_BYTES(internalPacket->dataBitLength), false);
					...
					if (internalPacket->orderingIndex ==
							...
							if (IsOlderOrderedPacket(
								...
								if (packetId == ID_USER_PACKET_ENUM + 1) {
									if (receivedPacketNumber < packetNumber) {
										if (fp) {
											fprintf(fp,
													"Out of order packet from fallthrough! "
													"Expecting %i got %i\n",
								...
								// Means a duplicated RELIABLE_SEQUENCED or UNRELIABLE_SEQUENCED
								// packet would be returned to the user
								highestSequencedReadIndex[internalPacket->orderingChannel] =
...
#ifdef PRINT_TO_FILE_RELIABLE_ORDERED_TEST
							if (packetId == ID_USER_PACKET_ENUM + 1 && fp) {
								fprintf(fp, "outputting immediate %i, %s. OI=%i. SI=%i.",
										receivedPacketNumber, type,
										internalPacket->orderingIndex.val,
										internalPacket->sequencingIndex);
							...
							while (orderingHeaps[internalPacket->orderingChannel].Size() > 0 &&
...
#ifdef PRINT_TO_FILE_RELIABLE_ORDERED_TEST
								BitStream bitStream2(internalPacket->data,
								...
								if (packetId == ID_USER_PACKET_ENUM + 1 && fp) {
									...
									if (receivedPacketNumber < packetNumber) {
										if (packetId == ID_USER_PACKET_ENUM + 1 && fp) {
											fprintf(fp,
													"Out of order packet from heap! Expecting %i "
													"got %i\n",
													receivedPacketNumber, packetNumber);
											fflush(fp);
										}
						...
						// If a message has a greater ordering index, and is sequenced or ordered,
						// buffer it Sequenced has a lower heap weight, ordered has max sequenced
						// weight
						...
						if (packetId == ID_USER_PACKET_ENUM + 1 && fp) {
							fprintf(fp,
									...
									receivedPacketNumber, type, weight,
									internalPacket->orderingIndex.val,
						...
						for (unsigned int messageHandlerIndex = 0;
							 messageHandlerIndex < messageHandlerList.Size(); messageHandlerIndex++)
							messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
								"Larger number ordered packet leaving holes", BYTES_TO_BITS(length),
								systemAddress, false);
#endif
						...
						for (unsigned int messageHandlerIndex = 0;
							 messageHandlerIndex < messageHandlerList.Size(); messageHandlerIndex++)
							messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
								"Rejected older resend", BYTES_TO_BITS(length), systemAddress,
								false);
#endif
...
bool ReliabilityLayer::Send(char *data, BitSize_t numberOfBitsToSend, PacketPriority priority,
	...
	if (makeDataCopy) {
		...
		// internalPacket->data = (unsigned char*) rakMalloc_Ex( numberOfBytesToSend,
		// _FILE_AND_LINE_ );
	...
	unsigned int maxDataSizeBytes = GetMaxDatagramSizeExcludingMessageHeaderBytes() -
	...
	if (internalPacket->reliability == RELIABLE_SEQUENCED ||
		...
		// example, with sequenced unreliable sound packets just because you send a newer one
		// doesn't mean you don't need the older ones because the odds are they will still arrive in
		// order
		/*
		for (int i=0; i < NUMBER_OF_PRIORITIES; i++)
		{
	...
	{
		// Must split the packet.  This will also generate the SHA1 if it is required. It also adds
		// it to the send list.
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::Update(RakNetSocket2 *s, SystemAddress &systemAddress, int MTUSize,
...

{
	...
	while (delayList.Size()) {
		if (delayList.Peek()->sendTime <= timeMs) {
			...
			//			SocketLayer::SendTo( dat->s, dat->data, dat->length, systemAddress, __FILE__,
			//__LINE__  );
...
#pragma warning(disable : 4127) // warning C4127: conditional expression is constant
				...
				while (1) {
					if (time > cur->creationTime + (CCTimeType)unreliableTimeout) {
						...
						// sendPacketSet later This fixes a problem where a remote system
						// disconnects, but we don't know it yet, and memory consumption increases
						// to a huge value
						...
						// They should be inserted in-order, so no need to iterate past the first
						// failure
						break;
					}
				}
	...
	if (unreliableWithAckReceiptHistory.Size() > 0) {
		...
		while (i < unreliableWithAckReceiptHistory.Size()) {
			...
			if (time - unreliableWithAckReceiptHistory[i].nextActionTime < (((CCTimeType)-1) / 2)) {
				...
				memcpy(ackReceipt->data + sizeof(MessageID),
					   &unreliableWithAckReceiptHistory[i].sendReceiptSerial, sizeof(uint32_t));
				outputQueue.Push(ackReceipt, _FILE_AND_LINE_);
			...
			while ((int)BITS_TO_BYTES(allDatagramSizesSoFar) < retransmissionBandwidth) {
				...
				while (IsResendQueueEmpty() == false) {
					...
					if (time - internalPacket->nextActionTime < (((CCTimeType)-1) / 2)) {
						...
						PushPacket(time, internalPacket,
						...
						// Filled one datagram.
						// If the 2nd and it's time to send a datagram pair, will be marked as a
						// pair
						PushDatagram();
						break;
					}
		...
		if ((int)BITS_TO_BYTES(allDatagramSizesSoFar) < transmissionBandwidth) {
			...
			while (
				...
				// while ( sendPacketSet[ i ].Size() )
				{
					...
					if (isReliable
						/*
						I thought about this and agree that UNRELIABLE_SEQUENCED_WITH_ACK_RECEIPT
						and RELIABLE_SEQUENCED_WITH_ACK_RECEIPT is not useful unless you also know
						if the message was discarded.

						The problem is that internally, message numbers are only assigned to
						reliable messages, because message numbers are only used to discard
						duplicate message receipt and only reliable messages get sent more than
						once. However, without message numbers getting assigned and transmitted,
						there is no way to tell the sender about which messages were discarded. In
						fact, in looking this over I realized that
						UNRELIABLE_SEQUENCED_WITH_ACK_RECEIPT introduced a bug, because the remote
						system assumes all message numbers are used (no holes). With that send type,
						on packetloss, a permanent hole would have been created which eventually
						would cause the system to discard all further packets.

						So I have two options. Either do not support ack receipts when sending
						sequenced, or write complex and major new systems.
						UNRELIABLE_SEQUENCED_WITH_ACK_RECEIPT would need to send the message ID
						number on a special channel which allows for non-delivery. And both of them
						would need to have a special range list to indicate which message numbers
						were not delivered, so when acks are sent that can be indicated as well. A
						further problem is that the ack itself can be lost - it is possible that the
						message can arrive but be discarded, yet the ack is lost. On resend, the
						resent message would be ignored as duplicate, and you'd never get the
...
*/
						...
						// resendTree.Insert( internalPacket->reliableMessageNumber,
						// internalPacket);
						if (resendBuffer[internalPacket->reliableMessageNumber &
										 (uint32_t)RESEND_BUFFER_ARRAY_MASK] != 0) {
							//								bool overflow = ResendBufferOverflow();
							RakAssert(0);
						}
						resendBuffer[internalPacket->reliableMessageNumber &
					...
					// If isReliable is false, the packet and its contents will be added to a list
					// to be freed in ClearPacketsAndDatagrams However, the internalPacket structure
					// will remain allocated and be in the resendBuffer list if it requires a
					// receipt
		...
		for (unsigned int datagramIndex = 0;
			 datagramIndex < packetsToSendThisUpdateDatagramBoundaries.Size(); datagramIndex++) {
			...
			while (msgIndex < msgTerm) {
				...
				if (packetsToSendThisUpdate[msgIndex]->reliability != UNRELIABLE &&
					...
					if (messageNumberNode == 0) {
						messageNumberNode = AddFirstToDatagramHistory(
							dhf.datagramNumber,
							packetsToSendThisUpdate[msgIndex]->reliableMessageNumber, time);
					} else {
				...
				RakAssert(updateBitStream.GetNumberOfBytesUsed() <=
						  MAXIMUM_MTU_SIZE - UDP_HEADER_SIZE);
				WriteToBitStreamFromInternalPacket(&updateBitStream,
												   packetsToSendThisUpdate[msgIndex], time);
				RakAssert(updateBitStream.GetNumberOfBytesUsed() <=
						  MAXIMUM_MTU_SIZE - UDP_HEADER_SIZE);
				msgIndex++;
			}
			...
			congestionManager.OnSendBytes(
				time, UDP_HEADER_SIZE + DatagramHeaderFormat::GetDataHeaderByteLength());
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::SendBitStream(RakNetSocket2 *s, SystemAddress &systemAddress,
...
unsigned ReliabilityLayer::RemovePacketFromResendListAndDeleteOlderReliableSequenced(
	...
	if (internalPacket && internalPacket->reliableMessageNumber == messageNumber) {
		...
		totalUserDataBytesAcked +=
			(double)BITS_TO_BYTES(internalPacket->headerLength + internalPacket->dataBitLength);
...
BitSize_t ReliabilityLayer::GetMessageHeaderLengthBits(const InternalPacket *const internalPacket) {
	...
	if (internalPacket->reliability == RELIABLE ||
		...
		//		internalPacket->reliability == RELIABLE_SEQUENCED_WITH_ACK_RECEIPT ||
		internalPacket->reliability == RELIABLE_ORDERED_WITH_ACK_RECEIPT)
		bitLength += 8 * 3; // bitStream->Write(internalPacket->reliableMessageNumber); // Message
							// sequence number
	...
	if (internalPacket->reliability == UNRELIABLE_SEQUENCED ||
		...
		bitLength +=
			8 * 1; // tempChar=internalPacket->orderingChannel; bitStream->WriteAlignedVar8((const
				   // char*)& tempChar); // Used for UNRELIABLE_SEQUENCED, RELIABLE_SEQUENCED,
				   // RELIABLE_ORDERED. 5 bits needed, write one byte
	}
	if (internalPacket->splitPacketCount > 0) {
		bitLength +=
			8 * 4; // bitStream->WriteAlignedVar32((const char*)& internalPacket->splitPacketCount);
		...
		bitLength +=
			8 * 4; // bitStream->WriteAlignedVar32((const char*)& internalPacket->splitPacketIndex);
...
BitSize_t ReliabilityLayer::WriteToBitStreamFromInternalPacket(
	RakNet::BitStream *bitStream, const InternalPacket *const internalPacket, CCTimeType curTime) {
	(void)curTime;
	...
	if (internalPacket->reliability == UNRELIABLE_SEQUENCED ||
		...
		bitStream->WriteAlignedVar8(
			(const char *)&tempChar); // Used for UNRELIABLE_SEQUENCED, RELIABLE_SEQUENCED,
									  // RELIABLE_ORDERED. 5 bits needed, write one byte
	}
	...
	bitStream->WriteAlignedBytes((unsigned char *)internalPacket->data,
								 BITS_TO_BYTES(internalPacket->dataBitLength));
...
InternalPacket *ReliabilityLayer::CreateInternalPacketFromBitStream(RakNet::BitStream *bitStream,
	...
	if (internalPacket->reliability == UNRELIABLE_SEQUENCED ||
		...
		internalPacket->reliability == RELIABLE_ORDERED_WITH_ACK_RECEIPT) {
		bitStream->Read(
			internalPacket->orderingIndex); // Used for UNRELIABLE_SEQUENCED, RELIABLE_SEQUENCED,
											// RELIABLE_ORDERED. 4 bytes.
		readSuccess = bitStream->ReadAlignedVar8(
			(char *)&internalPacket
				->orderingChannel); // Used for UNRELIABLE_SEQUENCED, RELIABLE_SEQUENCED,
	...
	if (hasSplitPacket) {
		...
		bitStream->ReadAlignedVar16(
			(char *)&internalPacket->splitPacketId); // Only needed if splitPacketCount>0.
	...
	// Set the last byte to 0 so if ReadBits does not read a multiple of 8 the last bits are 0'ed
	// out
	internalPacket->data[BITS_TO_BYTES(internalPacket->dataBitLength) - 1] = 0;

	// Read the data the packet holds
	bitStreamSucceeded = bitStream->ReadAlignedBytes((unsigned char *)internalPacket->data,
													 BITS_TO_BYTES(internalPacket->dataBitLength));
...
// Get the SHA1 code
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::GetSHA1(unsigned char *const buffer, unsigned int nbytes,
...
// Check the SHA1 code
//-------------------------------------------------------------------------------------------------------
bool ReliabilityLayer::CheckSHA1(char code[SHA1_LENGTH], unsigned char *const buffer,
								 unsigned int nbytes) {
...
//			theList[ i ]->reliability == UNRELIABLE_SEQUENCED_WITH_ACK_RECEIPT
			...
			theList[ i ]->orderingChannel == orderingChannel && ( splitPacketId == -1 || theList[ i
]->splitPacketId != (unsigned int) splitPacketId ) )
		{
...
void ReliabilityLayer::SplitPacket(InternalPacket *internalPacket) {
	...
	if (sizeof(InternalPacket *) * internalPacket->splitPacketCount < MAX_ALLOCA_STACK_ALLOCATION) {
		internalPacketArray =
			(InternalPacket **)alloca(sizeof(InternalPacket *) * internalPacket->splitPacketCount);
		usedAlloca = true;
	} else
#endif
		internalPacketArray = (InternalPacket **)rakMalloc_Ex(
			sizeof(InternalPacket *) * internalPacket->splitPacketCount, _FILE_AND_LINE_);
	...
	do {
		...
		AllocInternalPacketData(internalPacketArray[splitPacketIndex], &refCounter,
								internalPacket->data, internalPacket->data + byteOffset);
		//		internalPacketArray[ splitPacketIndex ]->data = (unsigned char*) rakMalloc_Ex(
		//bytesToSend, _FILE_AND_LINE_ ); 		memcpy( internalPacketArray[ splitPacketIndex ]->data,
		//internalPacket->data + byteOffset, bytesToSend );
		...
		RakAssert(internalPacketArray[splitPacketIndex]->dataBitLength <
				  BYTES_TO_BITS(MAXIMUM_MTU_SIZE));
	...
	for (i = 0; i < (int)internalPacket->splitPacketCount; i++) {
		...
		outgoingPacketBuffer.PushSeries(GetNextWeight(internalPacketArray[i]->priority),
...
void ReliabilityLayer::InsertIntoSplitPacketList(InternalPacket *internalPacket, CCTimeType time) {
	...
	if (objectExists == false) {
		...
		newChannel->splitPacketList.Preallocate(internalPacket->splitPacketCount, __FILE__,
												__LINE__);
	...
	if (internalPacket->splitPacketIndex == 0) {
		...
		for (unsigned int j = 0; j < splitPacketChannelList[index]->splitPacketList.Size(); j++) {
			...
			FreeInternalPacketData(splitPacketChannelList[index]->splitPacketList[j], __FILE__,
								   __LINE__);
	...
	if (splitPacketChannelList[index]->gotFirstPacket == true && splitMessageProgressInterval &&
		...
		//		unsigned int len = sizeof(MessageID) + sizeof(unsigned int)*2 + sizeof(unsigned int) +
		...
		memcpy(progressIndicator->data + sizeof(MessageID) + sizeof(unsigned int) * 1, &temp,
			   sizeof(unsigned int));
		//		temp=(unsigned int)
		//BITS_TO_BYTES(splitPacketChannelList[index]->firstPacket->dataBitLength);
		temp = (unsigned int)BITS_TO_BYTES(l);
		memcpy(progressIndicator->data + sizeof(MessageID) + sizeof(unsigned int) * 2, &temp,
			   sizeof(unsigned int));
	...
	if (splitMessageProgressInterval && splitPacketChannelList[index]->firstPacket &&
		...
		(splitPacketChannelList[index]->splitPacketList.Size() % splitMessageProgressInterval) ==
			0) {
		// Return ID_DOWNLOAD_PROGRESS
		// Write splitPacketIndex (SplitPacketIndexType)
		// Write splitPacketCount (SplitPacketIndexType)
...
InternalPacket *ReliabilityLayer::BuildPacketFromSplitPacketList(
	...
	for (j = 0; j < splitPacketChannel->splitPacketList.Size(); j++) {
		...
		memcpy(internalPacket->data + BITS_TO_BYTES(offset), splitPacket->data,
			   (size_t)BITS_TO_BYTES(splitPacketChannel->splitPacketList[j]->dataBitLength));
...
//-------------------------------------------------------------------------------------------------------
InternalPacket *ReliabilityLayer::BuildPacketFromSplitPacketList(SplitPacketIdType splitPacketId,
																 CCTimeType time, RakNetSocket2 *s,
...
InternalPacket *ReliabilityLayer::CreateInternalPacketCopy(InternalPacket *original,
														   int dataByteOffset, int dataByteLength,
														   CCTimeType time) {
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::InsertPacketIntoResendList(InternalPacket *internalPacket, CCTimeType time,
												  bool firstResend,
...
RakNetStatistics *ReliabilityLayer::GetStatistics(RakNetStatistics *rns) {
	...
	uint64Denominator =
		(rns->runningTotal[USER_MESSAGE_BYTES_SENT] + rns->runningTotal[USER_MESSAGE_BYTES_RESENT]);
	if (uint64Denominator != 0 &&
		rns->runningTotal[USER_MESSAGE_BYTES_SENT] / uint64Denominator > 0) {
		doubleDenominator = ((double)rns->runningTotal[USER_MESSAGE_BYTES_SENT] +
							 (double)rns->runningTotal[USER_MESSAGE_BYTES_RESENT]);
		if (doubleDenominator != 0) {
			rns->packetlossTotal =
				(float)((double)rns->runningTotal[USER_MESSAGE_BYTES_RESENT] / doubleDenominator);
		}
	}
...
unsigned int ReliabilityLayer::GetResendListDataSize(void) const {
	// Not accurate but thread-safe.  The commented version might crash if the queue is cleared
	// while we loop through it return resendTree.Size();
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::PushPacket(CCTimeType time, InternalPacket *internalPacket,
								  bool isReliable) {
	BitSize_t bitsForThisPacket = BYTES_TO_BITS(BITS_TO_BYTES(internalPacket->dataBitLength) +
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::SendACKs(RakNetSocket2 *s, SystemAddress &systemAddress, CCTimeType time,
								RakNetRandom *rnr, BitStream &updateBitStream) {
...
ReliabilityLayer::MessageNumberNode *ReliabilityLayer::AddFirstToDatagramHistory(
	DatagramSequenceNumberType datagramNumber, DatagramSequenceNumberType messageNumber,
	CCTimeType timeSent) {
	(void)datagramNumber;
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::FreeInternalPacketData(InternalPacket *internalPacket, const char *file,
											  unsigned int line) {
