...
int CCRakNetUDT::GetRetransmissionBandwidth(CCTimeType curTime, CCTimeType timeSinceLastTick,
	...
	return GetTransmissionBandwidth(curTime, timeSinceLastTick, unacknowledgedBytes,
									isContinuousSend);
}
// ----------------------------------------------------------------------------------------------------------------------------
int CCRakNetUDT::GetTransmissionBandwidth(CCTimeType curTime, CCTimeType timeSinceLastTick,
	...
	if (isInSlowStart) {
		uint32_t CWNDLimit =
			(uint32_t)(CWND * MAXIMUM_MTU_INCLUDING_UDP_HEADER - unacknowledgedBytes);
		return CWNDLimit;
	}
	...
	bytesCanSendThisTick =
		(int)((double)timeSinceLastTick * ((double)1.0 / SND) + (double)bytesCanSendThisTick);
...
BytesPerMicrosecond CCRakNetUDT::ReceiverCalculateDataArrivalRateMedian(void) const {
	return CalculateListMedianRecursive(packetArrivalHistory, CC_RAKNET_UDT_PACKET_HISTORY_LENGTH,
										0, 0);
}
// ----------------------------------------------------------------------------------------------------------------------------
BytesPerMicrosecond CCRakNetUDT::CalculateListMedianRecursive(
	const BytesPerMicrosecond inputList[CC_RAKNET_UDT_PACKET_HISTORY_LENGTH], int inputListLength,
...
bool CCRakNetUDT::GreaterThan(DatagramSequenceNumberType a, DatagramSequenceNumberType b) {
	// a > b?
	const DatagramSequenceNumberType halfSpan = (DatagramSequenceNumberType)(
...
bool CCRakNetUDT::OnGotPacket(DatagramSequenceNumberType datagramSequenceNumber,
	...
	if (curTime > lastPacketArrivalTime) {
		...
		if (isContinuousSend) {
			...
			packetArrivalHistoryContinuousGaps[packetArrivalHistoryContinuousGapsIndex++] =
				(int)interval;
...
void CCRakNetUDT::OnAck(CCTimeType curTime, CCTimeType rtt, bool hasBAndAS, BytesPerMicrosecond _B,
						BytesPerMicrosecond _AS, double totalUserDataBytesAcked,
...
// ----------------------------------------------------------------------------------------------------------------------------
void CCRakNetUDT::UpdateWindowSizeAndAckOnAckPerSyn(CCTimeType curTime, CCTimeType rtt,
