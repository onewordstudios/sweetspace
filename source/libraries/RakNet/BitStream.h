...
class RAK_DLL_EXPORT BitStream {
   public:
	...
	/// \details There is no benefit to calling this, unless you know exactly how many bytes you
	/// need and it is greater than BITSTREAM_STACK_ALLOCATION_SIZE. In that case all it does is
	/// save you one or more realloc calls. \param[in] initialBytesToAllocate the number of bytes to
	/// pre-allocate.
	...
	/// you are passing. Set it to false to just save a pointer to the data. You shouldn't call
	/// Write functions with \a _copyData as false, as this will write to unallocated memory 99% of
	/// the time you will use this function to cast Packet::data to a bitstream for reading, in
	/// which case you should write something as follows: \code RakNet::BitStream bs(packet->data,
	/// packet->length, false); \endcode \param[in] _data An array of bytes. \param[in]
	/// lengthInBytes Size of the \a _data. \param[in] _copyData true or false to make a copy of \a
	/// _data or not.
	...
	/// writeToBitstream is false and the read was successful.  false if \a writeToBitstream is
	...
	/// \brief Bidirectional version of SerializeDelta when you don't know what the last value is,
	/// or there is no last value. \param[in] writeToBitstream true to write from your data to this
	...
	/// \brief Bidirectional serialize/deserialize any integral type to/from a bitstream.
	/// \details Undefine __BITSTREAM_NATIVE_END if you need endian swapping.
	/// If you are not using __BITSTREAM_NATIVE_END the opposite is true for types larger than 1
	/// byte For floating point, this is lossy, using 2 bytes for a float and 4 for a double.  The
	/// range must be between -1 and +1. For non-floating point, this is lossless, but only has
	/// benefit if you use less than half the bits of the type \param[in] writeToBitstream true to
	/// write from your data to this bitstream.  False to read from this bitstream and write to your
	/// data \param[in] inOutTemplateVar The value to write \return true if \a writeToBitstream is
	/// true.  true if \a writeToBitstream is false and the read was successful.  false if \a
	/// writeToBitstream is false and the read was not successful.
	template <class templateType>
	...
	/// \brief Serialize a float into 2 bytes, spanning the range between \a floatMin and \a
	/// floatMax \param[in] writeToBitstream true to write from your data to this bitstream.  False
	/// to read from this bitstream and write to your data \param[in] inOutFloat The float to write
	...
	/// bits to represent the range Then serialize only those bits \note A static is used so that
	/// the required number of bits for (maximum-minimum) is only calculated once. This does require
	/// that \a minimum and \maximum are fixed values for a given line of code for the life of the
	/// program \param[in] writeToBitstream true to write from your data to this bitstream.  False
	/// to read from this bitstream and write to your data \param[in] value Integer value to write,
	/// which should be between \a minimum and \a maximum \param[in] minimum Minimum value of \a
	/// value \param[in] maximum Maximum value of \a value \param[in] allowOutsideRange If true, all
	/// sends will take an extra bit, however value can deviate from outside \a minimum and \a
	/// maximum. If false, will assert if the value deviates
	template <class templateType>
	...
	bool SerializeNormVector(bool writeToBitstream, templateType &x, templateType &y,
							 templateType &z);
	...
	/// instead of 16 bytes. Slightly lossy. \param[in] writeToBitstream true to write from your
	/// data to this bitstream.  False to read from this bitstream and write to your data \param[in]
	/// w w \param[in] x x \param[in] y y \param[in] z z \return true if \a writeToBitstream is
	/// true.  true if \a writeToBitstream is false and the read was successful.  false if \a
	/// writeToBitstream is false and the read was not successful.
	template <class templateType> // templateType for this function must be a float or double
	bool SerializeNormQuat(bool writeToBitstream, templateType &w, templateType &x, templateType &y,
						   templateType &z);

	/// \brief Bidirectional serialize/deserialize an orthogonal matrix by creating a quaternion,
	/// and writing 3 components of the quaternion in 2 bytes each. \details Use 6 bytes instead of
	/// 36 Lossy, although the result is renormalized \return true on success, false on failure.
	template <class templateType> // templateType for this function must be a float or double
	bool SerializeOrthMatrix(bool writeToBitstream, templateType &m00, templateType &m01,
							 templateType &m02, templateType &m10, templateType &m11,
							 templateType &m12, templateType &m20, templateType &m21,
							 templateType &m22);
	...
	bool SerializeBits(bool writeToBitstream, unsigned char *inOutByteArray,
					   const BitSize_t numberOfBitsToSerialize, const bool rightAlignedBits = true);
	...
	/// byte For floating point, this is lossy, using 2 bytes for a float and 4 for a double.  The
	/// range must be between -1 and +1. For non-floating point, this is lossless, but only has
	/// benefit if you use less than half the bits of the type \param[in] inTemplateVar The value to
	/// write
	template <class templateType>
	...
	/// \brief Read any integral type from a bitstream.
	/// \details If the written value differed from the value compared against in the write
	/// function, var will be updated.  Otherwise it will retain the current value. ReadDelta is
	/// only valid from a previous call to WriteDelta \param[in] outTemplateVar The value to read
	/// \return true on success, false on failure.
	template <class templateType>
	...
	/// \brief Read any integral type from a bitstream.
	/// \details If the written value differed from the value compared against in the write
	/// function, var will be updated.  Otherwise it will retain the current value. the current
	/// value will be updated. For floating point, this is lossy, using 2 bytes for a float and 4
	/// for a double.  The range must be between -1 and +1. For non-floating point, this is
	/// lossless, but only has benefit if you use less than half the bits of the type If you are not
	/// using __BITSTREAM_NATIVE_END the opposite is true for types larger than 1 byte
	...
	/// required number of bits for (maximum-minimum) is only calculated once. This does require
	/// that \a minimum and \maximum are fixed values for a given line of code for the life of the
	/// program \param[in] value Integer value to write, which should be between \a minimum and \a
	/// maximum \param[in] minimum Minimum value of \a value \param[in] maximum Maximum value of \a
	/// value \param[in] allowOutsideRange If true, all sends will take an extra bit, however value
	/// can deviate from outside \a minimum and \a maximum. If false, will assert if the value
	/// deviates. This should match the corresponding value passed to Read().
	template <class templateType>
	void WriteBitsFromIntegerRange(const templateType value, const templateType minimum,
	...
	/// \brief Write a normalized 3D vector, using (at most) 4 bytes + 3 bits instead of 12-24
	/// bytes. \details Will further compress y or z axis aligned vectors. Accurate to 1/32767.5.
	/// \param[in] x x
	/// \param[in] y y
	/// \param[in] z z
	template <class templateType> // templateType for this function must be a float or double
	...
	/// renormalized
	template <class templateType> // templateType for this function must be a float or double
	void WriteOrthMatrix(templateType m00, templateType m01, templateType m02, templateType m10,
	...
	/// required number of bits for (maximum-minimum) is only calculated once. This does require
	/// that \a minimum and \maximum are fixed values for a given line of code for the life of the
	/// program \param[in] value Integer value to read, which should be between \a minimum and \a
	/// maximum \param[in] minimum Minimum value of \a value \param[in] maximum Maximum value of \a
	/// value \param[in] allowOutsideRange If true, all sends will take an extra bit, however value
	/// can deviate from outside \a minimum and \a maximum. If false, will assert if the value
	...
	/// \brief Read an orthogonal matrix from a quaternion, reading 3 components of the quaternion
	/// in 2 bytes each and extrapolatig the 4th. \details Use 6 bytes instead of 36 Lossy, although
	/// the result is renormalized \return true on success, false on failure.
	template <class templateType> // templateType for this function must be a float or double
	bool ReadOrthMatrix(templateType &m00, templateType &m01, templateType &m02, templateType &m10,
						templateType &m11, templateType &m12, templateType &m20, templateType &m21,
						templateType &m22);
	...
	void WriteBits(const unsigned char *inByteArray, BitSize_t numberOfBitsToWrite,
	...
	/// \brief Same as ReadAlignedBytesSafe() but allocates the memory for you using new, rather
	/// than assuming it is safe to write to \param[in] outByteArray outByteArray will be deleted if
	/// it is not a pointer to 0 \return true on success, false on failure.
	...
	static void ReverseBytes(unsigned char *inByteArray, unsigned char *inOutByteArray,
   ...
   private:
	...
	void WriteCompressed(const unsigned char *inByteArray, const unsigned int size,
...
template <class templateType>
inline bool BitStream::SerializeNormVector(bool writeToBitstream, templateType &x, templateType &y,
										   templateType &z) {
	if (writeToBitstream)
...
template <class templateType>
inline bool BitStream::SerializeVector(bool writeToBitstream, templateType &x, templateType &y,
									   templateType &z) {
	if (writeToBitstream)
		WriteVector(x, y, z);
	else
...
template <class templateType>
inline bool BitStream::SerializeNormQuat(bool writeToBitstream, templateType &w, templateType &x,
...
template <class templateType>
inline bool BitStream::SerializeOrthMatrix(bool writeToBitstream, templateType &m00,
...
template <class templateType> // templateType for this function must be a float or double
void BitStream::WriteOrthMatrix(templateType m00, templateType m01, templateType m02,
...
template <class templateType>
bool BitStream::ReadBitsFromIntegerRange(templateType &value, const templateType minimum,
										 const templateType maximum, bool allowOutsideRange) {
	int requiredBits = BYTES_TO_BITS(sizeof(templateType)) -
					   NumberOfLeadingZeroes(templateType(maximum - minimum));
	return ReadBitsFromIntegerRange(value, minimum, maximum, requiredBits, allowOutsideRange);
...
template <class templateType> // templateType for this function must be a float or double
bool BitStream::ReadOrthMatrix(templateType &m00, templateType &m01, templateType &m02,
							   templateType &m10, templateType &m11, templateType &m12,
							   templateType &m20, templateType &m21, templateType &m22) {
	float qw, qx, qy, qz;
