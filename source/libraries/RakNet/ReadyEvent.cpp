...
int RakNet::ReadyEvent::ReadyEventNodeComp(const int &key,
	...
	if (eventExists) {
		if (guid == UNASSIGNED_RAKNET_GUID) {
			...
			unsigned systemIndex =
				readyEventNodeList[eventIndex]->systemList.GetIndexFromKey(guid, &systemExists);
			if (systemExists) {
...
void ReadyEvent::OnReadyEventQuery(Packet *packet) {
	...
	if (objectExists) {
		unsigned systemIndex =
			readyEventNodeList[readyIndex]->systemList.GetIndexFromKey(packet->guid, &objectExists);
...
bool ReadyEvent::SetEventByIndex(int eventIndex, bool isReady) {
	ReadyEventNode *ren = readyEventNodeList[eventIndex];
	if ((ren->eventStatus == ID_READY_EVENT_ALL_SET || ren->eventStatus == ID_READY_EVENT_SET) &&
		isReady == true)
...
void ReadyEvent::SendReadyUpdate(unsigned eventIndex, unsigned systemIndex,
	...
	if ((ren->eventStatus != ren->systemList[systemIndex].lastSentStatus) ||
		...
		SendUnified(&bs, HIGH_PRIORITY, RELIABLE_ORDERED, channel,
					ren->systemList[systemIndex].rakNetGuid, false);
