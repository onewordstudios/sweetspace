...
class RAK_DLL_EXPORT Table {
   public:
	...
	/// \note If you are going to update many cells of a particular row, it is more efficient to
	/// call GetRow and perform the operations on the row directly. \note Row pointers do not
	/// change, so you can also write directly to the rows for more efficiency. \param[in] rowId The
	/// ID of the row \param[in] columnIndex The column of the cell \param[in] value The data to set
	...
	/// GetCell. You can also update cells in rows from this function. \param[in] rowId The ID of
	/// the row \return The desired row, or 0 if no such row.
	...
	/// \param[in] columnSubset An array of column indices.  Only columns in this array are
	/// returned.  Pass 0 for all columns \param[in] numColumnSubset The number of elements in \a
	/// columnSubset \param[in] inclusionFilters An array of FilterQuery.  All filters must pass for
	/// the row to be returned. \param[in] numInclusionFilters The number of elements in \a
	/// inclusionFilters \param[in] rowIds An arrow of row IDs.  Only these rows with these IDs are
	/// returned.  Pass 0 for all rows. \param[in] numRowIDs The number of elements in \a rowIds
	/// \param[out] result The result of the query.  If no rows are returned, the table will only
	/// have columns.
	void QueryTable(unsigned *columnIndicesSubset, unsigned numColumnSubset,
					FilterQuery *inclusionFilters, unsigned numInclusionFilters, unsigned *rowIds,
