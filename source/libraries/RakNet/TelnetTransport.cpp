...
Packet *TelnetTransport::Receive(void) {
	...
	*/
	...
	// Hack - I don't know what the hell this is about but cursor keys send 3 characters at a time.
	// I can block these
	// Up=27,91,65
	// Down=27,91,66
	// Right=27,91,67
	...
	for (i = 0; i < p->length; i++) {
		...
		if (gotLine && remoteClient->textInput[0]) {
			...
			reassembledLine->data =
				(unsigned char *)rakMalloc_Ex(reassembledLine->length + 1, _FILE_AND_LINE_);
...
SystemAddress TelnetTransport::HasNewIncomingConnection(void) {
	...
	// 03/16/06 Can't force the stupid windows telnet to use line mode or local echo so now I have
	// to track all the remote players and their input buffer
