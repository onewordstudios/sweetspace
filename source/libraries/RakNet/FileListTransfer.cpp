...
void FileListTransfer::Send(FileList *fileList, RakNet::RakPeerInterface *rakPeer,
	...
	// Write the chunk header, which contains the frequency table, the total number of files, and
	...
	if (anythingToWrite) {
		...
		if (rakPeer)
			rakPeer->Send(&outBitstream, priority, RELIABLE_ORDERED, orderingChannel, recipient,
						  false);
		else
			SendUnified(&outBitstream, priority, RELIABLE_ORDERED, orderingChannel, recipient,
						false);
		...
		for (i = 0; i < fileList->fileList.Size(); i++) {
			...
			if (sendReference) {
				...
				outBitstream.Write(setID);
				StringCompressor::Instance()->EncodeString(fileList->fileList[i].filename, 512,
														   &outBitstream);
				...
				SendListUnified(dataBlocks, lengths, 2, priority, RELIABLE_ORDERED, orderingChannel,
								recipient, false);
			}
		}
...
bool FileListTransfer::DecodeFile(Packet *packet, bool isTheFullFile) {
	...
	if (isTheFullFile) {
		...
		onFileStruct.fileData =
			(char *)rakMalloc_Ex((size_t)onFileStruct.byteLengthOfThisFile, _FILE_AND_LINE_);
...
void FileListTransfer::OnReferencePush(Packet *packet, bool isTheFullFile) {
	...
	if (isTheFullFile) {
		if (mb.flrMemoryBlock) {
			...
			fps.iriDataChunk =
				(char *)inBitStream.GetData() + BITS_TO_BYTES(inBitStream.GetReadOffset());
		}
	...
	if (finished) {
		...
		if (isTheFullFile) {
			...
			if (fps.allocateIrIDataChunkAutomatically == false) {
				rakFree_Ex(fileListReceiver->pushedFiles.Get(onFileStruct.fileIndex).flrMemoryBlock,
						   _FILE_AND_LINE_);
			...
			if (isTheFullFile == false)
				currentNotificationIndex = (offset + partCount * fileListReceiver->partLength) /
...
int SendIRIToAddressCB(FileListTransfer::ThreadData threadData, bool *returnOutput,
					   void *perThreadData) {
	(void)perThreadData;
	...
	for (ftpIndex = 0; ftpIndex < fileListTransfer->fileToPushRecipientList.Size(); ftpIndex++) {
		...
		if (ftpr->systemAddress == systemAddress && ftpr->setId == setId) {
			...
			bytesRead = ftp->incrementalReadInterface->GetFilePart(
				ftp->fileListNode.fullPathToFile, ftp->currentOffset, ftp->chunkSize, buff,
			...
			while (done && ftp->currentOffset == 0 && smallFileTotalSize < ftp->chunkSize) {
				...
				outBitstream.Write(setId);
				StringCompressor::Instance()->EncodeString(ftp->fileListNode.filename, 512,
														   &outBitstream);
				...
				fileListTransfer->SendListUnified(dataBlocks, lengths, 2, ftp->packetPriority,
												  RELIABLE_ORDERED, ftp->orderingChannel,
												  systemAddress, false);
			...
			for (unsigned int flpcIndex = 0;
				 flpcIndex < fileListTransfer->fileListProgressCallbacks.Size(); flpcIndex++)
			...
			fileListTransfer->SendListUnified(dataBlocks, lengths, 2, packetPriority,
											  RELIABLE_ORDERED, orderingChannel, systemAddress,
											  false);
