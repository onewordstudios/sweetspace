...
enum PublicKeyMode {
	...
	/// I recommend not using this mode except for server-to-server communication as it
	/// significantly increases the CPU requirements during connections for both sides. Furthermore,
...
struct RAK_DLL_EXPORT SocketDescriptor {
	...
	/// On the positive side, NAT Punchthrough is not needed and should not be used with IPV6
	/// because there are enough addresses that routers do not need to create address mappings.
...
struct RAK_DLL_EXPORT SystemAddress {
	...
	/// SystemAddress, with RAKNET_SUPPORT_IPV6 defined, holds both an sockaddr_in6 and a
	/// sockaddr_in
	union // In6OrIn4
	{
	...
	/// example "192.0.2.1|1234" \param[in] str A printable IP string, for example "192.0.2.1" or
	...
	/// Only used if str is a pre-defined address in the wrong format, such as 127.0.0.1 but you
	/// want ip version 6, so you can pass 6 here to do the conversion \note The current port is
	/// unchanged if a port is not specified in \a str \return True on success, false on ipVersion
	/// does not match type of passed string
