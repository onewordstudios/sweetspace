...
class RAK_DLL_EXPORT RakString {
   public:
	...
	/// Searches the string for the content specified in stringToFind and returns the position of
	/// the first occurrence in the string. Search only includes characters on or after position
	/// pos, ignoring any possible occurrences in previous locations. \param[in] stringToFind The
	/// string to find inside of this object's string \param[in] pos The position in the string to
	/// start the search \return Returns the position of the first occurrence in the string.
	...
	static RakNet::RakString FormatForPOST(const char *uri, const char *contentType,
										   const char *body, const char *extraHeaders = "");
	static RakNet::RakString FormatForPUT(const char *uri, const char *contentType,
										  const char *body, const char *extraHeaders = "");
	...
	/// bandwidth) \pre StringCompressor::AddReference must have been called to instantiate the
	/// class (Happens automatically from RakPeer::Startup())
	...
	/// \param[in] bs Bitstream to serialize from
	/// \param[in] readLanguageId If true, looks for the variable langaugeId in the data stream.
	/// Must match what was passed to SerializeCompressed \return true if the deserialization was
	/// successful \pre StringCompressor::AddReference must have been called to instantiate the
	/// class (Happens automatically from RakPeer::Startup())
   ...
   protected:
	static RakNet::RakString FormatForPUTOrPost(const char *type, const char *uri,
