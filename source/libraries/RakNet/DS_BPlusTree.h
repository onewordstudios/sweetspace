...
struct RAK_DLL_EXPORT Page {
	...
	// For an index it means the number of keys and is one less than the number of children
	// pointers.
	int size;
	...
	// Used only for leaf nodes.  Data is the actual data, while next is the pointer to the next
	// leaf (for B+)
	DataType data[order];
...
class RAK_DLL_EXPORT BPlusTree {
   ...
   protected:
	...
	void RotateLeft(Page<KeyType, DataType, order> *cur, int childIndex,
					ReturnAction *returnAction);
	Page<KeyType, DataType, order> *InsertIntoNode(const KeyType key, const DataType &childData,
												   int insertionIndex,
	...
	bool FindDeleteRebalance(const KeyType key, Page<KeyType, DataType, order> *cur,
							 bool *underflow, KeyType rightRootKey, ReturnAction *returnAction,
							 DataType &out);
...
bool BPlusTree<KeyType, DataType, order>::Delete(const KeyType key, DataType &out) {
	...
	if (root == leftmostLeaf) {
		...
		return true;
	} else if (FindDeleteRebalance(key, root, &underflow, root->keys[0], &returnAction, out) ==
			   false)
		return false;
...
bool BPlusTree<KeyType, DataType, order>::FindDeleteRebalance(const KeyType key,
	...
	if (cur->children[branchIndex]->isLeaf == false) {
		...
		if (FindDeleteRebalance(key, cur->children[branchIndex], underflow, rightRootKey,
...
Page<KeyType, DataType, order> *BPlusTree<KeyType, DataType, order>::InsertIntoNode(
	...
	Page<KeyType, DataType, order> *nodeData, Page<KeyType, DataType, order> *cur,
	ReturnAction *returnAction) {
	int i;
	if (cur->size < order) {
		...
		if (insertionIndex >= (order + 1) / 2) {
			...
			if (cur->isLeaf) {
				...
				// sourceIndex+1 is sort of a hack but it works - because there is one extra child
				// than keys skip past the last child for cur
				for (; sourceIndex + 1 < cur->size + 1; sourceIndex++, destIndex++) {
					newPage->children[destIndex] = cur->children[sourceIndex + 1];
				}

				// the first key is the middle key.  Remove it from the page and push it to the
				// parent
...
template <class KeyType, class DataType, int order>
bool BPlusTree<KeyType, DataType, order>::CanRotateLeft(Page<KeyType, DataType, order> *cur,
														int childIndex) {
	return childIndex > 0 && cur->children[childIndex - 1]->size < order;
}

template <class KeyType, class DataType, int order>
void BPlusTree<KeyType, DataType, order>::RotateLeft(Page<KeyType, DataType, order> *cur,
													 int childIndex, ReturnAction *returnAction) {
...
template <class KeyType, class DataType, int order>
bool BPlusTree<KeyType, DataType, order>::CanRotateRight(Page<KeyType, DataType, order> *cur,
														 int childIndex) {
	return childIndex < cur->size && cur->children[childIndex + 1]->size < order;
}

template <class KeyType, class DataType, int order>
void BPlusTree<KeyType, DataType, order>::RotateRight(Page<KeyType, DataType, order> *cur,
													  int childIndex, ReturnAction *returnAction) {
	Page<KeyType, DataType, order> *dest = cur->children[childIndex + 1];
	Page<KeyType, DataType, order> *source = cur->children[childIndex];
	returnAction->key1 = dest->keys[0];
...
template <class KeyType, class DataType, int order>
Page<KeyType, DataType, order> *BPlusTree<KeyType, DataType, order>::GetLeafFromKey(
...
Page<KeyType, DataType, order> *BPlusTree<KeyType, DataType, order>::InsertBranchDown(
	const KeyType key, const DataType &data, Page<KeyType, DataType, order> *cur,
	ReturnAction *returnAction, bool *success) {
	int childIndex;
	int branchIndex;
	if (GetIndexOf(key, cur, &childIndex))
	...
	if (cur->isLeaf == false) {
		...
		if (newPage) {
			if (newPage->isLeaf == false) {
				...
				newPage->size--;
				return InsertIntoNode(returnAction->key1, data, branchIndex, newPage, cur,
									  returnAction);
			} else {
...
template <class KeyType, class DataType, int order>
bool BPlusTree<KeyType, DataType, order>::GetIndexOf(const KeyType key,
...
template <class KeyType, class DataType, int order>
void BPlusTree<KeyType, DataType, order>::PrintLeaf(Page<KeyType, DataType, order> *leaf,
													int index) {
	int i;
...
void main(void)
{
	...
	for (testSize=0; testSize < 514; testSize++)
	{
		...
		for (i=0; i<testSize; i++)
		{
			btree.Delete(haveList[0]); // Asserts on 8th call.  Fails on going to remove 8 (7th
call) removedList.Insert(haveList[0]); haveList.RemoveAtIndex(0); for (j=0; j < removedList.Size();
j++)
			{
