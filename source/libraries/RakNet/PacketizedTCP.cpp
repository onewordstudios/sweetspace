...
void PacketizedTCP::Send(const char *data, unsigned length, const SystemAddress &systemAddress,
						 bool broadcast) {
	PTCPHeader dataLength;
	dataLength = length;
#ifndef __BITSTREAM_NATIVE_END
	if (RakNet::BitStream::DoEndianSwap())
		RakNet::BitStream::ReverseBytes((unsigned char *)&length, (unsigned char *)&dataLength,
										sizeof(dataLength));
#else
	dataLength = length;
#endif
...
bool PacketizedTCP::SendList(const char **data, const unsigned int *lengths,
...
#ifndef __BITSTREAM_NATIVE_END
	if (RakNet::BitStream::DoEndianSwap())
		RakNet::BitStream::ReverseBytes((unsigned char *)&totalLengthOfUserData,
	...
	return TCPInterface::SendList(dataArray, lengthsArray, numParameters + 1, systemAddress,
								  broadcast);
}
	...
	while (incomingPacket) {
		...
		if (incomingPacket->deleteData == true) {
			...
			if (index < connections.Size()) {
				...
				if (RakNet::BitStream::DoEndianSwap())
					RakNet::BitStream::ReverseBytesInPlace((unsigned char *)&dataLength,
														   sizeof(dataLength));
				// Header indicates packet length. If enough data is available, read out and return
				// one packet
				if (bq->GetBytesWritten() >= dataLength + sizeof(PTCPHeader)) {
					...
					if (newWritten / 65536 != oldWritten / 65536) {
						outgoingPacket = RakNet::OP_NEW<Packet>(_FILE_AND_LINE_);
						outgoingPacket->length = sizeof(MessageID) + sizeof(unsigned int) * 2 +
												 sizeof(unsigned int) + 65536;
						...
						outgoingPacket->data =
							(unsigned char *)rakMalloc_Ex(outgoingPacket->length, _FILE_AND_LINE_);
						if (outgoingPacket->data == 0) {
							notifyOutOfMemory(_FILE_AND_LINE_);
							RakNet::OP_DELETE(outgoingPacket, _FILE_AND_LINE_);
							return 0;
						...
						memcpy(outgoingPacket->data + sizeof(MessageID), &partIndex,
							   sizeof(unsigned int));
						memcpy(outgoingPacket->data + sizeof(MessageID) + sizeof(unsigned int) * 1,
							   &totalParts, sizeof(unsigned int));
						memcpy(outgoingPacket->data + sizeof(MessageID) + sizeof(unsigned int) * 2,
							   &oneChunkSize, sizeof(unsigned int));
						bq->IncrementReadOffset(sizeof(PTCPHeader));
						bq->ReadBytes((char *)outgoingPacket->data + sizeof(MessageID) +
