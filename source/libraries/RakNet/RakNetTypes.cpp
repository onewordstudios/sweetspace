...
void SystemAddress::ToString_New(bool writePort, char *dest, char portDelineator) const {
	...
	if (address.addr4.sin_family == AF_INET) {
		ret = getnameinfo((struct sockaddr *)&address.addr4, sizeof(struct sockaddr_in), dest, 22,
						  NULL, 0, NI_NUMERICHOST);
	} else {
		ret = getnameinfo((struct sockaddr *)&address.addr6, sizeof(struct sockaddr_in6), dest,
	...
	if (ipVersion == 4 && strcmp(str, IPV6_LOOPBACK) == 0) {
		...
		for (; i < sizeof(ipPart) && str[i] != 0 && str[i] != portDelineator; i++) {
			if ((str[i] < '0' || str[i] > '9') && (str[i] < 'a' || str[i] > 'f') &&
				(str[i] < 'A' || str[i] > 'F') && str[i] != '.' && str[i] != ':' && str[i] != '%' &&
				str[i] != '-' && str[i] != '/')
				break;
