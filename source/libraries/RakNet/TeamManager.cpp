...
bool TM_TeamMember::RequestTeam(TeamSelection teamSelection) {
	if (teamSelection.joinTeamType == JOIN_NO_TEAM) {
		...
		// On local, call UpdateTeamsRequestedToAny(). Send event to also execute this to remote
		// host If we are host, execute JoinAnyTeam(myguid).
		// - JoinAnyTeam(requesterGuid): Attempt to join any team immediately.  If fails, send to
		// all except requestGuid UpdateTeamsRequestedToAny(). Else sends out new team, including to
		...
		// JoinAnyTeamCheck() passed on remote host, call UpdateTeamsRequestedToAny() for this
		// player. execute JoinAnyTeam(packet->guid).
		...
		bsOut.Write(world->GetWorldId());
		bsOut.Write(networkId);
		world->GetTeamManager()->SendUnified(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0,
											 world->GetHost(), false);
	} else {
		...
		// team, including to caller. On remote host, execute JoinSpecificTeamCheck(). If fails,
		// just ignore. Assuming JoinSpecificTeamCheck() passed on host, call
		// AddSpecificToRequestedList(). Execute JoinSpecificTeam(packet->guid)
...
bool TM_TeamMember::DeserializeConstruction(TeamManager *teamManager,
	...
	for (unsigned int i = 0; i < teamsRequestedSize; i++) {
		...
		rt.requestIndex = world->teamRequestIndex++; // In case whenRequested is the same between
...
bool TM_TeamMember::SwitchSpecificTeamCheck(TM_Team *teamToJoin, TM_Team *teamToLeave,
...
TM_Team::TM_Team() {
	...
	world = 0;
	joinPermissions =
		ALLOW_JOIN_ANY_AVAILABLE_TEAM | ALLOW_JOIN_SPECIFIC_TEAM | ALLOW_JOIN_REBALANCING;
...
void TM_World::OnNewConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
							   bool isIncoming) {
	(void)isIncoming;
	(void)systemAddress;
...
void TM_World::KickExcessMembers(NoTeamId noTeamId) {
	...
	// overfull if the team has ALLOW_JOIN_REBALANCING set If cannot move the player to another
	// team, just take the player off the team and set to noTeamId if they have no team at that
	// point
	...
	for (i = 0; i < teams.Size(); i++) {
		...
		while (team->GetMemberLimitSetting() < team->GetTeamMembersCount() ||
			...
			teamIndex =
				GetAvailableTeamIndexWithFewestMembers(balancedTeamLimit, ALLOW_JOIN_REBALANCING);
...
void TM_World::FillRequestedSlots(void) {
	...
	DataStructures::OrderedList<TM_World::JoinRequestHelper, TM_World::JoinRequestHelper,
	...
	for (joinRequestIndex = 0; joinRequestIndex < joinRequests.Size(); joinRequestIndex++) {
		...
		if (teamMember->teamsRequested.Size() == 0) {
			if (teamMember->joinTeamType == JOIN_ANY_AVAILABLE_TEAM)
				teamIndex = GetAvailableTeamIndexWithFewestMembers(balancedTeamLimit,
																   ALLOW_JOIN_ANY_AVAILABLE_TEAM);
			else
...
void TM_World::BroadcastToParticipants(unsigned char *data, const int length,
									   RakNetGUID exclusionGuid) {
	for (unsigned int i = 0; i < participants.Size(); i++) {
		if (participants[i] == exclusionGuid) continue;
		teamManager->SendUnified((const char *)data, length, HIGH_PRIORITY, RELIABLE_ORDERED, 0,
...
TM_Team *TM_World::JoinAnyTeam(TM_TeamMember *teamMember, int *resultCode) {
	...
	unsigned int idx =
		GetAvailableTeamIndexWithFewestMembers(balancedLimit, ALLOW_JOIN_ANY_AVAILABLE_TEAM);
...
int TM_World::JoinSpecificTeam(TM_TeamMember *teamMember, TM_Team *team, bool isTeamSwitch,
							   TM_Team *teamToLeave,
							   DataStructures::List<TM_Team *> &teamsWeAreLeaving) {
	if (team->GetJoinPermissions() & ALLOW_JOIN_SPECIFIC_TEAM) {
		if (balanceTeamsIsActive == false || teamsWeAreLeaving.Size() == 0) {
			if (team->GetMemberLimit() > team->GetTeamMembersCount()) {
				// Can join normally
				teamMember->StoreLastTeams();
				teamManager->RemoveFromTeamsRequestedAndAddTeam(teamMember, team, isTeamSwitch,
																teamToLeave);
				return 1;
			} else {
				// Full
			...
			if (teamsWeAreLeaving.Size() == 1) {
				...
				if (j != (unsigned int)-1) {
					...
					teamManager->RemoveFromTeamsRequestedAndAddTeam(swappingMember,
																	teamsWeAreLeaving[0], true, 0);
...
void TeamManager::EncodeTeamFull(RakNet::BitStream *bitStream, TM_TeamMember *teamMember,
								 TM_Team *team) {
...
void TeamManager::DecomposeTeamFull(Packet *packet, TM_World **world, TM_TeamMember **teamMember,
	...
	DecomposeTeamFullOrLocked(&bsIn, world, teamMember, team, currentMembers,
							  memberLimitIncludingBalancing, balancingIsActive, joinPermissions);
}

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void TeamManager::EncodeTeamLocked(RakNet::BitStream *bitStream, TM_TeamMember *teamMember,
								   TM_Team *team) {
...
void TeamManager::EncodeTeamFullOrLocked(RakNet::BitStream *bitStream, TM_TeamMember *teamMember,
										 TM_Team *team) {
...
void TeamManager::DecomposeTeamFullOrLocked(RakNet::BitStream *bsIn, TM_World **world,
											TM_TeamMember **teamMember, TM_Team **team,
...
void TeamManager::DecomposeTeamLocked(Packet *packet, TM_World **world, TM_TeamMember **teamMember,
									  TM_Team **team, uint16_t &currentMembers,
									  uint16_t &memberLimitIncludingBalancing,
									  bool &balancingIsActive, JoinPermissions &joinPermissions) {
	BitStream bsIn(packet->data, packet->length, false);
...
void TeamManager::ProcessTeamAssigned(RakNet::BitStream *bsIn) {
	...
	DecodeTeamAssigned(bsIn, &world, &teamMember, noTeamId, joinTeamType, newTeam, teamsLeft,
					   teamsJoined);
	if (teamMember) {
		teamMember->StoreLastTeams();
...
void TeamManager::DecodeTeamAssigned(BitStream *bsIn, TM_World **world, TM_TeamMember **teamMember,
									 NoTeamId &noTeamId, JoinTeamType &joinTeamType,
									 DataStructures::List<TM_Team *> &newTeam,
									 DataStructures::List<TM_Team *> &teamsLeft,
									 DataStructures::List<TM_Team *> &teamsJoined) {
...
void TeamManager::OnNewConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
								  bool isIncoming) {
	for (unsigned int i = 0; i < worldsList.Size(); i++) {
...
void TeamManager::Send(const RakNet::BitStream *bitStream, const AddressOrGUID systemIdentifier,
					   bool broadcast) {
...
void TeamManager::RemoveFromTeamsRequestedAndAddTeam(TM_TeamMember *teamMember, TM_Team *team,
													 bool isTeamSwitch, TM_Team *teamToLeave) {
...
void TeamManager::OnJoinAnyTeam(Packet *packet, TM_World *world) {
	...
	if (teamMember) {
		...
		if (resultCode == 1) {
			// Broadcast packet - remote systems should clear requested teams to none, and add the
			// team we joined. Broadcast includes non-host sender (all participants)
			BitStream bsOut;
			bsOut.WriteCasted<MessageID>(ID_TEAM_BALANCER_INTERNAL);
			bsOut.WriteCasted<MessageID>(ID_RUN_UpdateTeamsRequestedToNoneAndAddTeam);
			bsOut.Write(world->GetWorldId());
...
void TeamManager::OnJoinRequestedTeam(Packet *packet, TM_World *world) {
	...
	if (teamToJoin && teamMember) {
		if (isTeamSwitch) {
			if (teamMember->SwitchSpecificTeamCheck(teamToJoin, teamToLeave,
													packet->guid == GetMyGUIDUnified()) == false)
				return;

			teamMember->AddToRequestedTeams(teamToJoin, teamToLeave);
		} else {
			if (teamMember->JoinSpecificTeamCheck(teamToJoin, packet->guid == GetMyGUIDUnified()) ==
				false)
				return;
		...
		int resultCode = world->JoinSpecificTeam(teamMember, teamToJoin, isTeamSwitch, teamToLeave,
												 teamsWeAreLeaving);

		if (resultCode == 1) {
			// Broadcast packet - remote systems should remove from requested teams and add the team
	...
	if (team) {
		...
		if (world->GetHost() == world->GetTeamManager()->GetMyGUIDUnified()) {
			if (packet->guid == GetMyGUIDUnified())
				...
				world->BroadcastToParticipants(packet->data, packet->length,
