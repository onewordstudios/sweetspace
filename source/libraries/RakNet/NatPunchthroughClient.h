...
struct RAK_DLL_EXPORT PunchthroughConfiguration {
	...
	/// After trying all external ports, how long to wait before returning
...
class RAK_DLL_EXPORT NatPunchthroughClient : public PluginInterface2 {
   public:
	...
	/// you can call FindRouterPortStride() This will determine the stride that your router uses
	/// when assigning ports, if your router is full-cone This function is also called automatically
	/// when you call OpenNAT - however, calling it earlier when you are connected to the
	/// facilitator will speed up the process \param[in] destination The system to punch. Must
	/// already be connected to \a facilitator
	...
	/// \param[in] facilitator A system we are already connected to running the
	/// NatPunchthroughServer plugin \sa OpenNATGroup() You will get ID_NAT_PUNCHTHROUGH_SUCCEEDED
	/// on success You will get ID_NAT_TARGET_NOT_CONNECTED, ID_NAT_TARGET_UNRESPONSIVE,
	/// ID_NAT_CONNECTION_TO_TARGET_LOST, ID_NAT_ALREADY_IN_PROGRESS, or ID_NAT_PUNCHTHROUGH_FAILED
	/// on failures of various types However, if you lose connection to the facilitator, you may not
	/// necessarily get above
	...
	/// This is useful for peer to peer games where you want to connect to every system in the
	remote session, not just one particular system
	/// \note For cloud computing, all systems in the group must be connected to the same
	facilitator since we're only specifying one
	...
	*/
	...
	struct SendPing {
		...
		uint16_t sessionId;
		bool sentTTL;
		// Give priority to internal IP addresses because if we are on a LAN, we don't want to try
