...
RNS2_SendParameters_NativeClient *RNS2_NativeClient::CloneSP(RNS2_SendParameters *sp,
...
RNS2SendResult RNS2_NativeClient::Send(RNS2_SendParameters *sendParameters, const char *file,
									   unsigned int line) {
...
void RNS2_NativeClient::BufferSend(RNS2_SendParameters *sendParameters, const char *file,
								   unsigned int line) {
...
bool IRNS2_Berkley::IsPortInUse(unsigned short port, const char *hostAddress,
...
void SocketReadCallback(CFSocketRef s, CFSocketCallBackType type, CFDataRef address,
...
RNS2BindResult RNS2_Windows::Bind(RNS2_BerkleyBindParameters *bindParameters, const char *file,
								  unsigned int line) {
...
RNS2SendResult RNS2_Windows::Send(RNS2_SendParameters *sendParameters, const char *file,
								  unsigned int line) {
	if (slo) {
		RNS2SendResult len;
		len = slo->RakNetSendTo(sendParameters->data, sendParameters->length,
...
#else
RNS2BindResult RNS2_Linux::Bind(RNS2_BerkleyBindParameters *bindParameters, const char *file,
								unsigned int line) {
	return BindShared(bindParameters, file, line);
}
RNS2SendResult RNS2_Linux::Send(RNS2_SendParameters *sendParameters, const char *file,
								unsigned int line) {
	return Send_Windows_Linux_360NoVDP(rns2Socket, sendParameters, file, line);
}
