...
void HuffmanEncodingTree::GenerateFromFrequencyTable(unsigned int frequencyTable[256]) {
	...
	HuffmanEncodingTreeNode
		*leafList[256]; // Keep a copy of the pointers to all the leaves so we can generate the
	...
	// two trees are the left and right children of a new node, where the new node has the weight
	// the sum of the weight of the left and right child nodes.
#ifdef _MSC_VER
	...
	for (counter = 0; counter < 256; counter++) {
		...
		do {
			if (currentNode->parent->left ==
				currentNode) // We're storing the paths in reverse order.since we are going from the
							 // leaf to the root
		...
		while (tempPathLength-- > 0) {
			if (tempPath[tempPathLength]) // Write 1's and 0's because writing a bool will write the
										  ...
										  // defined along with the actual data bit, which is not
										  // what we want
				bitStream.Write1();
			else
				bitStream.Write0();
		...
		// Read data from the bitstream, which is written to the encoding table in bits and
		// bitlength. Note this function allocates the encodingTable[counter].encoding pointer
...
unsigned HuffmanEncodingTree::DecodeArray(RakNet::BitStream *input, BitSize_t sizeInBits,
...
void HuffmanEncodingTree::DecodeArray(unsigned char *input, BitSize_t sizeInBits,
									  RakNet::BitStream *output) {
	HuffmanEncodingTreeNode *currentNode;
	...
	for (unsigned counter = 0; counter < sizeInBits; counter++) {
		...
		if (currentNode->left == 0 && currentNode->right == 0) // Leaf
		{
			output->WriteBits(&(currentNode->value), sizeof(char) * 8,
							  true); // Use WriteBits instead of Write(char) because we want to
									 // avoid TYPE_CHECKING
			currentNode = root;
		}
	}
