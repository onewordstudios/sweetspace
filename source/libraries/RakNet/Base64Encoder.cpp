...
int Base64Encoding(const unsigned char *inputData, int dataLength, char *outputData) {
	...
	for (j = 0; j < write3Count; j++) {
		...
		// Remaining 2 bits from first byte, placed in position, and 4 high bits from the second
		...
		// 4 low bits from the second byte and the two high bits from the third byte, masked to
		// ignore bits 7,8
		outputData[outputOffset++] =
			base64Map[((inputData[j * 3 + 1] << 2) | (inputData[j * 3 + 2] >> 6)) &
					  63]; // Third 6 bits
		if ((++charCount % 76) == 0) {
			outputData[outputOffset++] = '\r';
			outputData[outputOffset++] = '\n';
