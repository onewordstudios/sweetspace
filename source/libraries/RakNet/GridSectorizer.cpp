...
void GridSectorizer::Init(const float _maxCellWidth, const float _maxCellHeight, const float minX,
...
void GridSectorizer::AddEntry(void *entry, const float minX, const float minY, const float maxX,
							  const float maxY) {
...
#ifdef _USE_ORDERED_LIST
void GridSectorizer::RemoveEntry(void *entry, const float minX, const float minY, const float maxX,
								 const float maxY) {
	RakAssert(cellWidth > 0.0f);
	RakAssert(minX <= maxX && minY <= maxY);
...
void GridSectorizer::MoveEntry(void *entry, const float sourceMinX, const float sourceMinY,
...
#endif
void GridSectorizer::GetEntries(DataStructures::List<void *> &intersectionList, const float minX,
								const float minY, const float maxX, const float maxY) {
#ifdef _USE_ORDERED_LIST
...
int GridSectorizer::WorldToCellXOffsetAndClamped(const float input) const {
	...
	cell = gridCellWidthCount - 1 < cell ? gridCellWidthCount - 1
...
int GridSectorizer::WorldToCellYOffsetAndClamped(const float input) const {
	...
	cell = cell > 0 ? cell : 0; // __max(cell,0);
	cell = gridCellHeightCount - 1 < cell ? gridCellHeightCount - 1
