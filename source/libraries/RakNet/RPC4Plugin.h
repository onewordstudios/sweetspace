...
class RAK_DLL_EXPORT RPC4 : public PluginInterface2 {
   public:
	...
	/// actual Packet that RakNet got. \sa RegisterPacketCallback() \param[in] uniqueID Identifier
	/// to be associated with \a functionPointer. If this identifier is already in use, the call
	/// will return false. \param[in] functionPointer C function pointer to be called \return True
	/// if the hash of uniqueID is not in use, false otherwise.
	...
	/// are called by order of the highest callPriority first. For slots with the same priority,
	...
	void Call(const char *uniqueID, RakNet::BitStream *bitStream, PacketPriority priority,
	...
	/// \return true if successfully called. False on disconnect, function not registered, or not
	...
	/// \param[in] sharedIdentifier parameter of the same name passed to RegisterSlot() on the
	/// remote system \param[in] bitStream bitStream encoded data to send to the function callback
	/// \param[in] priority See RakPeerInterface::Send()
	/// \param[in] reliability See RakPeerInterface::Send()
	/// \param[in] orderingChannel See RakPeerInterface::Send()
	/// \param[in] systemIdentifier See RakPeerInterface::Send()
	...
	DataStructures::Hash<RakNet::RakString, LocalSlot *, 256, RakNet::RakString::ToInteger>
		localSlots;
   ...
   protected:
	...
	DataStructures::Hash<RakNet::RakString, void (*)(RakNet::BitStream *, Packet *), 64,
