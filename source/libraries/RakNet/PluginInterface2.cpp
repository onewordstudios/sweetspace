...
void PluginInterface2::SendUnified(const RakNet::BitStream *bitStream, PacketPriority priority,
	...
	if (rakPeerInterface) {
		rakPeerInterface->Send(bitStream, priority, reliability, orderingChannel, systemIdentifier,
							   broadcast);
		return;
	}
#if _RAKNET_SUPPORT_PacketizedTCP == 1 && _RAKNET_SUPPORT_TCPInterface == 1
...
void PluginInterface2::SendUnified(const char *data, const int length, PacketPriority priority,
								   ...
								   const AddressOrGUID systemIdentifier, bool broadcast) {
	if (rakPeerInterface) {
		rakPeerInterface->Send(data, length, priority, reliability, orderingChannel,
...
bool PluginInterface2::SendListUnified(const char **data, const int *lengths,
									   ...
									   const AddressOrGUID systemIdentifier, bool broadcast) {
	if (rakPeerInterface) {
		return rakPeerInterface->SendList(data, lengths, numParameters, priority, reliability,
										  orderingChannel, systemIdentifier, broadcast) != 0;
	}
#if _RAKNET_SUPPORT_PacketizedTCP == 1 && _RAKNET_SUPPORT_TCPInterface == 1
	else if (tcpInterface) {
		return tcpInterface->SendList(data, (const unsigned int *)lengths, numParameters,
									  systemIdentifier.systemAddress, broadcast);
	}
#endif
	...
	if (broadcast == false && systemIdentifier.rakNetGuid == GetMyGUIDUnified()) {
		...
		SendUnified(dataAggregate, totalLength, priority, reliability, orderingChannel,
					systemIdentifier, broadcast);
