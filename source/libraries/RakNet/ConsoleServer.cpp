...
void ConsoleServer::SetTransportProvider(TransportInterface *transportInterface,
										 unsigned short port) {
	// Replace the current TransportInterface, stopping the old one, if present, and starting the
	// new one.
...
void ConsoleServer::Update(void) {
	...
	while (p) {
		...
		RakNet::CommandParserInterface::ParseConsoleString(
			(char *)p->data, COMMAND_DELINATOR, COMMAND_DELINATOR_TOGGLE, &numParameters,
		...
		if (_stricmp(*parameterList, "help") == 0 && numParameters <= 2) {
			...
			if (numParameters == 1) {
				...
				transport->Send(p->systemAddress,
								"You can direct commands to a parser by prefixing the parser name "
								"or number.\r\n");
				...
				transport->Send(p->systemAddress,
								"help <ParserName>                           Show help on a "
								"particular parser.\r\n");
				transport->Send(p->systemAddress,
								"help <CommandName>                          Show help on a "
								"particular command.\r\n");
				transport->Send(
					p->systemAddress,
			...
			if (numParameters >= 2) // At minimum <CommandParserName> <Command>
			{
				...
				if (**parameterList >= '0' && **parameterList <= '9') {
					commandParserIndex =
						atoi(*parameterList); // Use specified parser unless it's an invalid number
					commandParserIndex--;	  // Subtract 1 since we displayed numbers starting at
											  // index+1
					...
					// // Prefixing with the name of a command parser directs to that parser.  See
					// if the first word matches a parser
					for (i = 0; i < commandParserList.Size(); i++) {
			...
			if (failed == false && tryAllParsers) {
				for (i = 0; i < commandParserList.Size(); i++) {
					// Undirected command.  Try all the parsers to see if they understand the
					// command Pass the 1nd element as the command, and the remainder as the
					// parameter list
