...
void RNS2_Berkley::GetSystemAddressIPV4And6(RNS2Socket rns2Socket,
...
RNS2BindResult RNS2_Berkley::BindSharedIPV4(RNS2_BerkleyBindParameters *bindParameters,
	...
	rns2Socket = (int)socket__(bindParameters->addressFamily, bindParameters->type,
	...
	ret = bind__(rns2Socket, (struct sockaddr *)&boundAddress.address.addr4,
	...
	if (ret <= -1) {
		...
		switch (ret) {
			...
			case EINVAL:
				RAKNET_DEBUG_PRINTF(
					"bind__(): The addrlen is wrong, or the socket was not in the AF_UNIX "
					"family.\n");
				break;
			case EROFS:
				RAKNET_DEBUG_PRINTF(
...
void RNS2_Berkley::RecvFromBlockingIPV4And6(RNS2RecvStruct *recvFromStruct) {
...

	{
		if (their_addr.ss_family == AF_INET) {
			memcpy(&recvFromStruct->systemAddress.address.addr4, (sockaddr_in *)&their_addr,
				   sizeof(sockaddr_in));
...
void RNS2_Berkley::RecvFromBlockingIPV4(RNS2RecvStruct *recvFromStruct) {
	...
	recvFromStruct->bytesRead =
		recvfrom__(GetSocket(), recvFromStruct->data, sizeof(recvFromStruct->data), flag,
