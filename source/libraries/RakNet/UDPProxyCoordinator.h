...
class RAK_DLL_EXPORT UDPProxyCoordinator : public PluginInterface2 {
   public:
	...
	struct ForwardingRequest {
		...
		SystemAddress requestingAddress; // Which system originally sent the network message to
										 // start forwarding
   ...
   protected:
	...
	void SendAllBusy(SystemAddress senderClientAddress, SystemAddress targetClientAddress,
					 RakNetGUID targetClientGuid, SystemAddress requestingAddress);
	void Clear(void);

	void SendForwardingRequest(SystemAddress sourceAddress, SystemAddress targetAddress,
