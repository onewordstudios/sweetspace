...
unsigned short DirectoryDeltaTransfer::DownloadFromSubdirectory(
	...
	localFiles.AddFilesFromDirectory(prependAppDirToOutputSubdir ? applicationDirectory : 0,
									 outputSubdir, true, false, true,
									 FileListNodeContext(0, 0, 0, 0));
	return DownloadFromSubdirectory(localFiles, subdir, outputSubdir, prependAppDirToOutputSubdir,
									host, onFileCallback, _priority, _orderingChannel, cb);
}
...
void DirectoryDeltaTransfer::OnDownloadRequest(Packet *packet) {
	...
	// the remote system
	fileListTransfer->Send(&delta, rakPeerInterface, packet->systemAddress, setId, priority,
						   orderingChannel, incrementalReadInterface, chunkSize);
}
