...
class CCRakNetUDT {
   public:
	...
	int GetRetransmissionBandwidth(CCTimeType curTime, CCTimeType timeSinceLastTick,
	...
	/// Acks do not have to be sent immediately. Instead, they can be buffered up such that groups
	/// of acks are sent at a time This reduces overall bandwidth usage How long they can be
	/// buffered depends on the retransmit time of the sender Should call once per update tick, and
	/// send if needed
	...
	void OnGotPacketPair(DatagramSequenceNumberType datagramSequenceNumber, uint32_t sizeInBytes,
						 CCTimeType curTime);
	...
	bool OnGotPacket(DatagramSequenceNumberType datagramSequenceNumber, bool isContinuousSend,
	...
	void OnAck(CCTimeType curTime, CCTimeType rtt, bool hasBAndAS, BytesPerMicrosecond _B,
	...
	void OnSendAckGetBAndAS(CCTimeType curTime, bool *hasBAndAS, BytesPerMicrosecond *_B,
	...
	/// = (RTT + 4 * RTTVar) + SYN If we have been continuously sending for the last RTO, and no ACK
	/// or NAK at all, SND*=2; This is per message, which is different from UDT, but RakNet supports
   ...
   protected:
	...
	/// packetPairRecieptHistory Used to prevent it from being calculated and send too frequently,
	/// as they are slow operations
	...
	/// Every outgoing datagram is assigned a sequence number, which increments by 1 every
	/// assignment
	DatagramSequenceNumberType nextDatagramSequenceNumber;

	/// If a packet is marked as a packet pair, lastPacketPairPacketArrivalTime is set to the time
	/// it arrives This is used so when the 2nd packet of the pair arrives, we can calculate the
	/// time interval between the two
	CCTimeType lastPacketPairPacketArrivalTime;

	/// If a packet is marked as a packet pair, lastPacketPairSequenceNumber is checked to see if
	/// the last packet we got was the packet immediately before the one that arrived If so, we can
	/// use lastPacketPairPacketArrivalTime to get the time between the two packets, and thus
	/// estimate the link capacity Initialized to -1, so the first packet of a packet pair won't be
