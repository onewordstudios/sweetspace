...
RakWString &RakWString::operator=(const RakWString &right) {
	...
	c_str = (wchar_t *)rakMalloc_Ex((right.GetLength() + 1) * MAX_BYTES_PER_UNICODE_CHAR,
									_FILE_AND_LINE_);
	if (!c_str) {
		c_strCharLength = 0;
...
RakWString &RakWString::operator=(const wchar_t *const str) {
	...
	c_str = (wchar_t *)rakMalloc_Ex((c_strCharLength + 1) * MAX_BYTES_PER_UNICODE_CHAR,
									_FILE_AND_LINE_);
	if (!c_str) {
		c_strCharLength = 0;
		notifyOutOfMemory(_FILE_AND_LINE_);
...
RakWString &RakWString::operator+=(const RakWString &right) {
	...
	if (isEmpty)
		newCStr = (wchar_t *)rakMalloc_Ex((newCharLength + 1) * MAX_BYTES_PER_UNICODE_CHAR,
										  _FILE_AND_LINE_);
	else
		newCStr = (wchar_t *)rakRealloc_Ex(c_str, (newCharLength + 1) * MAX_BYTES_PER_UNICODE_CHAR,
										   _FILE_AND_LINE_);
	if (!newCStr) {
		notifyOutOfMemory(_FILE_AND_LINE_);
		return *this;
...
#else
		c_str = (wchar_t *)rakMalloc_Ex((mbByteLength + 1) * MAX_BYTES_PER_UNICODE_CHAR,
										_FILE_AND_LINE_);
