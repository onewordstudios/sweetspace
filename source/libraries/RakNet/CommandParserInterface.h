...
class RAK_DLL_EXPORT CommandParserInterface {
   public:
	...
	virtual void OnNewIncomingConnection(const SystemAddress &systemAddress,
										 TransportInterface *transport);

	/// \brief A callback for when \a systemAddress has disconnected, either gracefully or
	/// forcefully \param[in] systemAddress The player that has disconnected. \param[in] transport
	/// The transport interface that sent us this information.
	virtual void OnConnectionLost(const SystemAddress &systemAddress,
								  TransportInterface *transport);
	...
	/// \param[in] delineatorToggle When encountered the delineator replacement is toggled on and
	/// off \param[out] numParameters How many pointers were written to \a parameterList \param[out]
	/// parameterList An array of pointers to characters.  Will hold pointers to locations inside \a
	/// str \param[in] parameterListLength How big the \a parameterList array is
	...
	/// Registers a command.
	/// \param[in] parameterCount How many parameters your command requires.  If you want to accept
	...
	virtual void RegisterCommand(unsigned char parameterCount, const char *command,
								 const char *commandHelp);
	...
	/// \param[in] systemAddress The player this result will be sent to
	virtual void ReturnResult(const char *command, TransportInterface *transport,
							  const SystemAddress &systemAddress);
