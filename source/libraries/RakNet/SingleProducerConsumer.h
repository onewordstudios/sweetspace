...
class RAK_DLL_EXPORT SingleProducerConsumer {
   public:
	...
	/// WriteLock must be immediately followed by WriteUnlock.  These two functions must be called
	/// in the same thread. \return A pointer to a block of data you can write to.
	...
	/// This function will estimate how many elements are waiting to be read.  It's threadsafe
	/// enough that the value returned is stable, but not threadsafe enough to give accurate
	/// results. \return An ESTIMATE of how many data elements are waiting to be read
	int Size(void) const;
   ...
   private:
	struct DataPlusPtr {
		...
		// Ready to read is so we can use an equality boolean comparison, in case the writePointer
		// var is trashed while context switching.
