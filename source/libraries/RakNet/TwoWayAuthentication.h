...
class RAK_DLL_EXPORT TwoWayAuthentication : public PluginInterface2 {
   public:
	...
	/// remote system, rather than having to hash against every known password. \param[in]
	/// identifier A unique identifier representing this password. This is transmitted in plaintext
	/// and should be considered insecure \param[in] password The password to add \return True on
	/// success, false on identifier==password, either identifier or password is blank, or
	...
	/// ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_FAILED ID_TWO_WAY_AUTHENTICATION_SUCCESS will
	/// be returned if and only if the other system has called AddPassword() with the same
