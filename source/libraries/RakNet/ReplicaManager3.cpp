...
bool PRO::operator==(const PRO &right) const {
	return priority == right.priority && reliability == right.reliability &&
		   orderingChannel == right.orderingChannel && sendReceipt == right.sendReceipt;
}
...
RakNet::Connection_RM3 *ReplicaManager3::PopConnection(unsigned int index, WorldId worldId) {
	...
	for (index2 = 0; index2 < replicaList.Size(); index2++) {
		...
		if (action == RM3AOPC_DELETE_REPLICA) {
			...
			for (unsigned int index3 = 0; index3 < connection->queryToSerializeReplicaList.Size();
				 index3++) {
	...
	for (index2 = 0; index2 < destructionList.Size(); index2++) {
		...
		Replica3 *replicaToDestroy =
			world->networkIDManager->GET_OBJECT_FROM_ID<Replica3 *>(destructionList[index2]);
		if (replicaToDestroy) {
...
unsigned int ReplicaManager3::ReferenceInternal(RakNet::Replica3 *replica3, WorldId worldId) {
	...
	if (index == (unsigned int)-1) {
		...
		// If it crashes on rakPeerInterface==0 then you didn't call
		// RakPeerInterface::AttachPlugin()
...
void ReplicaManager3::GetReplicasCreatedByMe(DataStructures::List<Replica3 *> &replicaListOut,
	...
	GetReplicasCreatedByGuid(rakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS),
							 replicaListOut, worldId);
}
...
Connection_RM3 *ReplicaManager3::GetConnectionBySystemAddress(const SystemAddress &sa,
...
void ReplicaManager3::RM3World::Clear(ReplicaManager3 *replicaManager3) {
	if (replicaManager3->GetAutoDestroyConnections()) {
		...
		// Clear out downloadGroup even if not auto destroying the connection, since the packets
		// need to go back to RakPeer
		for (unsigned int i = 0; i < connectionList.Size(); i++)
...
PluginReceiveResult ReplicaManager3::OnReceive(Packet *packet) {
	...
	if ((unsigned char)packet->data[0] == ID_TIMESTAMP) {
		if (packet->length > sizeof(unsigned char) + sizeof(RakNet::Time)) {
			packetIdentifier =
				(unsigned char)packet->data[sizeof(unsigned char) + sizeof(RakNet::Time)];
	...
	switch (packetIdentifier) {
		case ID_REPLICA_MANAGER_CONSTRUCTION:
			return OnConstruction(packet, packet->data, packet->length, packet->guid,
								  packetDataOffset, incomingWorldId);
		case ID_REPLICA_MANAGER_SERIALIZE:
			return OnSerialize(packet, packet->data, packet->length, packet->guid, timestamp,
							   packetDataOffset, incomingWorldId);
		case ID_REPLICA_MANAGER_DOWNLOAD_STARTED:
			if (packet->wasGeneratedLocally == false) {
				return OnDownloadStarted(packet, packet->data, packet->length, packet->guid,
										 packetDataOffset, incomingWorldId);
			} else
				break;
		case ID_REPLICA_MANAGER_DOWNLOAD_COMPLETE:
			if (packet->wasGeneratedLocally == false) {
				return OnDownloadComplete(packet, packet->data, packet->length, packet->guid,
										  packetDataOffset, incomingWorldId);
			} else
				break;
...
void Connection_RM3::AutoConstructByQuery(ReplicaManager3 *replicaManager3, WorldId worldId) {
	...
	if (constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION ||
		...
		for (idx2 = 0; idx2 < destroyedReplicasCulled.Size(); idx2++) {
			exists = false;
			bool objectExists;
			idx1 = constructedReplicaList.GetIndexFromKey(destroyedReplicasCulled[idx2],
														  &objectExists);
			if (objectExists) {
			...
			for (index = 0; index < world->connectionList.Size(); index++) {
				...
				if (connection->QuerySerializationList(replicasToSerialize)) {
					...
					while (index2 < replicasToSerialize.Size()) {
						...
						ssicr = connection->SendSerializeIfChanged(lsr, &sp, GetRakPeerInterface(),
																   worldId, this, time);
...
void ReplicaManager3::OnNewConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
									  bool isIncoming) {
	(void)isIncoming;
...
PluginReceiveResult ReplicaManager3::OnConstruction(Packet *packet, unsigned char *packetData,
	...
	for (index = 0; index < constructionObjectListSize; index++) {
		...
		if (actuallyCreateObject) {
			...
			// didn't read out the bitStream the same way it was written bitOffset2 is already
			// aligned
		...
		if (pdcWritten) {
			...
			if (constructionTickStack[index] != 0) {
				...
				if (actuallyCreateObjectList[index])
					...
					constructionTickStack[index]->PostDeserializeConstructionExisting(&bsIn,
																					  connection);
			}
			bsIn.SetReadOffset(streamEnd);
		} else {
			if (constructionTickStack[index] != 0) {
				if (actuallyCreateObjectList[index])
					...
					constructionTickStack[index]->PostDeserializeConstructionExisting(&empty,
																					  connection);
			}
		}
	}
...
PluginReceiveResult ReplicaManager3::OnSerialize(Packet *packet, unsigned char *packetData,
												 int packetDataLength, RakNetGUID senderGuid,
...
PluginReceiveResult ReplicaManager3::OnDownloadStarted(Packet *packet, unsigned char *packetData,
													   int packetDataLength, RakNetGUID senderGuid,
													   unsigned char packetDataOffset,
													   WorldId worldId) {
	Connection_RM3 *connection = GetConnectionByGUID(senderGuid, worldId);
...
PluginReceiveResult ReplicaManager3::OnDownloadComplete(Packet *packet, unsigned char *packetData,
														int packetDataLength, RakNetGUID senderGuid,
														unsigned char packetDataOffset,
														WorldId worldId) {
	Connection_RM3 *connection = GetConnectionByGUID(senderGuid, worldId);
	...
	if (connection->groupConstructionAndSerialize == true && connection->downloadGroup.Size() > 0) {
		...
		// groupConstructionAndSerialize will be false and downloadGroup will be empty, so it will
		// go past this block
	...
	for (j = 0; j < world->connectionList.Size(); j++) {
		...
		if (cnt > 0) {
			...
			rakPeerInterface->Send(
				&bsOut, defaultSendParameters.priority, defaultSendParameters.reliability,
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void Connection_RM3::SendSerializeHeader(RakNet::Replica3 *replica, RakNet::Time timestamp,
...
SendSerializeIfChangedResult Connection_RM3::SendSerialize(
	...
	for (channelIndex = 0; channelIndex < RM3_NUM_OUTPUT_BITSTREAM_CHANNELS; channelIndex++) {
		if (channelIndex == 0) {
			...
			// Send remainder
			replica->OnSerializeTransmission(&out, this, bitsPerChannel, curTime);
			rakPeer->Send(&out, lastPro.priority, lastPro.reliability, lastPro.orderingChannel,
						  systemAddress, false, lastPro.sendReceipt);
...
SendSerializeIfChangedResult Connection_RM3::SendSerializeIfChanged(
	...
	if (replica->forceSendUntilNextUpdate) {
		...
		return SendSerialize(replica, replica->lastSentSerialization.indicesToSend,
							 replica->lastSentSerialization.bitStream, sp->messageTimestamp,
							 sp->pro, rakPeer, worldId, curTime);
	}
	...
	if (serializationResult == RM3SR_SERIALIZED_ALWAYS) {
		...
		return SendSerialize(replica, allIndices, sp->outputBitstream, sp->messageTimestamp,
							 sp->pro, rakPeer, worldId, curTime);
	}
	...
	// Send out the data
	return SendSerialize(replica, indicesToSend, sp->outputBitstream, sp->messageTimestamp, sp->pro,
...
void Connection_RM3::OnConstructToThisConnection(unsigned int queryToConstructIdx,
												 ReplicaManager3 *replicaManager) {
	ConstructionMode constructionMode = QueryConstructionMode();
	RakAssert(constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION ||
			  constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION_AND_DESTRUCTION);
...
void Connection_RM3::OnNeverSerialize(LastSerializationResult *lsr,
									  ReplicaManager3 *replicaManager) {
	ValidateLists(replicaManager);
...
void Connection_RM3::OnSendDestructionFromQuery(unsigned int queryToDestructIdx,
												ReplicaManager3 *replicaManager) {
	ConstructionMode constructionMode = QueryConstructionMode();
	RakAssert(constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION ||
			  constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION_AND_DESTRUCTION);
...
void Connection_RM3::SendConstruction(DataStructures::List<Replica3 *> &newObjects,
	...
	rakPeer->Send(&bsOut, sendParameters.priority, RELIABLE_ORDERED, sendParameters.orderingChannel,
				  systemAddress, false, sendParameters.sendReceipt);
	...
	for (newListIndex = 0; newListIndex < newObjects.Size(); newListIndex++) {
		...
		if (res != RM3SR_NEVER_SERIALIZE_FOR_THIS_CONNECTION && res != RM3SR_DO_NOT_SERIALIZE &&
			...
			SendSerialize(replica, allIndices, sp.outputBitstream, sp.messageTimestamp, sp.pro,
						  rakPeer, worldId, GetTime());
...
RM3ConstructionState Replica3::QueryConstruction_PeerToPeer(
	RakNet::Connection_RM3 *destinationConnection, Replica3P2PMode p2pMode) {
	(void)destinationConnection;

	if (p2pMode == R3P2PM_SINGLE_OWNER) {
		// We send to all, others do nothing
		if (creatingSystemGUID == replicaManager->GetRakPeerInterface()->GetGuidFromSystemAddress(
		...
		// RM3CS_ALREADY_EXISTS_REMOTELY will not send the object, but WILL call
		// QuerySerialization() and Serialize() on it.
		return RM3CS_ALREADY_EXISTS_REMOTELY;
	}
}
