...
namespace RakNet {
extern void ProcessNetworkPacket(const SystemAddress systemAddress, const char *data,
...
void SocketLayer::SetSocketOptions(__UDPSOCKET__ listenSocket, bool blockingSocket,
								   bool setBroadcast) {
	...
	if (setBroadcast) {
		...
		sock_opt = 1;
		if (setsockopt__(listenSocket, SOL_SOCKET, SO_BROADCAST, (char *)&sock_opt,
						 sizeof(sock_opt)) == -1) {
...
RakNet::RakString SocketLayer::GetSubNetForSocketAndIp(__UDPSOCKET__ inSock,
	...
	if (WSAIoctl(inSock, SIO_GET_INTERFACE_LIST, 0, 0, &InterfaceList, sizeof(InterfaceList),
...
void GetMyIP_Win32(SystemAddress addresses[MAXIMUM_NUMBER_OF_INTERNAL_IDS]) {
	...
	for (idx = 0, aip = servinfo; aip != NULL && idx < MAXIMUM_NUMBER_OF_INTERNAL_IDS;
		 aip = aip->ai_next, idx++) {
