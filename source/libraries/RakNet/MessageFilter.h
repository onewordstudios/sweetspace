...
struct FilterSet {
	...
	void (*invalidMessageCallback)(RakPeerInterface *peer, AddressOrGUID systemAddress,
								   int filterSetID, void *userData, unsigned char messageID);
	void *disallowedCallbackUserData;
	void (*timeoutCallback)(RakPeerInterface *peer, AddressOrGUID systemAddress, int filterSetID,
							void *userData);
...
class RAK_DLL_EXPORT MessageFilter : public PluginInterface2 {
   public:
	...
	/// constantly refer back to this function. \param[in] allow True to allow this message ID,
	/// false to disallow. By default, all messageIDs except the noted types are disallowed.  This
	/// includes messages from other plugins! \param[in] messageIDStart The first ID_* message to
	/// allow in the range.  Inclusive. \param[in] messageIDEnd The last ID_* message to allow in
	/// the range.  Inclusive. \param[in] filterSetID A user defined ID to represent a filter set.
	/// If no filter with this ID exists, one will be created with default settings.
	...
	/// RakPeer::AddToBanList. \param[in] filterSetID A user defined ID to represent a filter set.
	/// If no filter with this ID exists, one will be created with default settings.
	...
	/// milliseconds to allow a connection to stay in this filter set. \param[in] banOnExceed True
	/// or false to ban the system, or not, when \a allowedTimeMS is exceeded \param[in] banTimeMS
	...
	void SetFilterMaxTime(int allowedTimeMS, bool banOnExceed, RakNet::TimeMS banTimeMS,
						  int filterSetID);
	...
	/// SetAutoAddNewConnectionsToFilter() This function is used to change the filter set a system
	/// is using, to add it to a new filter set, or to remove it from all existin filter sets.
	/// \param[in] addressOrGUID The system we are referring to
	/// \param[in] filterSetID A user defined ID to represent a filter set.  If no filter with this
	/// ID exists, one will be created with default settings.  If -1, the system will be removed
	/// from all filter sets.
