...
enum NATTypeDetectionResult {
	/// Works with anyone
	NAT_TYPE_NONE,
	/// Accepts any datagrams to a port that has been previously used. Will accept the first
	/// datagram from the remote peer.
	NAT_TYPE_FULL_CONE,
	...
	/// Same as address-restricted cone NAT, but we had to send to both the correct remote IP
	/// address and correct remote port. The same source address and port to a different destination
	...
	/// a different destination uses a different mapping. Since the port will be different, the
	/// first external punchthrough attempt will fail. For this to work it requires port-prediction
