...
void UDPProxyCoordinator::OnForwardingRequestFromClientToCoordinator(Packet *packet) {
	...
	if (serverList.Size() > 1) {
		...
		rakPeerInterface->Send(&outgoingBs, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0, sourceAddress,
							   false);
		rakPeerInterface->Send(&outgoingBs, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0, targetAddress,
							   false);
		...
		forwardingRequestList.InsertAtIndex(fw, insertionIndex, _FILE_AND_LINE_);
		SendForwardingRequest(sourceAddress, targetAddress, fw->currentlyAttemptedServerAddress,
...
void UDPProxyCoordinator::OnLoginRequestFromServerToCoordinator(Packet *packet) {
	...
	outgoingBs.Write(password);
	rakPeerInterface->Send(&outgoingBs, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0, packet->systemAddress,
						   false);
}
...
void UDPProxyCoordinator::TryNextServer(SenderAndTargetAddress sata, ForwardingRequest *fw) {
	...
	if (pickedGoodServer == false) {
		SendAllBusy(sata.senderClientAddress, sata.targetClientAddress, sata.targetClientGuid,
...
void UDPProxyCoordinator::SendAllBusy(SystemAddress senderClientAddress,
	...
	rakPeerInterface->Send(&outgoingBs, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0, requestingAddress,
						   false);
}
...
void UDPProxyCoordinator::ForwardingRequest::OrderRemainingServersToTry(void) {
	...
	// swpList;
	DataStructures::OrderedList<unsigned short, UDPProxyCoordinator::ServerWithPing,
								ServerWithPingComp>
		swpList;
