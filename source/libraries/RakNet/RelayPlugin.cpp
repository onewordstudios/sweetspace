...
// \a key
void RelayPlugin::SendToParticipant(const RakNetGUID &relayPluginServerGuid, const RakString &key,
									BitStream *bitStream, PacketPriority priority,
		...
		switch (packet->data[1]) {
			case RPE_MESSAGE_TO_SERVER_FROM_CLIENT: {
				...
				if (strAndGuid && strAndGuidSender) {
					...
					bsOut.Write(bsData);
					SendUnified(&bsOut, priority, reliability, orderingChannel, (*strAndGuid)->guid,
								false);
				}
...
RelayPlugin::RP_Group *RelayPlugin::JoinGroup(RP_Group *room,
