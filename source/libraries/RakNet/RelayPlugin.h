...
class RAK_DLL_EXPORT RelayPlugin : public PluginInterface2 {
   public:
	...
	/// RPE_ADD_CLIENT_TARGET_NOT_CONNECTED, RPE_ADD_CLIENT_NAME_ALREADY_IN_USE, or
	/// RPE_ADD_CLIENT_OK
	...
	/// \pre The server must have called SetAcceptAddParticipantRequests(true) or the request will
	/// be ignored \param[in] key A string to identify out system. Passed to \a key on
	/// AddParticipantOnServer() \param[in] relayPluginServerGuid the RakNetGUID of the system
	/// running RelayPlugin
	void AddParticipantRequestFromClient(const RakString &key,
	...
	/// \param[in] destinationGuid The key value passed to AddParticipant() earlier on the server.
	/// If this was not done, the server will not relay the message (it will be silently discarded).
	...
	void SendToParticipant(const RakNetGUID &relayPluginServerGuid,
						   const RakString &destinationGuid, BitStream *bitStream,
						   PacketPriority priority, PacketReliability reliability,
						   char orderingChannel);

	void SendGroupMessage(const RakNetGUID &relayPluginServerGuid, BitStream *bitStream,
						  PacketPriority priority, PacketReliability reliability,
						  char orderingChannel);
   ...
   protected:
	...
	DataStructures::Hash<RakString, StrAndGuidAndRoom *, 8096, RakNet::RakString::ToInteger>
		strToGuidHash;
	DataStructures::Hash<RakNetGUID, StrAndGuidAndRoom *, 8096, RakNet::RakNetGUID::ToUint32>
		guidToStrHash;
