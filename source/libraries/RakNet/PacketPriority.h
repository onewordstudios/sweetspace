...
enum PacketPriority {
	/// The highest possible priority. These message trigger sends immediately, and are generally
	/// not buffered or aggregated into a single datagram.
	IMMEDIATE_PRIORITY,
...
enum PacketReliability {
	...
	/// to 17) + 21 bits of overhead, 16 of which is used to detect duplicate packets and 6 to 17 of
	...
	/// and ordered messages sent on the same channel will arrive in the order sent.
	RELIABLE_ORDERED,
	...
	/// RakPeerInterface::Receive(). Bytes 1-4 will contain the number returned from the Send()
	/// function. On disconnect or shutdown, all messages not previously acked should be considered
	/// lost. 05/04/10 You can't have sequenced and ack receipts, because you don't know if the
	/// other system discarded the message, meaning you don't know if the message was processed
	...
	/// returned when the message arrives, not necessarily the order when it was sent. Bytes 1-4
	/// will contain the number returned from the Send() function. On disconnect or shutdown, all
	...
	/// returned from the Send() function. On disconnect or shutdown, all messages not previously
	/// acked should be considered lost. 05/04/10 You can't have sequenced and ack receipts, because
	/// you don't know if the other system discarded the message, meaning you don't know if the
