...
void WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::AddNode(const node_type &node) {
	adjacencyLists.SetNew(
		node, RakNet::OP_NEW<DataStructures::Map<node_type, weight_type> >(_FILE_AND_LINE_));
}
...
#pragma warning(disable : 4127) // warning C4127: conditional expression is constant
	...
	path.Insert(startNode, _FILE_AND_LINE_);
	for (col = 0; outputQueue.Size(); col++) path.Insert(outputQueue.Pop(), _FILE_AND_LINE_);
	return true;
}
...
bool WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::GetSpanningTree(
	...
	// Find the shortest path from the start node to each of the input nodes.  Add this path to a
	// new WeightedGraph if the result is reachable
...
void WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::GenerateDisjktraMatrix(
	...
	for (col = 0; col < adjacencyLists.Size(); col++) {
		// This should be already sorted, so it's a bit inefficient to do an insertion sort, but
		// what the heck
