...
int CloudServer::BufferedGetResponseFromServerComp(
	const RakNetGUID &key, CloudServer::BufferedGetResponseFromServer *const &data) {
	if (key < data->serverAddress) return -1;
	if (key > data->serverAddress) return 1;
	return 0;
}
...
void CloudServer::OnPostRequest(Packet *packet) {
	...
	CloudDataList *cloudDataList =
		GetOrAllocateCloudDataList(key, &dataRepositoryExists, dataRepositoryIndex);
	...
	if (keyDataListExists == false) {
		if (maxUploadBytesPerClient > 0 &&
			remoteCloudClient->uploadedBytes + dataLengthBytes > maxUploadBytesPerClient) {
		...
		if (maxUploadBytesPerClient > 0 &&
			remoteCloudClient->uploadedBytes - cloudData->dataLengthBytes + dataLengthBytes >
...
void CloudServer::OnReleaseRequest(Packet *packet) {
	...
	for (unsigned int filterIndex = 0; filterIndex < queryFilters.Size(); filterIndex++) {
		if (queryFilters[filterIndex]->OnReleaseRequest(packet->guid, packet->systemAddress,
														cloudKeys) == false)
			return;
	}
...
void CloudServer::OnGetRequest(Packet *packet) {
	...
	for (unsigned int filterIndex = 0; filterIndex < queryFilters.Size(); filterIndex++) {
		if (queryFilters[filterIndex]->OnGetRequest(
				packet->guid, packet->systemAddress, getRequest->cloudQueryWithAddresses.cloudQuery,
	...
	GetServersWithUploadedKeys(getRequest->cloudQueryWithAddresses.cloudQuery.keys,
	...
	if (remoteServersWithData.Size() == 0) {
		...
		for (unsigned int remoteServerIndex = 0; remoteServerIndex < remoteServersWithData.Size();
			 remoteServerIndex++) {
	...
	if (getRequest->cloudQueryWithAddresses.cloudQuery.subscribeToResults) {
		// Add to key subscription list for the client, which contains a keyId /
		// specificUploaderList pair
		DataStructures::HashIndex remoteSystemsHashIndex = remoteSystems.GetIndexOf(packet->guid);
		RemoteCloudClient *remoteCloudClient;
		if (remoteSystemsHashIndex.IsInvalid()) {
			remoteCloudClient = RakNet::OP_NEW<RemoteCloudClient>(_FILE_AND_LINE_);
		...
		for (keyIndex = 0; keyIndex < getRequest->cloudQueryWithAddresses.cloudQuery.keys.Size();
			 keyIndex++) {
			...
			keySubscriberIndex =
				remoteCloudClient->subscribedKeys.GetIndexFromKey(cloudKey, &hasKeySubscriber);
			KeySubscriberID *keySubscriberId;
			if (hasKeySubscriber) {
				DataStructures::List<RakNetGUID> specificSystems;
				UnsubscribeFromKey(remoteCloudClient, packet->guid, keySubscriberIndex, cloudKey,
								   specificSystems);
			}
			...
			if (getRequest->cloudQueryWithAddresses.specificSystems.Size()) {
				...
				if (subscribedKeysIndexExists) {
					...
					unsigned int specificSystemIndex;
					for (specificSystemIndex = 0;
						 specificSystemIndex < keySubscriberId->specificSystemsSubscribedTo.Size();
						 specificSystemIndex++) {
						bool keyDataExists;
						unsigned int keyDataIndex = cloudDataList->keyData.GetIndexFromKey(
							keySubscriberId->specificSystemsSubscribedTo[specificSystemIndex],
							&keyDataExists);
						if (keyDataExists) {
...
void CloudServer::OnUnsubscribeRequest(Packet *packet) {
	...
	for (unsigned int filterIndex = 0; filterIndex < queryFilters.Size(); filterIndex++) {
		if (queryFilters[filterIndex]->OnUnsubscribeRequest(packet->guid, packet->systemAddress,
															cloudKeys, specificSystems) == false)
			return;
	}
...
void CloudServer::OnServerToServerGetResponse(Packet *packet) {
	...
	remoteServerResponsesIndex =
		getRequest->remoteServerResponses.GetIndexFromKey(packet->guid, &hasRemoteServer);
...
void CloudServer::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
	...
	if (objectExists) {
		// Update remoteServerResponses by removing this server and sending the response if it is
		// now complete
		...
		while (getRequestIndex < getRequests.Size()) {
			...
			unsigned int remoteServerResponsesIndex =
				getRequest->remoteServerResponses.GetIndexFromKey(rakNetGUID,
																  &waitingForThisServer);
			if (waitingForThisServer) {
				getRequest->remoteServerResponses[remoteServerResponsesIndex]->Clear(this);
				RakNet::OP_DELETE(getRequest->remoteServerResponses[remoteServerResponsesIndex],
								  _FILE_AND_LINE_);
	...
	if (remoteSystemIndex.IsInvalid() == false) {
		RemoteCloudClient *remoteCloudClient = remoteSystems.ItemAtIndex(remoteSystemIndex);
		unsigned int uploadedKeysIndex;
		for (uploadedKeysIndex = 0; uploadedKeysIndex < remoteCloudClient->uploadedKeys.Size();
			 uploadedKeysIndex++) {
			...
			if (keyDataRepositoryExists) {
				...
				if (keyDataExists) {
					...
					if (cloudData->IsUnused()) {
						...
						if (cloudDataList->IsNotUploaded()) {
							// Tell other servers that this key is no longer uploaded, so they do
							// not request it from us
							RemoveUploadedKeyFromServers(cloudDataList->key);
						}
...
void CloudServer::NotifyClientSubscribersOfDataChange(
	...
	DataStructures::OrderedList<RakNetGUID, RakNetGUID> &subscribers, bool wasUpdated) {
	RakNet::BitStream bsOut;
...
void CloudServer::NotifyClientSubscribersOfDataChange(
	CloudQueryRow *row, DataStructures::OrderedList<RakNetGUID, RakNetGUID> &subscribers,
	bool wasUpdated) {
	RakNet::BitStream bsOut;
	bsOut.Write((MessageID)ID_CLOUD_SUBSCRIPTION_NOTIFICATION);
	bsOut.Write(wasUpdated);
...
void CloudServer::NotifyServerSubscribersOfDataChange(CloudData *cloudData, CloudKey &key,
													  bool wasUpdated) {
	...
	for (i = 0; i < remoteServers.Size(); i++) {
		if (remoteServers[i]->gotSubscribedAndUploadedKeys == false ||
			...
			SendUnified(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, remoteServers[i]->serverAddress,
						false);
		}
	}
}
...
void CloudServer::ProcessAndTransmitGetRequest(GetRequest *getRequest) {
	...
	DataStructures::List<CloudData *> cloudDataResultList;
	DataStructures::List<CloudKey> cloudKeyResultList;
	ProcessCloudQueryWithAddresses(getRequest->cloudQueryWithAddresses, cloudDataResultList,
								   cloudKeyResultList);
	...
	for (unsigned int i = getRequest->cloudQueryWithAddresses.cloudQuery.startingRowIndex;
		 i < localNumRows; i++) {
		WriteCloudQueryRowFromResultList(i, cloudDataResultList, cloudKeyResultList, &bsOut);
	}
	...
	if (unlimitedRows == true ||
		getRequest->cloudQueryWithAddresses.cloudQuery.maxRowsToReturn > localRowsToWrite) {
		...
		for (remoteServerResponseIndex = 0;
			...
			for (cloudQueryRowIndex = 0;
				...
				bufferedGetResponseFromServer->queryResult.rowsReturned[cloudQueryRowIndex]
...
void CloudServer::ProcessCloudQueryWithAddresses(
	...
	for (queryIndex = 0; queryIndex < cloudQueryWithAddresses.cloudQuery.keys.Size();
		 queryIndex++) {
		...
		if (dataRepositoryExists) {
			...
			if (cloudDataList->uploaderCount > 0) {
				...
				if (cloudQueryWithAddresses.specificSystems.Size() > 0) {
					...
					unsigned int specificSystemIndex;
					for (specificSystemIndex = 0;
						 specificSystemIndex < cloudQueryWithAddresses.specificSystems.Size();
						 specificSystemIndex++) {
						bool uploaderExists;
...
void CloudServer::OnSendSubscribedKeyToServers(Packet *packet) {
	...
	// Do not need to send current values, the Get request will do that as the Get request is sent
	// at the same time
	if (alreadyHasKey == false)
		remoteServer->subscribedKeys.Insert(cloudKey, cloudKey, true, _FILE_AND_LINE_);
}
...
void CloudServer::OnServerDataChanged(Packet *packet) {
	...
	bool keyDataListExists;
	unsigned int keyDataListIndex =
		cloudDataList->keyData.GetIndexFromKey(row.clientGUID, &keyDataListExists);
...
CloudServer::CloudDataList *CloudServer::GetOrAllocateCloudDataList(
	CloudKey key, bool *dataRepositoryExists, unsigned int &dataRepositoryIndex) {
	CloudDataList *cloudDataList;
	...
	if (specificSystems.Size() == 0) {
		...
		if (cloudDataList->RemoveSubscriber(remoteCloudClientGuid) == false) {
			for (i = 0; i < keySubscriberId->specificSystemsSubscribedTo.Size(); i++) {
				RemoveSpecificSubscriber(keySubscriberId->specificSystemsSubscribedTo[i],
										 cloudDataList, remoteCloudClientGuid);
			}
		}
...
void CloudServer::RemoveSpecificSubscriber(RakNetGUID specificSubscriber,
	...
	bool keyDataListExists;
	unsigned int keyDataListIndex =
		cloudDataList->keyData.GetIndexFromKey(specificSubscriber, &keyDataListExists);
	if (keyDataListExists == false) return;
	CloudData *cloudData = cloudDataList->keyData[keyDataListIndex];
	bool hasSpecificSubscriber;
	unsigned int specificSubscriberIndex = cloudData->specificSubscribers.GetIndexFromKey(
