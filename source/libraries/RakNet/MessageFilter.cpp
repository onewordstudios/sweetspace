...
void MessageFilter::SetAllowMessageID(bool allow, int messageIDStart, int messageIDEnd,
									  int filterSetID) {
...
void MessageFilter::SetFilterMaxTime(int allowedTimeMS, bool banOnExceed, RakNet::TimeMS banTimeMS,
									 int filterSetID) {
	FilterSet *filterSet = GetFilterSetByID(filterSetID);
...
void MessageFilter::OnInvalidMessage(FilterSet *filterSet, AddressOrGUID systemAddress,
			...
			if (itemList[index].filter && itemList[index].filter->maxMemberTimeMS > 0 &&
				...
				if (itemList[index].filter->timeoutCallback)
					itemList[index].filter->timeoutCallback(
						rakPeerInterface, keyList[index], itemList[index].filter->filterSetID,
				...
				if (itemList[index].filter->banOnFilterTimeExceed && rakPeerInterface) {
					...
					rakPeerInterface->AddToBanList(str1,
												   itemList[index].filter->timeExceedBanTimeMS);
				}
...
void MessageFilter::OnNewConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
									bool isIncoming) {
	(void)systemAddress;
	(void)rakNetGUID;
	(void)isIncoming;
	...
	switch (packet->data[0]) {
		...
		default:
			...
			if (packet->data[0] == ID_RPC_PLUGIN) {
				...
				if (systemList.ItemAtIndex(index).filter->allowedRPC4.HasData(functionName) ==
					false) {
					OnInvalidMessage(systemList.ItemAtIndex(index).filter, packet, packet->data[0]);
					return RR_STOP_PROCESSING_AND_DEALLOCATE;
				}
