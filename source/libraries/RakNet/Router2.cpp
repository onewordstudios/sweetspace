...
bool Router2::ConnectInternal(RakNetGUID endpointGuid, bool returnConnectionLostOnFailure) {
	...
	if (largestPing < 0) {
		...
		if (debugInterface)
			debugInterface->ShowFailure(
				FormatStringTS(buff, "Router2: ConnectInternal(%I64d) failed at %s:%i\n",
	...
	for (unsigned int i = 0; i < guids.Size(); i++) {
		...
		if (guids[i] != endpointGuid) {
			...
			uint32_t pack_id = rakPeerInterface->Send(&bsOut, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0,
													  crs.guid, false);

			if (debugInterface) {
				char buff[512];
				debugInterface->ShowDiagnostic(
					FormatStringTS(buff, "Router2::ConnectInternal: at %s:%i, pack_id = %d",
			...
			if (debugInterface) {
				...
				debugInterface->ShowDiagnostic(FormatStringTS(
					buff, "Router2::ConnectInternal: at %s:%i [else ..].: %I64d==%I64d", __FILE__,
					__LINE__, guids[i].g, endpointGuid.g));
			}
		}
	}
	connectionRequestsMutex.Lock();
		...
		switch (packet->data[1]) {
			...
			case ID_ROUTER_2_REQUEST_FORWARDING: {
				if (debugInterface) {
					...
					debugInterface->ShowDiagnostic(FormatStringTS(
						buff, "Got ID_ROUTER_2_REQUEST_FORWARDING on ip %s from %I64d, ", buff2,
						packet->guid.g));
				}
		...
		switch (packet->data[1]) {
			case ID_ROUTER_2_REPLY_TO_SENDER_PORT: {
				...
				if (debugInterface) {
					...
					debugInterface->ShowDiagnostic(
						FormatStringTS(buff,
									   "Got ID_ROUTER_2_REPLY_TO_SENDER_PORT %i on address %s, "
...
void Router2::Update(void) {
	...
	while (connectionRequestIndex < connectionRequests.Size()) {
		...
		if (connectionRequest->requestState == R2RS_REQUEST_STATE_QUERY_FORWARDING &&
			...
			while (connectionRequestGuidIndex <
				...
				if (connectionRequest->connectionRequestSystems[connectionRequestGuidIndex]
						.pingToEndpoint < 0) {
					anyRemoved = true;
	...
	while (i < miniPunchesInProgress.Size()) {
		if (miniPunchesInProgress[i].timeout < curTime) {
			SendFailureOnCannotForward(miniPunchesInProgress[i].sourceGuid,
...
void Router2::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
	...
	while (forwardedConnectionIndex < forwardedConnectionList.Size()) {
		if (forwardedConnectionList[forwardedConnectionIndex].endpointGuid == rakNetGUID) {
		...
		} else if (forwardedConnectionList[forwardedConnectionIndex].intermediaryGuid ==
					   rakNetGUID &&
				   forwardedConnectionList[forwardedConnectionIndex].weInitiatedForwarding) {
			// Lost connection to intermediary. Restart process to connect to endpoint. If failed,
			// push ID_CONNECTION_LOST. Also remove connection request if it already is in the list,
			// to restart it
			connectionRequestsMutex.Lock();
			...
			if (debugInterface) {
				char buff[512];
				debugInterface->ShowDiagnostic(FormatStringTS(
					buff, "Closed connection %I64d, restarting forwarding at %s:%i\n", rakNetGUID.g,
					__FILE__, __LINE__));
			}
		...
		if (connectionRequestGuidIndex != (unsigned int)-1) {
				...
				false) // If returns false, no connection request systems left
			{
				if (debugInterface) {
					char buff[512];
					debugInterface->ShowDiagnostic(FormatStringTS(
						buff, "Aborted connection to the %I64d, aborted forwarding at %s:%i\n",
						rakNetGUID.g, __FILE__, __LINE__));
				}

				RemoveConnectionRequest(connectionRequestIndex);
			} else // Else a system in the connection request list dropped. If
				   // cr->requestState==R2RS_REQUEST_STATE_QUERY_FORWARDING then we are still
				   // waiting for other systems to respond.
			{
				if (debugInterface) {
					char buff[512];
					debugInterface->ShowDiagnostic(
						FormatStringTS(buff,
									   "Aborted connection attempt to %I64d, restarting forwarding "
									   "to %I64d at %s:%i\n",
									   rakNetGUID.g, cr->endpointGuid.g, __FILE__, __LINE__));
				}
				//                 if(volatile bool is_my_fix_a_truth = true) { // A system in the
				//                 list of potential systems to try routing to dropped. There is no
	...
	while (forwardedConnectionIndex < forwardedConnectionList.Size()) {
		if (forwardedConnectionList[forwardedConnectionIndex].intermediaryAddress ==
			...
			if (debugInterface) {
				char buff[512];
				debugInterface->ShowDiagnostic(FormatStringTS(
					buff, "Failed connection attempt to forwarded system (%I64d : %s) at %s:%i\n",
...
bool Router2::UpdateForwarding(ConnnectRequest *connectionRequest) {
	...
	if (connectionRequest->connectionRequestSystems.Size() == 0) {
		...
		if (connectionRequest->returnConnectionLostOnFailure) {
			ReturnToUser(ID_CONNECTION_LOST, connectionRequest->endpointGuid,
						 UNASSIGNED_SYSTEM_ADDRESS,
						 true); // This is a connection which was previously established. Rerouting
								// is not possible.
			...
			//             rakPeerInterface->CloseConnection(%I64d)" ,
			//             connectionRequest->endpointGuid.g);
		} else
			ReturnToUser(ID_ROUTER_2_FORWARDING_NO_PATH, connectionRequest->endpointGuid,
						 UNASSIGNED_SYSTEM_ADDRESS, false);

		if (debugInterface) {
			char buff[512];
			debugInterface->ShowDiagnostic(FormatStringTS(
				buff, "Forwarding failed, no remaining systems at %s:%i\n", _FILE_AND_LINE_));
		}
...
void Router2::RequestForwarding(ConnnectRequest *connectionRequest) {
	...
	if (connectionRequest->GetGuidIndex(connectionRequest->lastRequestedForwardingSystem) !=
		(unsigned int)-1) {
		char buff[512];
		if (debugInterface)
			debugInterface->ShowFailure(
	...
	DataStructures::OrderedList<ConnectionRequestSystem, ConnectionRequestSystem,
								ConnectionRequestSystemComp>
		commandList;
	unsigned int connectionRequestGuidIndex;
	connectionRequest->connectionRequestSystemsMutex.Lock();
	for (connectionRequestGuidIndex = 0;
		 connectionRequestGuidIndex < connectionRequest->connectionRequestSystems.Size();
		 connectionRequestGuidIndex++) {
		RakAssert(connectionRequest->connectionRequestSystems[connectionRequestGuidIndex]
					  .pingToEndpoint >= 0);
	...
	if (debugInterface) {
		...
		debugInterface->ShowDiagnostic(
			FormatStringTS(buff,
						   ...
						   "(connectionRequest->lastRequestedForwardingSystem = %I64d, "
						   "connectionRequest->endpointGuid = %I64d) at %s:%i\n",
...
int Router2::ReturnFailureOnCannotForward(RakNetGUID sourceGuid, RakNetGUID endpointGuid) {
	...
	if (udpForwarder == 0 ||
		...
		if (debugInterface)
			debugInterface->ShowFailure(
				FormatStringTS(buff, "Router2 failed (%I64d -> %I64d) at %s:%i\n", sourceGuid.g,
							   endpointGuid.g, __FILE__, __LINE__));
...
void Router2::OnQueryForwardingReply(Packet *packet) {
	...
	if (debugInterface) {
		...
		debugInterface->ShowDiagnostic(FormatStringTS(buff,
													  "Got ID_ROUTER_2_REPLY_FORWARDING on address "
													  "%s(%I64d -> %I64d) canForward=%i at %s:%i\n",
													  buff2, packet->guid.g, endpointGuid.g,
	...
	if (canForward) {
		...
		connectionRequests[connectionRequestIndex]
			->connectionRequestSystems[connectionRequestGuidIndex]
			.pingToEndpoint = rakPeerInterface->GetAveragePing(packet->guid) + pingToEndpoint;
	} else {
		connectionRequests[connectionRequestIndex]->connectionRequestSystems.RemoveAtIndex(
			connectionRequestGuidIndex);
	}
	connectionRequests[connectionRequestIndex]->connectionRequestSystemsMutex.Unlock();
...
void Router2::SendForwardingSuccess(MessageID messageId, RakNetGUID sourceGuid,
	...
	if (debugInterface) {
		...
		if (messageId == ID_ROUTER_2_FORWARDING_ESTABLISHED)
			debugInterface->ShowDiagnostic(FormatStringTS(
				buff, "Sending ID_ROUTER_2_FORWARDING_ESTABLISHED at %s:%i\n", _FILE_AND_LINE_));
		else
			debugInterface->ShowDiagnostic(
				FormatStringTS(buff, "Sending ID_ROUTER_2_REROUTED at %s:%i\n", _FILE_AND_LINE_));
	}
}
void Router2::SendOOBFromRakNetPort(OutOfBandIdentifiers oob, BitStream *extraData,
									SystemAddress sa) {
	...
	rakPeerInterface->SendOutOfBand((const char *)ipAddressString, sa.GetPort(),
									(const char *)oobBs.GetData(), oobBs.GetNumberOfBytesUsed());
}
void Router2::SendOOBFromSpecifiedSocket(OutOfBandIdentifiers oob, SystemAddress sa,
	...
	if (sa.address.addr4.sin_family == AF_INET) {
		sendto__(socket, (const char *)bs.GetData(), bs.GetNumberOfBytesUsed(), 0,
				 (const sockaddr *)&sa.address.addr4, sizeof(sockaddr_in));
	} else {
#if RAKNET_SUPPORT_IPV6 == 1
		sendto__(socket, (const char *)bs.GetData(), bs.GetNumberOfBytesUsed(), 0,
				 (const sockaddr *)&sa.address.addr6, sizeof(sockaddr_in6));
#endif
	}
}
void Router2::SendOOBMessages(Router2::MiniPunchRequest *mpr) {
	...
	// system should reply.
	SendOOBFromSpecifiedSocket(ID_ROUTER_2_REPLY_TO_SENDER_PORT, mpr->sourceAddress,
							   mpr->forwardingSocket);

	// Send from destToSourcePort to endpointSystemAddress (destination). If the message arrives,
	// the remote system should reply.
	SendOOBFromSpecifiedSocket(ID_ROUTER_2_REPLY_TO_SENDER_PORT, mpr->endpointAddress,
							   mpr->forwardingSocket);
	...
	if (debugInterface) {
		...
		debugInterface->ShowDiagnostic(
			FormatStringTS(buff, "call SendOOBFromSpecifiedSocket(...,%s,...)", buff2));
...
void Router2::OnRequestForwarding(Packet *packet) {
	...
	if (pingToEndpoint == -1) {
		...
		if (debugInterface)
			debugInterface->ShowFailure(FormatStringTS(
				buff, "Router2 failed (packet->guid =%I64d, endpointGuid = %I64d) at %s:%i\n",
	...
	UDPForwarderResult result =
		udpForwarder->StartForwarding(packet->systemAddress, endpointSystemAddress, 30000, 0,
	...
	if (result == UDPFORWARDER_FORWARDING_ALREADY_EXISTS) {
		...
		SendForwardingSuccess(ID_ROUTER_2_FORWARDING_ESTABLISHED, packet->guid, endpointGuid,
							  forwardingPort);
		...
		if (debugInterface)
			debugInterface->ShowFailure(
				FormatStringTS(buff,
							   ...
							   "packet->systemAddress=%s, endpointSystemAddress=%s, "
							   "forwardingPort=%i, forwardingSocket=%i\n",
							   __FILE__, __LINE__, buff2, buff3, forwardingPort, forwardingSocket));
		SendFailureOnCannotForward(packet->guid, endpointGuid);
...
void Router2::OnMiniPunchReplyBounce(Packet *packet) {
	...
	if (debugInterface) {
		...
		debugInterface->ShowDiagnostic(
			FormatStringTS(buff,
						   "Got ID_ROUTER_2_MINI_PUNCH_REPLY_BOUNCE from guid=%I64d "
						   "(miniPunchesInProgress.Size() = %d)",
		...
		if (miniPunchesInProgress[i].sourceGuid == packet->guid ||
			...
			if (miniPunchesInProgress[i].gotReplyFromEndpoint == true &&
				...
				SendForwardingSuccess(
					ID_ROUTER_2_FORWARDING_ESTABLISHED, miniPunchesInProgress[i].sourceGuid,
...
void Router2::OnMiniPunchReply(Packet *packet) {
	...
	if (debugInterface) {
		...
		debugInterface->ShowDiagnostic(
			FormatStringTS(buff, "Sending ID_ROUTER_2_MINI_PUNCH_REPLY_BOUNCE (%s) at %s:%i\n",
...
bool Router2::OnForwardingSuccess(Packet *packet) {
	...
	if (forwardingIndex < forwardedConnectionList.Size()) {
		...
		if (debugInterface) {
			...
			debugInterface->ShowDiagnostic(FormatStringTS(
				buff,
				"Got ID_ROUTER_2_FORWARDING_ESTABLISHED, returning ID_ROUTER_2_REROUTED, Calling "
				"RakPeer::ChangeSystemAddress at %s:%i\n",
				_FILE_AND_LINE_));
		}
...
void Router2::ReturnToUser(MessageID messageId, RakNetGUID endpointGuid,
