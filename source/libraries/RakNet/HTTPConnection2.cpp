...
HTTPConnection2::~HTTPConnection2() {}
bool HTTPConnection2::TransmitRequest(const char *stringToTransmit, const char *host,
									  unsigned short port, bool useSSL, int ipVersion,
									  SystemAddress useAddress, void *userData) {
...
bool HTTPConnection2::GetResponse(RakString &stringTransmitted, RakString &hostTransmitted,
								  RakString &responseReceived, SystemAddress &hostReceived,
								  int &contentOffset) {
	void *userData;
	return GetResponse(stringTransmitted, hostTransmitted, responseReceived, hostReceived,
					   contentOffset, &userData);
}
bool HTTPConnection2::GetResponse(RakString &stringTransmitted, RakString &hostTransmitted,
								  RakString &responseReceived, SystemAddress &hostReceived,
...
void ReadChunkBlock(size_t &currentChunkSize, size_t &bytesReadSoFar, char *txtIn,
					RakString &txtOut) {
	size_t bytesToRead;
	size_t sLen;
			...
			*/
			...
			if (isFirstChunk) {
				...
				if (sentRequest->thisChunkSize == 0) {
					...
					body_header += 2;
					ReadChunkBlock(sentRequest->thisChunkSize, sentRequest->bytesReadForThisChunk,
								   body_header, sentRequest->stringReceived);
				...
				ReadChunkBlock(sentRequest->thisChunkSize, sentRequest->bytesReadForThisChunk,
							   (char *)packet->data, sentRequest->stringReceived);
				...
				if (sentRequest->contentLength == -1) {
					const char *length_header =
						strstr(sentRequest->stringReceived.C_String(), "Content-Length: ");
					if (length_header) {
						...
						for (clLength = 0;
							...
							;
						if (clLength > 0 &&
							(length_header[clLength] == '\r' || length_header[clLength] == '\n')) {
...
void HTTPConnection2::OnNewConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
									  bool isIncoming) {
	(void)rakNetGUID;
...
void HTTPConnection2::SendNextPendingRequest(void) {
	...
	if (pendingRequests.Size() > 0) {
		...
		if (pendingRequest->ipVersion != 6) {
			tcpInterface->Connect(pendingRequest->host.C_String(), pendingRequest->port, false,
								  AF_INET);
		} else {
#if RAKNET_SUPPORT_IPV6
			tcpInterface->Connect(pendingRequest->host.C_String(), pendingRequest->port, false,
...
void HTTPConnection2::SendRequest(Request *request) {
	tcpInterface->Send(request->stringToTransmit.C_String(),
