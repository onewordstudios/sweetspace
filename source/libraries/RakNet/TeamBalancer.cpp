			...
			if (teamMembers[droppedMemberIndex].memberGuid == rakNetGUID) {
				...
				if (lockTeams == false) {
					if (forceTeamsToBeEven) {
						...
						// - teams were NOT forced to be even, and the team the dropped player on
						// was full, then move users wanting to join that team (if any)
		...
		if (memberIndex == (unsigned int)-1) {
			...
			if (tm.currentTeam == UNASSIGNED_TEAM_ID) {
				...
				if (tm.requestedTeam == UNASSIGNED_TEAM_ID) {
					...
					if (TeamWouldBeOverpopulatedOnAddition(tm.requestedTeam, teamMembers.Size()) ==
						false) {
...
void TeamBalancer::OnRequestSpecificTeam(Packet *packet) {
	...
	if (memberIndex == (unsigned int)-1) {
		...
		if (TeamsWouldBeEvenOnSwitch(tm.requestedTeam, oldTeamThisUserWasOn) == true) {
			...
			// If someone wants to join this user's old team, and we want to join their team, they
			// can swap
...
void TeamBalancer::EvenTeams(void) {
	...
	for (teamMemberCountsIndex = 0; teamMemberCountsIndex < teamMemberCounts.Size();
		...
		while (teamMemberCounts[teamMemberCountsIndex] < minMembersOnASingleTeam &&
			...
			memberIndexToSwitch =
				GetMemberIndexToSwitchTeams(overpopulatedTeams, teamMemberCountsIndex);
...
unsigned int TeamBalancer::GetMemberIndexToSwitchTeams(
	const DataStructures::List<TeamId> &sourceTeamNumbers, TeamId targetTeamNumber) {
...
TeamId TeamBalancer::MoveMemberThatWantsToJoinTeamInternal(TeamId teamId) {
	...
	if (membersThatWantToJoinTheTeam.Size() > 0) {
		...
		unsigned int swappedMemberIndex =
			membersThatWantToJoinTheTeam[randomMT() % membersThatWantToJoinTheTeam.Size()];
