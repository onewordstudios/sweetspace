...
void TableSerializer::SerializeColumns(DataStructures::Table *in, RakNet::BitStream *out) {
	...
	for (i = 0; i < columns.Size(); i++) {
		StringCompressor::Instance()->EncodeString(columns[i].columnName,
												   _TABLE_MAX_COLUMN_NAME_LENGTH, out);
...
bool TableSerializer::DeserializeCell(RakNet::BitStream *in, DataStructures::Table::Cell *cell,
	...
	if (isEmpty == false) {
		if (columnType == DataStructures::Table::NUMERIC) {
			...
			cell->Set((char *)in->GetData() + BITS_TO_BYTES(in->GetReadOffset()),
					  (int)binaryLength);
...
void TableSerializer::SerializeFilterQuery(RakNet::BitStream *in,
	...
	if (query->cellValue->isEmpty == false) {
		...
		in->WriteAlignedBytesSafe((const char *)query->cellValue->c,
...
bool TableSerializer::DeserializeFilterQuery(RakNet::BitStream *out,
	...
	StringCompressor::Instance()->DecodeString(query->columnName, _TABLE_MAX_COLUMN_NAME_LENGTH,
											   out, 0);
	...
	if (query->cellValue->isEmpty == false) {
		// HACK - cellValue->i is used for integer, character, and binary data. However, for
		// character and binary c will be 0. So use that to determine if the data was integer or
		// not.
