...
void NatPunchthroughClient::Update(void) {
	...
	if (sp.nextActionTime && sp.nextActionTime < time) {
		...
		{
			// Send to unused port. We do not want the message to arrive, just to open our router's
		table SystemAddress sa=sp.targetAddress; int ttlSendIndex; for (ttlSendIndex=0; ttlSendIndex
		<= pc.MAX_PREDICTIVE_PORT_RANGE; ttlSendIndex++)
			{
...
void NatPunchthroughClient::OnPunchthroughFailure(void) {
	if (pc.retryOnFailure == false) {
		if (natPunchthroughDebugInterface) {
			...
			sp.targetGuid.ToString(guidString);
			natPunchthroughDebugInterface->OnClientMessage(
				RakNet::RakString("Failed punchthrough once. Returning failure to guid %s, system "
								  "address %s to user.",
								  guidString, ipAddressString));
		}
	...
	for (i = 0; i < failedAttemptList.Size(); i++) {
		if (failedAttemptList[i].guid == sp.targetGuid) {
			if (natPunchthroughDebugInterface) {
				...
				sp.targetGuid.ToString(guidString);
				natPunchthroughDebugInterface->OnClientMessage(
					RakNet::RakString("Failed punchthrough twice. Returning failure to guid %s, "
									  "system address %s to user.",
									  guidString, ipAddressString));
			}
	...
	if (rakPeerInterface->GetConnectionState(sp.facilitator) != IS_CONNECTED) {
		if (natPunchthroughDebugInterface) {
			...
			sp.targetGuid.ToString(guidString);
			natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
				"Not connected to facilitator, so cannot retry punchthrough after first failure. "
	...
	if (natPunchthroughDebugInterface) {
		...
		sp.targetGuid.ToString(guidString);
		natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
			"First punchthrough failure on guid %s, system address %s. Reattempting.", guidString,
			ipAddressString));
	}
	...
	switch (packet->data[0]) {
			...
			if (packet->length >= 2 && packet->data[1] == ID_NAT_PONG) {
				...
				if (natPunchthroughDebugInterface)
					natPunchthroughDebugInterface->OnClientMessage(RakString(
						...
						portStride, externalPort, externalPort2));
				...
				// sp.targetGuid==packet->guid is because the internal IP addresses reported may
				// include loopbacks not reported by RakPeer::IsLocalIP()
				if (packet->data[1] == ID_NAT_ESTABLISH_UNIDIRECTIONAL &&
					...
					if (sp.testMode != SendPing::PUNCHING_FIXED_PORT) {
						...
						// Keeps trying until the other side gives up too, in case it is
						// unidirectional
						sp.punchingFixedPortAttempts =
							pc.UDP_SENDS_PER_PORT_EXTERNAL * (pc.MAX_PREDICTIVE_PORT_RANGE + 1);

						if (natPunchthroughDebugInterface) {
							char guidString[128];
							sp.targetGuid.ToString(guidString);
							natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
								"PUNCHING_FIXED_PORT: Received ID_NAT_ESTABLISH_UNIDIRECTIONAL "
						...
						if (natPunchthroughDebugInterface) {
							...
							sp.targetGuid.ToString(guidString);
							natPunchthroughDebugInterface->OnClientMessage(
								RakNet::RakString("Received ID_NAT_ESTABLISH_UNIDIRECTIONAL from "
												  "guid %s, system address %s.",
												  guidString, ipAddressString));
						}
					}
					...
					if (mostRecentExternalPort == 0) {
						...
						if (natPunchthroughDebugInterface) {
							natPunchthroughDebugInterface->OnClientMessage(
								RakNet::RakString("ID_NAT_ESTABLISH_BIDIRECTIONAL "
												  "mostRecentExternalPort first time set to %i",
						...
						if (sp.testMode != SendPing::TESTING_INTERNAL_IPS &&
							...
							if (hasPortStride != HAS_PORT_STRIDE) {
								...
								if (natPunchthroughDebugInterface) {
									natPunchthroughDebugInterface->OnClientMessage(RakString(
										"ID_NAT_ESTABLISH_BIDIRECTIONAL: Estimated port stride "
										"from incoming connection at %i. ourExternalPort=%i "
										"mostRecentExternalPort=%i",
										portStride, ourExternalPort, mostRecentExternalPort));
								}
					...
					sp.targetAddress = packet->systemAddress;
					PushSuccess();
					// UpdateGroupPunchOnNatResult(sp.facilitator, sp.targetGuid, sp.targetAddress,
					// 1);
					OnReadyForNextPunchthrough();
					...
					if (natPunchthroughDebugInterface) {
						...
						if (removedFromFailureQueue)
							natPunchthroughDebugInterface->OnClientMessage(
								RakNet::RakString("Punchthrough to guid %s, system address %s "
												  "succeeded on 2nd attempt.",
												  guidString, ipAddressString));
						else
							natPunchthroughDebugInterface->OnClientMessage(
								RakNet::RakString("Punchthrough to guid %s, system address %s "
												  "succeeded on 1st attempt.",
												  guidString, ipAddressString));
					}
				}
		...
		case ID_NAT_TARGET_UNRESPONSIVE: {
			...
			for (i = 0; i < failedAttemptList.Size(); i++) {
				if (failedAttemptList[i].guid == targetGuid) {
					if (natPunchthroughDebugInterface) {
						...
						targetGuid.ToString(guidString);
						natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
							"Punchthrough retry to guid %s failed due to %s.", guidString, reason));
					}

					// If the retry target is not connected, or loses connection, or is not
					// responsive, then previous failures cannot be retried.
			...
			if (natPunchthroughDebugInterface) {
				...
				targetGuid.ToString(guidString);
				natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
					"Punchthrough attempt to guid %s failed due to %s.", guidString, reason));
			}
...
void NatPunchthroughClient::SendOutOfBand(SystemAddress sa, MessageID oobId) {
	...
	rakPeerInterface->SendOutOfBand((const char *)ipAddressString, sa.GetPort(),
									(const char *)oob.GetData(), oob.GetNumberOfBytesUsed());
...
void NatPunchthroughClient::OnNewConnection(const SystemAddress &systemAddress,
	...
	if (ourExternalId != UNASSIGNED_SYSTEM_ADDRESS && mostRecentExternalPort == 0) {
		...
		if (natPunchthroughDebugInterface) {
			natPunchthroughDebugInterface->OnClientMessage(
				RakNet::RakString("OnNewConnection mostRecentExternalPort first time set to %i",
								  mostRecentExternalPort));
		}
	}
...
void NatPunchthroughClient::OnClosedConnection(const SystemAddress &systemAddress,
...
void NatPunchthroughClient::OnGetMostRecentPort(Packet *packet) {
	...
	if (mostRecentExternalPort == 0) {
		...
		if (natPunchthroughDebugInterface) {
			natPunchthroughDebugInterface->OnClientMessage(
				RakNet::RakString("OnGetMostRecentPort mostRecentExternalPort first time set to %i",
								  mostRecentExternalPort));
		}
	}
	...
	rakPeerInterface->Send(&outgoingBs, HIGH_PRIORITY, RELIABLE_ORDERED, 0, packet->systemAddress,
						   false);
