...
void BitStream::Reset(void) {
	...
	// free(data);  // Use realloc and free so we are more efficient than delete and new for
	// resizing
	numberOfBitsUsed = 0;
...
void BitStream::WriteAlignedBytesSafe(const char *inByteArray, const unsigned int inputLength,
	...
	WriteAlignedBytes((const unsigned char *)inByteArray,
...
bool BitStream::ReadAlignedBytesSafe(char *inOutByteArray, int &inputLength,
									 const int maxBytesToRead) {
	return ReadAlignedBytesSafe(inOutByteArray, (unsigned int &)inputLength,
...
bool BitStream::ReadAlignedBytesSafeAlloc(char **outByteArray, int &inputLength,
										  const unsigned int maxBytesToRead) {
...
void BitStream::WriteBits(const unsigned char *inByteArray, BitSize_t numberOfBitsToWrite,
	...
	// do
	{
		...
		if (numberOfBitsToWrite < 8 &&
			rightAlignedBits) // rightAlignedBits means in the case of a partial byte, the bits are
							  // aligned from the right (bit 0) rather than the left (as in the
							  // normal internal representation)
			dataByte <<= 8 - numberOfBitsToWrite; // shift left to get the bits on the left, as in
			...
			*(data + (numberOfBitsUsed >> 3)) |= dataByte >> (numberOfBitsUsedMod8); // First half
			...
			if (8 - (numberOfBitsUsedMod8) < 8 &&
				8 - (numberOfBitsUsedMod8) <
					numberOfBitsToWrite) // If we didn't write it all out in the first half (8 -
										 // (numberOfBitsUsed%8) is the number we wrote in the first
										 // half)
			{
				*(data + (numberOfBitsUsed >> 3) + 1) =
					(unsigned char)(dataByte
									<< (8 - (numberOfBitsUsedMod8))); // Second half (overlaps byte
																	  // boundary)
			}
		}
...
void BitStream::WriteCompressed(const unsigned char *inByteArray, const unsigned int size,
...
bool BitStream::ReadBits(unsigned char *inOutByteArray, BitSize_t numberOfBitsToRead,
	...
	while (numberOfBitsToRead > 0) {
		*(inOutByteArray + offset) |= *(data + (readOffset >> 3)) << (readOffsetMod8); // First half
		...
		if (readOffsetMod8 > 0 &&
			numberOfBitsToRead > 8 - (readOffsetMod8)) // If we have a second half, we didn't read
...
bool BitStream::ReadCompressed(unsigned char *inOutByteArray, const unsigned int size,
							   const bool unsignedData) {
	...
	// or 16 (negative) then what we read will be a 1 and the remaining 4 bits. Otherwise we read a
	// 0 and the 8 bytes
	// RakAssert(readOffset+1 <=numberOfBitsUsed); // If this assert is hit the stream wasn't long
	// enough to read from
...
void BitStream::AddBitsAndReallocate(const BitSize_t numberOfBitsToWrite) {
	...
	{
#ifdef _DEBUG
		// If this assert hits then we need to specify true for the third parameter in the
		// constructor It needs to reallocate to hold all the data and can't do it unless we
		// allocated to begin with Often hits if you call Write or Serialize on a read-only
		// bitstream
		...
		if (data == (unsigned char *)stackData) {
			if (amountToAllocate > BITSTREAM_STACK_ALLOCATION_SIZE) {
				...
				memcpy((void *)data, (void *)stackData,
...
void BitStream::PrintBits(char *out) const {
	...
	for (BitSize_t counter = 0; counter < BITS_TO_BYTES(numberOfBitsUsed) && strIndex < 2000;
		 counter++) {
		BitSize_t stop;
...
BitSize_t BitStream::CopyData(unsigned char **_data) const {
	...
	*_data =
		(unsigned char *)rakMalloc_Ex((size_t)BITS_TO_BYTES(numberOfBitsUsed), _FILE_AND_LINE_);
...
void BitStream::ReverseBytes(unsigned char *inByteArray, unsigned char *inOutByteArray,
...
bool BitStream::SerializeFloat16(bool writeToBitstream, float &inOutFloat, float floatMin,
								 float floatMax) {
	if (writeToBitstream)
