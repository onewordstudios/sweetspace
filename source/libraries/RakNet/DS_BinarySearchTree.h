...
class RAK_DLL_EXPORT BinarySearchTree {
   ...
   protected:
	...
	void DisplayPostorderRecursive(node* current, BinarySearchTreeType* return_array,
								   unsigned int& index);
...
class RAK_DLL_EXPORT AVLBalancedBinarySearchTree : public BinarySearchTree<BinarySearchTreeType> {
   ...
   private:
	void BalanceTree(typename BinarySearchTree<BinarySearchTreeType>::node* current,
					 bool rotateOnce);
...
template <class BinarySearchTreeType>
typename BinarySearchTree<BinarySearchTreeType>::node*&
BinarySearchTree<BinarySearchTreeType>::FindParent(const BinarySearchTreeType& element) {
...
typename BinarySearchTree<BinarySearchTreeType>::node* BinarySearchTree<BinarySearchTreeType>::Del(
	...
	if ((current->right) == 0 && (current->left) == 0) // Leaf node, just remove it
	{
		...
		return parent;
			   ...
			   (current->left) == 0) // Node has only one child, delete it and cause the parent to
									 // point to that child
	{
		if (parent) {
			if (direction == RIGHT)
...
void BinarySearchTree<BinarySearchTreeType>::DisplayPostorderRecursive(
	typename BinarySearchTree::node* current, BinarySearchTreeType* return_array,
	unsigned int& index) {
