...
class RAK_DLL_EXPORT UDPProxyServer : public PluginInterface2 {
   public:
	...
	/// SetServerPublicIP() to tell the client what address to connect to \param[in] ip IP address
	/// to report in UDPProxyClientResultHandler::OnForwardingSuccess() and
	...
	/// the default UDPForwarder::Startup(), UDPForwarder::Shutdown(), and UDPForwarder::Update()
	/// are called automatically by the plugin
