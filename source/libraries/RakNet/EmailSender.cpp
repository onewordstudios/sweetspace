...
const char *EmailSender::Send(const char *hostAddress, unsigned short hostPort, const char *sender,
							  const char *recipient, const char *senderName,
...
#pragma warning(disable : 4127) // conditional expression is constant
	...
	for (i = 1, j = 1; i < bodyLength; i++) {
		...
		// do not count \n alone but it does) In fact having to process .. is a bug too - because
	...
	if (attachedFiles && attachedFiles->fileList.Size()) {
		for (i = 0; i < (int)attachedFiles->fileList.Size(); i++) {
			...
			newBody = (char *)rakMalloc_Ex(
				(size_t)(attachedFiles->fileList[i].dataLengthBytes * 3) / 2, _FILE_AND_LINE_);
...
const char *EmailSender::GetResponse(TCPInterface *tcpInterface, const SystemAddress &emailServer,
									 bool doPrintf) {
	RakNet::Packet *packet;
