...
bool RakNet::CanConnect(NATTypeDetectionResult type1, NATTypeDetectionResult type2) {
	...
	/// If one system is NAT_TYPE_PORT_RESTRICTED, the other must be NAT_TYPE_PORT_RESTRICTED or
	/// less
	bool connectionGraph[NAT_TYPE_COUNT][NAT_TYPE_COUNT] = {
		// None,	Full Cone,	Address Restricted,		Port Restricted,	Symmetric,	Unknown,	InProgress,
		// Supports_UPNP
...
RakNetSocket2 *RakNet::CreateNonblockingBoundSocket(const char *bindAddr
	...
	chromeInstance ); #else RakNetSocket2 *s = SocketLayer::CreateBoundSocket( 0, 0, false,
	bindAddr, true, 0, AF_INET, 0 ); #endif

	#ifdef _WIN32
		unsigned long nonblocking = 1;
		s->IOCTLSocket( FIONBIO, &nonblocking );
	#elif defined(_PS3) || defined(__PS3__) || defined(SN_TARGET_PS3) || defined(_PS4) ||
	defined(SN_TARGET_PSP2) int sock_opt=1; s->SetSockOpt(SOL_SOCKET, SO_NBIO, ( char * ) &
	sock_opt, sizeof ( sock_opt ) ); #elif defined(__native_client__)
		// Nop
	#else
