...
class RAK_DLL_EXPORT TCPInterface {
   public:
	...
	/// Sends a byte stream
	virtual void Send(const char *data, unsigned int length, const SystemAddress &systemAddress,
					  bool broadcast);
...
#else
	bool CreateListenSocket(unsigned short port, unsigned short maxIncomingConnections,
							unsigned short socketFamily, const char *hostAddress);
#endif
	...
	*/
	...
	__TCPSOCKET__ SocketConnect(const char *host, unsigned short remotePort,
