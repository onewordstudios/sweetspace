...
#ifdef _DEBUG
			// Critical error - got a string index out of range, which means AddString was called
			// more times on the remote system than on this system. All systems must call AddString
			// the same number of types, with the same strings in the same order.
			RakAssert(0);
#endif
			return false;
		}
