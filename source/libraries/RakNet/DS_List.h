...
class RAK_DLL_EXPORT List {
   public:
	...
	void Insert(const list_type &input, const unsigned int position, const char *file,
				unsigned int line);
...
template <class list_type>
void List<list_type>::Insert(const list_type &input, const unsigned int position, const char *file,
							 unsigned int line) {
#ifdef _DEBUG
	if (position > list_size) {
		RakAssert(position <= list_size);
...
template <class list_type>
inline void List<list_type>::Replace(const list_type &input, const list_type filler,
