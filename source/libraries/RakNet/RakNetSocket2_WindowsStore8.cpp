...
ref class ListenerContext sealed {
   ...
   private:
	...
	void EchoMessage(Windows::Networking::Sockets::DatagramSocketMessageReceivedEventArgs ^
					 eventArguments);
};
} // namespace RakNet
...
OutputStreamAndDataWriter ^ ListenerContext::GetOutputStreamAndDataWriter(uint64_t s_addr) {
	...
	task<IOutputStream ^> op(
		listener->GetOutputStreamAsync(hostName, ref new Platform::String(w_char2)));
	op.wait();
...
RNS2SendResult RNS2_WindowsStore8::Send(RNS2_SendParameters *sendParameters, const char *file,
										unsigned int line) {
...
void RNS2_WindowsStore8::DomainNameToIP(const char *domainName, char ip[65]) {
	...
	op->Completed = ref new AsyncOperationCompletedHandler<
		Windows::Foundation::Collections::IVectorView<Windows::Networking::EndpointPair ^> ^>(
		[&](IAsyncOperation<Windows::Foundation::Collections::IVectorView<
...
int RNS2_WindowsStore8::WinRTSetSockOpt(Windows::Networking::Sockets::DatagramSocket ^ s, int level,
