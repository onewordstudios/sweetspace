...
int StatisticsHistory::TrackedObjectComp(const uint64_t &key,
...
StatisticsHistory::TrackedObjectData::TrackedObjectData(uint64_t _objectId, int _objectType,
														void *_userData) {
	objectId = _objectId;
...
bool StatisticsHistory::AddValueByObjectID(uint64_t objectId, RakString key, SHValueType val,
										   Time curTime, bool combineEqualTimes) {
	unsigned int idx = GetObjectIndex(objectId);
	if (idx == (unsigned int)-1) return false;
...
StatisticsHistory::SHErrorCode StatisticsHistory::GetHistoryForKey(
	uint64_t objectId, RakString key, StatisticsHistory::TimeAndValueQueue **values,
	Time curTime) const {
...
bool StatisticsHistory::GetHistorySorted(
	uint64_t objectId, SHSortOperation sortType,
	...
	DataStructures::OrderedList<TimeAndValueQueue *, TimeAndValueQueue *, TimeAndValueQueueCompAsc>
		sortedQueues;
	for (unsigned int i = 0; i < itemList.Size(); i++) {
		...
		else if (sortType == SH_SORT_BY_LONG_TERM_SUM_ASCENDING ||
		...
		else if (sortType == SH_SORT_BY_RECENT_AVERAGE_ASCENDING ||
		...
		else if (sortType == SH_SORT_BY_RECENT_HIGHEST_ASCENDING ||
				 sortType == SH_SORT_BY_RECENT_HIGHEST_DESCENDING)
			tavq->sortValue = tavq->GetRecentHighest();
		else if (sortType == SH_SORT_BY_RECENT_LOWEST_ASCENDING ||
...
void StatisticsHistory::MergeAllObjectsOnKey(RakString key, TimeAndValueQueue *tavqOutput,
	...
	for (unsigned int idx = 0; idx < objects.Size(); idx++) {
		...
		if (hi.IsInvalid() == false) {
			...
			TimeAndValueQueue::MergeSets(tavqOutput, dataCategory, tavqInput, dataCategory,
										 tavqOutput);
		}
	}
}
...
void StatisticsHistory::TimeAndValueQueue::MergeSets(const TimeAndValueQueue *lhs,
	...
	if (lhsDataCategory == StatisticsHistory::DC_DISCRETE &&
		...
		while (rhsIndex < rhs->values.Size() && lhsIndex < lhs->values.Size()) {
			if (rhs->values[rhsIndex].time < lhs->values[lhsIndex].time) {
				...
				newTimeAndValue.val = rhs->values[rhsIndex].val + lastTimeAndValueLhs.val +
									  lastSlopeLhs * timeSinceOppositeValue;
				...
				newTimeAndValue.val = lhs->values[lhsIndex].val + lastTimeAndValueRhs.val +
									  lastSlopeRhs * timeSinceOppositeValue;
...
StatisticsHistory::TimeAndValueQueue &StatisticsHistory::TimeAndValueQueue::operator=(
...
StatisticsHistoryPlugin::~StatisticsHistoryPlugin() {}
void StatisticsHistoryPlugin::SetTrackConnections(bool _addNewConnections,
...
*/
void StatisticsHistoryPlugin::OnClosedConnection(const SystemAddress &systemAddress,
...
void StatisticsHistoryPlugin::OnNewConnection(const SystemAddress &systemAddress,
	...
	if (addNewConnections) {
		statistics.AddObject(
			StatisticsHistory::TrackedObjectData(rakNetGUID.g, newConnectionsObjectType, 0));
	}
}
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
