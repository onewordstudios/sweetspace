...
class RAK_DLL_EXPORT PluginInterface2 {
   public:
	...
	/// \param[in] frameNumber The number of frames sent or received so far for this player
	/// depending on \a isSend .  Indicates the frame of this user message. \param[in]
	/// remoteSystemAddress The player we sent or got this packet from \param[in] time The current
	/// time as returned by RakNet::GetTimeMS() \param[in] isSend Is this callback representing a
	/// send event or receive event?
   ...
   protected:
	...
	void SendUnified(const char *data, const int length, PacketPriority priority,
					 PacketReliability reliability, char orderingChannel,
					 const AddressOrGUID systemIdentifier, bool broadcast);
	bool SendListUnified(const char **data, const int *lengths, const int numParameters,
