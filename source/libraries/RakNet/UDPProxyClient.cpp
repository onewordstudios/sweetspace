...
bool UDPProxyClient::RequestForwarding(SystemAddress proxyCoordinator, SystemAddress sourceAddress,
	...
	rakPeerInterface->Send(&outgoingBs, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0, proxyCoordinator,
						   false);
...
void UDPProxyClient::Update(void) {
	...
	while (idx1 < pingServerGroups.Size()) {
		...
		if (psg->serversToPing.Size() > 0 &&
			...
			// If they didn't reply within DEFAULT_UNRESPONSIVE_PING_TIME_COORDINATOR, just give up
			// on them
		...
		switch (packet->data[1]) {
			...
			case ID_UDP_PROXY_FORWARDING_NOTIFICATION: {
				...
				switch (packet->data[1]) {
						...
						if (packet->data[1] == ID_UDP_PROXY_FORWARDING_SUCCEEDED) {
							if (resultHandler)
								resultHandler->OnForwardingSuccess(
									serverIP.C_String(), forwardingPort, packet->systemAddress,
							...
							// Send a datagram to the proxy, so if we are behind a router, that
							// router adds an entry to the routing table. Otherwise the router would
							// block the incoming datagrams from source It doesn't matter if the
							// message actually arrives as long as it goes through the router
					...
					case ID_UDP_PROXY_ALL_SERVERS_BUSY:
						if (resultHandler)
							resultHandler->OnAllServersBusy(packet->systemAddress, senderAddress,
															targetAddress, targetGuid, this);
						break;
					case ID_UDP_PROXY_NO_SERVERS_ONLINE:
						if (resultHandler)
							resultHandler->OnNoServersOnline(packet->systemAddress, senderAddress,
															 targetAddress, targetGuid, this);
						break;
					case ID_UDP_PROXY_RECIPIENT_GUID_NOT_CONNECTED_TO_COORDINATOR: {
						if (resultHandler)
							resultHandler->OnRecipientNotConnected(packet->systemAddress,
...
void UDPProxyClient::PingServerGroup::SendPingedServersToCoordinator(
	...
	rakPeerInterface->Send(&outgoingBs, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0,
						   coordinatorAddressForPings, false);
}
