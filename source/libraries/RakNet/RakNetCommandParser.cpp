...
RakNetCommandParser::RakNetCommandParser() {
	...
	RegisterCommand(3, "CloseConnection",
					"( const SystemAddress target, bool sendDisconnectionNotification, unsigned "
	...
	RegisterCommand(2, "SetOfflinePingResponse",
	...
	RegisterCommand(2, "SetIncomingPassword",
...
bool RakNetCommandParser::OnCommand(const char *command, unsigned numParameters,
	...
	if (strcmp(command, "Startup") == 0) {
		RakNet::SocketDescriptor socketDescriptor((unsigned short)atoi(parameterList[1]),
												  parameterList[2]);
		ReturnResult(peer->Startup((unsigned short)atoi(parameterList[0]), &socketDescriptor, 1),
					 command, transport, systemAddress);
	} else if (strcmp(command, "InitializeSecurity") == 0) {
		ReturnResult(peer->InitializeSecurity(parameterList[0], parameterList[1]), command,
					 transport, systemAddress);
		...
		ReturnResult(peer->IsInSecurityExceptionList(parameterList[1]), command, transport,
					 systemAddress);
		...
		ReturnResult(peer->Connect(parameterList[0], (unsigned short)atoi(parameterList[1]),
								   parameterList[2],
		...
		ReturnResult((int)peer->GetConnectionState(SystemAddress(parameterList[0])), command,
					 transport, systemAddress);
	} else if (strcmp(command, "GetIndexFromSystemAddress") == 0) {
		ReturnResult(peer->GetIndexFromSystemAddress(SystemAddress(parameterList[0])), command,
					 transport, systemAddress);
	} else if (strcmp(command, "GetSystemAddressFromIndex") == 0) {
		ReturnResult(peer->GetSystemAddressFromIndex(atoi(parameterList[0])), command, transport,
					 systemAddress);
		...
		peer->Ping(parameterList[0], (unsigned short)atoi(parameterList[1]),
				   atoi(parameterList[2]) != 0);
		ReturnResult(command, transport, systemAddress);
	} else if (strcmp(command, "GetAveragePing") == 0) {
		ReturnResult(peer->GetAveragePing(SystemAddress(parameterList[0])), command, transport,
					 systemAddress);
	} else if (strcmp(command, "GetLastPing") == 0) {
		ReturnResult(peer->GetLastPing(SystemAddress(parameterList[0])), command, transport,
					 systemAddress);
	} else if (strcmp(command, "GetLowestPing") == 0) {
		ReturnResult(peer->GetLowestPing(SystemAddress(parameterList[0])), command, transport,
					 systemAddress);
		...
		ReturnResult(peer->GetExternalID(SystemAddress(parameterList[0])), command, transport,
					 systemAddress);
	...
	{
		ReturnResult(peer->SetMTUSize(atoi(parameterList[0]), UNASSIGNED_SYSTEM_ADDRESS), command,
	transport, systemAddress);
	}
	*/
	else if (strcmp(command, "GetMTUSize") == 0) {
		ReturnResult(peer->GetMTUSize(UNASSIGNED_SYSTEM_ADDRESS), command, transport,
					 systemAddress);
		...
		ReturnResult((char *)peer->GetLocalIP(atoi(parameterList[0])), command, transport,
					 systemAddress);
		...
		peer->AdvertiseSystem(parameterList[0], (unsigned short)atoi(parameterList[1]),
							  parameterList[2], atoi(parameterList[3]));
		ReturnResult(command, transport, systemAddress);
