...
void FileList::AddFile(const char *filename, const char *fullPathToFile, const char *data,
					   const unsigned dataLength, const unsigned fileLength,
...
void FileList::AddFilesFromDirectory(const char *applicationDirectory, const char *subDirectory,
		...
		do {
			...
			if ((fileInfo.attrib & (_A_HIDDEN | _A_SUBDIR | _A_SYSTEM)) == 0) {
				...
				for (unsigned int flpcIndex = 0; flpcIndex < fileListProgressCallbacks.Size();
					 flpcIndex++)
					fileListProgressCallbacks[flpcIndex]->OnFile(this, dirSoFar, fileInfo.name,
																 fileInfo.size);
				...
				if (writeData && writeHash) {
					...
					if (fp) {
						fileData =
							(char *)rakMalloc_Ex(fileInfo.size + HASH_LENGTH, _FILE_AND_LINE_);
						RakAssert(fileData);
					...
					// File data only
					AddFile(fullPath + rootLen, fullPath, fileData, fileInfo.size, fileInfo.size,
							context);
				} else {
					// Just the filename
...
void FileList::Serialize(RakNet::BitStream *outBitStream) {
	...
	for (i = 0; i < fileList.Size(); i++) {
		...
		StringCompressor::Instance()->EncodeString(fileList[i].filename.C_String(),
												   MAX_FILENAME_LENGTH, outBitStream);
...
bool FileList::Deserialize(RakNet::BitStream *inBitStream) {
	...
	for (i = 0; i < fileListSize; i++) {
		...
		StringCompressor::Instance()->DecodeString((char *)filename, MAX_FILENAME_LENGTH,
												   inBitStream);
...
void FileList::GetDeltaToCurrent(FileList *input, FileList *output, const char *dirSubset,
	...
	for (thisIndex = 0; thisIndex < fileList.Size(); thisIndex++) {
		...
		for (inputIndex = 0; inputIndex < input->fileList.Size(); inputIndex++) {
			...
			if (_stricmp(input->fileList[inputIndex].filename.C_String() + remoteSubdirLen,
				...
				if (input->fileList[inputIndex].fileLengthBytes ==
					...
					output->AddFile(fileList[thisIndex].filename,
									fileList[thisIndex].fullPathToFile, 0, 0,
									fileList[thisIndex].fileLengthBytes,
		...
		if (match == false) {
			...
			output->AddFile(fileList[thisIndex].filename, fileList[thisIndex].fullPathToFile, 0, 0,
							fileList[thisIndex].fileLengthBytes, FileListNodeContext(0, 0, 0, 0),
							false);
		}
	}
}
void FileList::ListMissingOrChangedFiles(const char *applicationDirectory,
	...
	for (i = 0; i < fileList.Size(); i++) {
		...
		fp = fopen(fullPath, "rb");
		if (fp == 0) {
			missingOrChangedFiles->AddFile(fileList[i].filename, fileList[i].fullPathToFile, 0, 0,
										   0, FileListNodeContext(0, 0, 0, 0), false);
		} else {
			fseek(fp, 0, SEEK_END);
			fileLength = ftell(fp);
			fseek(fp, 0, SEEK_SET);
			...
			if (fileLength != fileList[i].fileLengthBytes && alwaysWriteHash == false) {
				missingOrChangedFiles->AddFile(fileList[i].filename, fileList[i].fullPathToFile, 0,
											   0, fileLength, FileListNodeContext(0, 0, 0, 0),
											   false);
			} else {
				...
				// if (fileLength != fileList[i].fileLength || memcmp( sha1.GetHash(),
				// fileList[i].data, HASH_LENGTH)!=0)
				if (fileLength != fileList[i].fileLengthBytes ||
					...
					if (neverWriteHash == false)
						...
						missingOrChangedFiles->AddFile((const char *)fileList[i].filename,
													   (const char *)fileList[i].fullPathToFile, 0,
													   0, fileLength,
													   FileListNodeContext(0, 0, 0, 0), false);
				}
			}
...
void FileList::WriteDataToDisk(const char *applicationDirectory) {
	...
	for (i = 0; i < fileList.Size(); i++) {
		...
		WriteFileWithDirectories(fullPath, fileList[i].data,
								 (unsigned int)fileList[i].dataLengthBytes);
	}
}
