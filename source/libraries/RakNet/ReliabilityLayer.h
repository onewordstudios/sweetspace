...
class ReliabilityLayer //<ReliabilityLayer>
{
   public:
	...
	/// to deliver a reliable packet Default time is 10,000 or 10 seconds in release and 30,000 or
	/// 30 seconds in debug. \param[in] time Time, in MS
	...
	/// and sequencing of packets. \param[in] makeDataCopy If true \a data will be copied.
	/// Otherwise, only a pointer will be stored. \param[in] MTUSize maximum datagram size
	/// \param[in] currentTime Current time, as per RakNet::GetTimeMS()
	/// \param[in] receipt This number will be returned back with ID_SND_RECEIPT_ACKED or
	/// ID_SND_RECEIPT_LOSS and is only returned with the reliability types that contain RECEIPT in
	/// the name \return True or false for success or failure.
	...
	void Update(RakNetSocket2 *s, SystemAddress &systemAddress, int MTUSize, CCTimeType time,
				...
				DataStructures::List<PluginInterface2 *> &messageHandlerList, RakNetRandom *rnr,
	...
	// Encoded as numMessages[unsigned int], message1BitLength[unsigned int], message1Data
	// (aligned), ...
   ...
   private:
	...
	void SendBitStream(RakNetSocket2 *s, SystemAddress &systemAddress, RakNet::BitStream *bitStream,
	...
	InternalPacket *CreateInternalPacketFromBitStream(RakNet::BitStream *bitStream,
													  CCTimeType time);
	...
	/// Take all split chunks with the specified splitPacketId and try to reconstruct a packet. If
	/// we can, allocate and return it.  Otherwise return 0
	...
	InternalPacket *BuildPacketFromSplitPacketList(SplitPacketChannel *splitPacketChannel,
												   CCTimeType time);
	...
	InternalPacket *CreateInternalPacketCopy(InternalPacket *original, int dataByteOffset,
	...
	void InsertPacketIntoResendList(InternalPacket *internalPacket, CCTimeType time,
									bool firstResend, bool modifyUnacknowledgedBytes);
	...
	// datagramHistory holds a linked list of MessageNumberNode. Each MessageNumberNode refers to
	// one element in resendList which can be cleared on an ack.
	...
	// If the current time - timeResendQueueNonEmpty is greater than a threshold, we are
	// disconnected
	...
	//    sequencedWriteIndex resets to 0 every time orderedWriteIndex increments. It increments by
	//    1 every time a sequenced message is sent.
	// 3. The receiver maintains the next expected value for the orderedWriteIndex, stored in
	// orderedReadIndex.
	// 4. As messages arrive:
	//    If a message has the current ordering index, and is sequenced, and is < the current
	//    highest sequence value, discard If a message has the current ordering index, and is
	//    sequenced, and is >= the current highest sequence value, return immediately If a message
	//    has a greater ordering index, and is sequenced or ordered, buffer it If a message has the
	//    current ordering index, and is ordered, buffer, then push off messages from buffer
	...
	//    next message to be returned does not preserve the ordered index For an empty heap, the
	//    heap weight should start at the lowest value based on the next expected ordering index, to
	...
	/// It stores the time to stop waiting for a particular packet number, where the packet number
	/// is receivedPacketsBaseIndex + the index into the queue If 0, we got got that packet.
	/// Otherwise, the time to give up waiting for that packet. If we get a packet number where
	...
	void SendACKs(RakNetSocket2 *s, SystemAddress &systemAddress, CCTimeType time,
				  RakNetRandom *rnr, BitStream &updateBitStream);
	...
	// Allocate new
	void AllocInternalPacketData(InternalPacket *internalPacket, unsigned int numBytes,
								 bool allowStack, const char *file, unsigned int line);
