...
#if defined(WINDOWS_STORE_RT)
bool TCPInterface::CreateListenSocket_WinStore8(unsigned short port,
...
SystemAddress TCPInterface::Connect(const char *host, unsigned short remotePort, bool block,
	...
	if (block) {
		...
		completedConnectionAttempts.Push(remoteClients[newRemoteClientIndex].systemAddress,
										 _FILE_AND_LINE_);
...
#endif
void TCPInterface::Send(const char *data, unsigned length, const SystemAddress &systemAddress,
						bool broadcast) {
...
void TCPInterface::CloseConnection(SystemAddress systemAddress) {
	...
	unsigned int i;
	for (i = 0; i < messageHandlerList.Size(); i++)
		messageHandlerList[i]->OnClosedConnection(systemAddress, UNASSIGNED_RAKNET_GUID,
												  LCR_CLOSED_BY_USER);
...
SystemAddress TCPInterface::HasLostConnection(void) {
	...
	if (out) {
		...
		unsigned int i;
		for (i = 0; i < messageHandlerList.Size(); i++)
			messageHandlerList[i]->OnClosedConnection(out2, UNASSIGNED_RAKNET_GUID,
...
__TCPSOCKET__ TCPInterface::SocketConnect(const char *host, unsigned short remotePort,
										  unsigned short socketFamily, const char *bindAddress) {
#ifdef __native_client__
	return 0;
#else
...
RAK_THREAD_DECLARATION(RakNet::ConnectionAttemptLoop) {
	...
	__TCPSOCKET__ sockfd =
		tcpInterface->SocketConnect(str1, systemAddress.GetPort(), socketFamily, s->bindAddress);
	if (sockfd == 0) {
...
#pragma warning(disable : 4244) // warning C4127: conditional expression is constant
			...
			selectResult =
				(int)select__(largestDescriptor + 1, &readFD, &writeFD, &exceptionFD, &tv);

			if (selectResult <= 0) break;

			if (sts->listenSocket != 0 && FD_ISSET(sts->listenSocket, &readFD)) {
				newSock =
					accept__(sts->listenSocket, (sockaddr *)&sockAddr, (socklen_t *)&sockAddrSize);

				if (newSock != 0) {
					int newRemoteClientIndex = -1;
					for (newRemoteClientIndex = 0; newRemoteClientIndex < sts->remoteClientsLength;
						 newRemoteClientIndex++) {
						...
						if (sts->remoteClients[newRemoteClientIndex].isActive == false) {
							...
							sts->remoteClients[newRemoteClientIndex].systemAddress.systemIndex =
								newRemoteClientIndex;
#else
						...
						if (FD_ISSET(socketCopy, &readFD)) {
							...
							if (len > 0) {
								...
								incomingMessage->data[len] =
									0; // Null terminate this so we can print it out as regular
									   // strings.  This is different from RakNet which does not do
									   // this.
								// printf("RECV: %s\n",incomingMessage->data);
								/*
								if (1)
								...
								*/
								...
								incomingMessage->deleteData =
									true; // actually means came from SPSC, rather than
										  // AllocatePacket
