...
static const unsigned int MAX_OFFLINE_DATA_LENGTH =
	400; // I set this because I limit ID_CONNECTION_REQUEST to 512 bytes, and the password is
		 // appended to that packet.
...
Packet *RakPeer::AllocPacket(unsigned dataSize, unsigned char *data, const char *file,
							 unsigned int line) {
...
RakPeer::RakPeer() {
	...
	CAT_AUDIT_PRINTF(
		"AUDIT: Initializing RakPeer security flags: using_security = false, server_handshake = "
		"null, cookie_jar = null\n");
	...
	packetAllocationPoolMutex.Lock();
	packetAllocationPool.SetPageSize(sizeof(DataStructures::MemoryPool<Packet>::MemoryWithPage) *
									 32);
	packetAllocationPoolMutex.Unlock();

	remoteSystemIndexPool.SetPageSize(
		sizeof(DataStructures::MemoryPool<RemoteSystemIndex>::MemoryWithPage) * 32);
...
socketDescriptors[i].blockingSocket, addrToBind, socketDescriptors[i].socketFamily ); #endif
	...
	if (maximumNumberOfPeers == 0) {
		...
		// The disconnects are not consistently processed and the process was sloppy and
		// complicated. Allocate 10% extra to handle new connections from players trying to connect
		...
		// _FILE_AND_LINE_ );
		remoteSystemList =
			RakNet::OP_NEW_ARRAY<RemoteSystemStruct>(maximumNumberOfPeers, _FILE_AND_LINE_);
...
#ifdef _DEBUG
			remoteSystemList[i].reliabilityLayer.ApplyNetworkSimulator(_packetloss, _minExtraPing,
																	   _extraPingVariance);
#endif
		...
		for (unsigned int i = 0;
			 i < (unsigned int)maximumNumberOfPeers * REMOTE_SYSTEM_LOOKUP_HASH_MULTIPLE; i++) {
...
bool RakPeer::InitializeSecurity(const char *public_key, const char *private_key,
	...
	if (_server_handshake->Initialize(public_key, private_key)) {
		CAT_AUDIT_PRINTF(
			"AUDIT: Successfully initialized, filling cookie jar with goodies, storing public key "
			"and setting using security flag to true\n");
	...
	CAT_AUDIT_PRINTF(
		"AUDIT: Failure to initialize so deleting server handshake and cookie jar; also setting "
		"using_security flag = false\n");
...
// existing peers
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
ConnectionAttemptResult RakPeer::Connect(const char *host, unsigned short remotePort,
...
ConnectionAttemptResult RakPeer::ConnectWithSocket(const char *host, unsigned short remotePort,
	...
	return SendConnectionRequest(host, remotePort, passwordData, passwordDataLength, publicKey, 0,
								 0, sendConnectionAttemptCount, timeBetweenSendConnectionAttemptsMS,
								 timeoutTime, socket);
}
...
//	RakNet::TimeMS timeout;
	...
	for (i=0; i < socketList.Size(); i++)
	{
		if (SocketLayer::SendTo(socketList[i], (const char*) &i,1,socketList[i]->GetBoundAddress(),
	_FILE_AND_LINE_)!=0) break;
	}
	*/
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
uint32_t RakPeer::Send(const char *data, const int length, PacketPriority priority,
...
uint32_t RakPeer::Send(const RakNet::BitStream *bitStream, PacketPriority priority,
					   PacketReliability reliability, char orderingChannel,
					   const AddressOrGUID systemIdentifier, bool broadcast,
					   uint32_t forceReceiptNumber) {
#ifdef _DEBUG
	...
	// associated with the reliability layer can change, from that thread, resulting in a send to
	// the wrong player!  While I could mutex the systemAddress, that is much slower than doing this
	SendBuffered((const char *)bitStream->GetData(), bitStream->GetNumberOfBitsUsed(), priority,
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
uint32_t RakPeer::SendList(const char **data, const int *lengths, const int numParameters,
	...
	SendBufferedList(data, lengths, numParameters, priority, reliability, orderingChannel,
					 systemIdentifier, broadcast, RemoteSystemStruct::NO_ACTION, usedSendReceipt);
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#ifdef _MSC_VER
#pragma warning(disable : 4701) // warning C4701: local variable <variable name> may be used without
	...
	for (i=0; i < socketList.Size(); i++)
	{
		while (1)
		{
			...
			SocketLayer::RecvFromBlocking(
				...
				recvFromStruct->extraSocketOptions, recvFromStruct->data,
&recvFromStruct->bytesRead, &recvFromStruct->systemAddress, &recvFromStruct->timeRead); if
(recvFromStruct->bytesRead<=0)
			{
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::Ping(const char *host, unsigned short remotePort,
	...
	*/
	...
	bsp.systemAddress.FromStringExplicitPort(
		host, remotePort, socketList[realIndex]->GetBoundAddress().GetIPVersion());
	...
	for (i = 0; i < pluginListNTS.Size(); i++)
		pluginListNTS[i]->OnDirectSocketSend((const char *)bitStream.GetData(),
											 bitStream.GetNumberOfBitsUsed(), bsp.systemAddress);
...
int RakPeer::GetLastPing(const AddressOrGUID systemIdentifier) const {
	...
	if (remoteSystem->pingAndClockDifferentialWriteIndex == 0)
		...
		return remoteSystem
			->pingAndClockDifferential[remoteSystem->pingAndClockDifferentialWriteIndex - 1]
			.pingTime;
}
...
bool RakPeer::GetClientPublicKeyFromSystemAddress(const SystemAddress input,
...
RakNet::TimeMS RakPeer::GetTimeoutTime(const SystemAddress target) {
	if (target == UNASSIGNED_SYSTEM_ADDRESS) {
		...
		if (remoteSystem != 0) remoteSystem->reliabilityLayer.GetTimeoutTime();
	}
	return defaultTimeoutTime;
}
...
// remotePort: Which port to connect to on the remote machine.
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::AdvertiseSystem(const char *host, unsigned short remotePort, const char *data,
							  int dataLength, unsigned connectionSocketIndex) {
	RakNet::BitStream bs;
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::SendTTL(const char *host, unsigned short remotePort, int ttl,
	...
	if (socketList[realIndex]->IsBerkleySocket()) {
		...
		unsigned i;
		for (i = 0; i < pluginListNTS.Size(); i++)
			pluginListNTS[i]->OnDirectSocketSend((const char *)bsp.data, BYTES_TO_BITS(bsp.length),
												 bsp.systemAddress);
		socketList[realIndex]->Send(&bsp, _FILE_AND_LINE_);
	}
#endif
...
void RakPeer::PushBackPacket(Packet *packet, bool pushAtHead) {
	...
	unsigned i;
	for (i = 0; i < pluginListTS.Size(); i++)
		pluginListTS[i]->OnPushBackPacket((const char *)packet->data, packet->bitSize,
										  packet->systemAddress);
	for (i = 0; i < pluginListNTS.Size(); i++)
		pluginListNTS[i]->OnPushBackPacket((const char *)packet->data, packet->bitSize,
										   packet->systemAddress);
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::ApplyNetworkSimulator(float packetloss, unsigned short minExtraPing,
	...
	if (remoteSystemList) {
		...
		for (i = 0; i < maximumNumberOfPeers; i++)
			// for (i=0; i < remoteSystemListSize; i++)
			remoteSystemList[i].reliabilityLayer.ApplyNetworkSimulator(packetloss, minExtraPing,
																	   extraPingVariance);
	}
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::SendOutOfBand(const char *host, unsigned short remotePort, const char *data,
							BitSize_t dataLength, unsigned connectionSocketIndex) {
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::GetStatisticsList(DataStructures::List<SystemAddress> &addresses,
								DataStructures::List<RakNetGUID> &guids,
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#if LIBCAT_SECURITY == 1
bool RakPeer::GenerateConnectionRequestChallenge(RequestedConnectionStruct *rcs,
												 PublicKey *publicKey) {
	...
	switch (publicKey->publicKeyMode) {
		...
		case PKM_USE_TWO_WAY_AUTHENTICATION:
			...
			rcs->client_handshake = RakNet::OP_NEW<cat::ClientEasyHandshake>(_FILE_AND_LINE_);
			memcpy(rcs->remote_public_key, publicKey->remoteServerPublicKey,
				   cat::EasyHandshake::PUBLIC_KEY_BYTES);

			if (!rcs->client_handshake->Initialize(publicKey->remoteServerPublicKey) ||
				!rcs->client_handshake->SetIdentity(publicKey->myPublicKey,
			...
			CAT_AUDIT_PRINTF(
				"AUDIT: Success initializing new client handshake object with identity for this "
...
ConnectionAttemptResult RakPeer::SendConnectionRequest(
	...
	if (!systemAddress.FromStringExplicitPort(
			host, remotePort, socketList[connectionSocketIndex]->GetBoundAddress().GetIPVersion()))
...
RakPeer::RemoteSystemStruct *RakPeer::GetRemoteSystem(const AddressOrGUID systemIdentifier,
	...
	if (systemIdentifier.rakNetGuid != UNASSIGNED_RAKNET_GUID)
		...
		return GetRemoteSystemFromSystemAddress(systemIdentifier.systemAddress,
												calledFromNetworkThread, onlyActive);
}
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
RakPeer::RemoteSystemStruct *RakPeer::GetRemoteSystemFromSystemAddress(
	...
	if (calledFromNetworkThread) {
		...
		// Active connections take priority.  But if there are no active connections, return the
		// first systemAddress match found
		for (i = 0; i < maximumNumberOfPeers; i++) {
...
void RakPeer::ParseConnectionRequestPacket(RakPeer::RemoteSystemStruct *remoteSystem,
										   const SystemAddress &systemAddress, const char *data,
										   int byteSize) {
	...
	if (_using_security) {
		...
		if (!remoteSystem->reliabilityLayer.GetAuthenticatedEncryption()->ValidateProof(
				proof, sizeof(proof))) {
		...
		if (doClientKey == 1) {
			...
			if (_require_client_public_key) {
				...
				if (!_server_handshake->VerifyInitiatorIdentity(remoteSystem->answer, ident,
					...
					bitStream.Write(
						(MessageID)
							ID_REMOTE_SYSTEM_REQUIRES_PUBLIC_KEY); // Report an error since the
																   // client is not providing an
																   // identity when it is necessary
																   // to connect
					bitStream.Write((unsigned char)2); // Indicate client identity is invalid
					SendImmediate((char *)bitStream.GetData(), bitStream.GetNumberOfBytesUsed(),
								  IMMEDIATE_PRIORITY, RELIABLE, 0, systemAddress, false, false,
								  RakNet::GetTimeUS(), 0);
					remoteSystem->connectMode = RemoteSystemStruct::DISCONNECT_ASAP_SILENTLY;
					return;
				}
			...
			if (_require_client_public_key) {
				...
				bitStream.Write(
					(MessageID)
						ID_REMOTE_SYSTEM_REQUIRES_PUBLIC_KEY); // Report an error since the client
...
void RakPeer::OnConnectionRequest(RakPeer::RemoteSystemStruct *remoteSystem,
	...
	SendImmediate((char *)bitStream.GetData(), bitStream.GetNumberOfBitsUsed(), IMMEDIATE_PRIORITY,
				  RELIABLE_ORDERED, 0, remoteSystem->systemAddress, false, false,
				  RakNet::GetTimeUS(), 0);
}
...
void RakPeer::NotifyAndFlagForShutdown(const SystemAddress systemAddress, bool performImmediate,
	...
	if (performImmediate) {
		SendImmediate((char *)temp.GetData(), temp.GetNumberOfBitsUsed(),
					  disconnectionNotificationPriority, RELIABLE_ORDERED, orderingChannel,
		...
		SendBuffered((const char *)temp.GetData(), temp.GetNumberOfBitsUsed(),
					 disconnectionNotificationPriority, RELIABLE_ORDERED, orderingChannel,
		...
		if (IsLoopbackAddress(systemAddress, false) == false) {
			for (i = 0; i < maximumNumberOfPeers; i++) {
				if (remoteSystemList[i].isActive == true &&
					...
					// 4/13/09 Attackers can flood ID_OPEN_CONNECTION_REQUEST and use up all
					// available connection slots Ignore connection attempts if this IP address
					// connected within the last 100 milliseconds
	...
	for (assignedIndex = 0; assignedIndex < maximumNumberOfPeers; assignedIndex++) {
		if (remoteSystemList[assignedIndex].isActive == false) {
			...
			if (incomingRakNetSocket->GetBoundAddress() == bindingAddress) {
				...
				// machine with multiple IP addresses to reply back on the IP that the datagram came
				// in on
...
incomingRakNetSocket->GetChromeInstance() ); else rns = SocketLayer::CreateBoundSocket_PS3Lobby(
bindingAddress.GetPort(), incomingRakNetSocket->GetBlockingSocket(), ipListFoundIndexStr,
...
bool RakPeer::IsLoopbackAddress(const AddressOrGUID &systemIdentifier, bool matchPort) const {
	...
	for (int i = 0; i < MAXIMUM_NUMBER_OF_INTERNAL_IDS && ipList[i] != UNASSIGNED_SYSTEM_ADDRESS;
		 i++) {
		if (matchPort) {
...
void RakPeer::PingInternal(const SystemAddress target, bool performImmediate,
	...
	bitStream.Write(RakNet::GetTime());
	if (performImmediate)
		SendImmediate((char *)bitStream.GetData(), bitStream.GetNumberOfBitsUsed(),
					  IMMEDIATE_PRIORITY, reliability, 0, target, false, false, RakNet::GetTimeUS(),
					  0);
	else
...
void RakPeer::SendBuffered(const char *data, BitSize_t numberOfBitsToSend, PacketPriority priority,
	...
	bcs->data = (char *)rakMalloc_Ex(
		...
		_FILE_AND_LINE_); // Making a copy doesn't lose efficiency because I tell the reliability
						  // layer to use this allocation for its own copy
	if (bcs->data == 0) {
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::SendBufferedList(const char **data, const int *lengths, const int numParameters,
							   PacketPriority priority, PacketReliability reliability,
							   char orderingChannel, const AddressOrGUID systemIdentifier,
							   bool broadcast, RemoteSystemStruct::ConnectMode connectionMode,
							   uint32_t receipt) {
	BufferedCommandStruct *bcs;
...
bool RakPeer::SendImmediate(char *data, BitSize_t numberOfBitsToSend, PacketPriority priority,
							PacketReliability reliability, char orderingChannel,
							const AddressOrGUID systemIdentifier, bool broadcast,
							bool useCallerDataAllocation, RakNet::TimeUS currentTime,
							uint32_t receipt) {
	unsigned *sendList;
...
//			int debugIndex = GetRemoteSystemIndex(systemIdentifier.systemAddress);
...
#else
		sendList =
			(unsigned *)rakMalloc_Ex(sizeof(unsigned) * maximumNumberOfPeers, _FILE_AND_LINE_);
#endif
	...
	for (sendListIndex = 0; sendListIndex < sendListSize; sendListIndex++) {
		...
		bool useData = useCallerDataAllocation && callerDataAllocationUsed == false &&
		...
		if (reliability == RELIABLE || reliability == RELIABLE_ORDERED ||
			reliability == RELIABLE_SEQUENCED || reliability == RELIABLE_WITH_ACK_RECEIPT ||
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::OnConnectedPong(RakNet::Time sendPingTime, RakNet::Time sendPongTime,
							  RemoteSystemStruct *remoteSystem) {
	RakNet::Time ping;
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
namespace RakNet {
bool ProcessOfflineNetworkPacket(SystemAddress systemAddress, const char *data, const int length,
								 RakPeer *rakPeer, RakNetSocket2 *rakNetSocket,
	...
	if (rakPeer->IsBanned(str1)) {
		...
		for (i = 0; i < rakPeer->pluginListNTS.Size(); i++)
			rakPeer->pluginListNTS[i]->OnDirectSocketSend((char *)bs.GetData(),
														  bs.GetNumberOfBitsUsed(), systemAddress);
	...
	if (length <= 2) {
		...
		*isOfflineMessage = memcmp(data + sizeof(unsigned char) + sizeof(RakNet::Time),
			   ...
			   (size_t)length >= sizeof(unsigned char) + sizeof(RakNet::TimeMS) +
									 RakNetGUID::size() + sizeof(OFFLINE_MESSAGE_DATA_ID)) {
		*isOfflineMessage =
			memcmp(data + sizeof(unsigned char) + sizeof(RakNet::Time) + RakNetGUID::size(),
				   OFFLINE_MESSAGE_DATA_ID, sizeof(OFFLINE_MESSAGE_DATA_ID)) == 0;
	} else if ((unsigned char)data[0] == ID_OUT_OF_BAND_INTERNAL &&
			   (size_t)length >=
				   sizeof(MessageID) + RakNetGUID::size() + sizeof(OFFLINE_MESSAGE_DATA_ID)) {
		*isOfflineMessage = memcmp(data + sizeof(MessageID) + RakNetGUID::size(),
								   OFFLINE_MESSAGE_DATA_ID, sizeof(OFFLINE_MESSAGE_DATA_ID)) == 0;
			   ...
			   (size_t)length >=
				   sizeof(MessageID) + RakNetGUID::size() + sizeof(OFFLINE_MESSAGE_DATA_ID)) {
		*isOfflineMessage = memcmp(data + sizeof(MessageID), OFFLINE_MESSAGE_DATA_ID,
								   sizeof(OFFLINE_MESSAGE_DATA_ID)) == 0;
	} else if (((unsigned char)data[0] == ID_INCOMPATIBLE_PROTOCOL_VERSION &&
				(size_t)length ==
					sizeof(MessageID) * 2 + RakNetGUID::size() + sizeof(OFFLINE_MESSAGE_DATA_ID))) {
		*isOfflineMessage = memcmp(data + sizeof(MessageID) * 2, OFFLINE_MESSAGE_DATA_ID,
								   sizeof(OFFLINE_MESSAGE_DATA_ID)) == 0;
	} else {
	...
	if (*isOfflineMessage) {
				...
				rakPeer->AllowIncomingConnections()) // Open connections with players
			{
				...
				for (i = 0; i < rakPeer->pluginListNTS.Size(); i++)
					rakPeer->pluginListNTS[i]->OnDirectSocketSend(
						(const char *)outBitStream.GetData(), outBitStream.GetNumberOfBytesUsed(),
						systemAddress);
		...
		// UNCONNECTED MESSAGE Pong with no data.
			...
			packet = rakPeer->AllocPacket(
				(unsigned int)(length - sizeof(OFFLINE_MESSAGE_DATA_ID) - RakNetGUID::size() -
			...
			bsOut.Write(pingMS);
			bsOut.WriteAlignedBytes((const unsigned char *)data + sizeof(unsigned char) +
			...
			rakPeer->AddPacketToProducer(packet);
				   ...
				   (size_t)length >
					   sizeof(OFFLINE_MESSAGE_DATA_ID) + sizeof(MessageID) + RakNetGUID::size() &&
				   (size_t)length < MAX_OFFLINE_DATA_LENGTH + sizeof(OFFLINE_MESSAGE_DATA_ID) +
										sizeof(MessageID) + RakNetGUID::size()) {
			...
			if (data[sizeof(OFFLINE_MESSAGE_DATA_ID) + sizeof(MessageID) + RakNetGUID::size()] ==
				ID_ADVERTISE_SYSTEM) {
				packet->length--;
			...
			// Even if the server has security, it may not be required of us if we are in the
				...
				if (rcs->systemAddress == systemAddress) {
					if (serverHasSecurity) {
						...
						if (rcs->publicKeyMode == PKM_ACCEPT_ANY_PUBLIC_KEY) {
							memcpy(rcs->remote_public_key, public_key,
								   cat::EasyHandshake::PUBLIC_KEY_BYTES);
							if (!rcs->client_handshake->Initialize(public_key) ||
								!rcs->client_handshake->GenerateChallenge(
									rcs->handshakeChallenge)) {
								CAT_AUDIT_PRINTF(
									"AUDIT: Server passed a bad public key with "
									"PKM_ACCEPT_ANY_PUBLIC_KEY");
								return true;
							}
						}

						if (cat::SecureEqual(public_key, rcs->remote_public_key,
						...
						if (rcs->client_handshake != 0) {
							...
							packet = rakPeer->AllocPacket(sizeof(char), _FILE_AND_LINE_);
							packet->data[0] =
								ID_OUR_SYSTEM_REQUIRES_SECURITY; // Attempted a connection and
																 // couldn't
							packet->bitSize = (sizeof(char) * 8);
							packet->systemAddress = rcs->systemAddress;
							packet->guid = serverGuid;
					...
					// bsOut.GetData(), bsOut.GetNumberOfBytesUsed(), rcs->systemAddress,
					// _FILE_AND_LINE_ );
				...
				if (rcs->systemAddress == systemAddress) {
					...
					if (doSecurity) {
						if (rcs->client_handshake == 0) {
							...
							packet->data[0] =
								ID_REMOTE_SYSTEM_REQUIRES_PUBLIC_KEY; // Attempted a connection and
																	  // couldn't
						...
						CAT_AUDIT_PRINTF(
							"AUDIT: Looks good, preparing to send challenge to server! "
							"client_handshake = %x\n",
							client_handshake);
					}
					...
					if (remoteSystem == 0) {
						if (rcs->socket == 0) {
							remoteSystem = rakPeer->AssignSystemAddressToRemoteSystemList(
								systemAddress, RakPeer::RemoteSystemStruct::UNVERIFIED_SENDER,
								rakNetSocket, &thisIPConnectedRecently, bindingAddress, mtu, guid,
								doSecurity);
						} else {
							remoteSystem = rakPeer->AssignSystemAddressToRemoteSystemList(
								systemAddress, RakPeer::RemoteSystemStruct::UNVERIFIED_SENDER,
								rcs->socket, &thisIPConnectedRecently, bindingAddress, mtu, guid,
								doSecurity);
						}
					}
					...
					if (thisIPConnectedRecently == false) {
						...
						if (remoteSystem) {
							...
							if (rcs->client_handshake) {
								...
								if (rcs->publicKeyMode == PKM_USE_TWO_WAY_AUTHENTICATION) {
									...
									if (!rcs->client_handshake->ProcessAnswer(
											answer, remoteSystem->reliabilityLayer
														.GetAuthenticatedEncryption())) {
										CAT_AUDIT_PRINTF(
											"AUDIT: Processing answer -- Invalid Answer\n");
										rakPeer->requestedConnectionQueueMutex.Unlock();
							...
							if (rcs->outgoingPasswordLength > 0)
								temp.Write((char *)rcs->outgoingPassword,
										   rcs->outgoingPasswordLength);

							rakPeer->SendImmediate((char *)temp.GetData(),
												   temp.GetNumberOfBitsUsed(), IMMEDIATE_PRIORITY,
												   RELIABLE, 0, systemAddress, false, false,
												   timeRead, 0);
						} else {
...
#if LIBCAT_SECURITY == 1
					CAT_AUDIT_PRINTF(
						"AUDIT: Deleting client_handshake object %x and rcs->client_handshake "
						"object %x\n",
...
#if LIBCAT_SECURITY == 1
					CAT_AUDIT_PRINTF(
						"AUDIT: Connection attempt canceled so deleting rcs->client_handshake "
						"object %x\n",
			...
			if (remoteProtocol != RAKNET_PROTOCOL_VERSION) {
				...
				// SocketLayer::SendTo( rakNetSocket, (char*)bs.GetData(),
				// bs.GetNumberOfBytesUsed(), systemAddress, _FILE_AND_LINE_ );
			...
			if (rakPeer->_using_security) {
				...
				// Write cookie
				uint32_t cookie = rakPeer->_cookie_jar->Generate(&systemAddress.address,
																 sizeof(systemAddress.address));
				CAT_AUDIT_PRINTF("AUDIT: Writing cookie %i to %i:%i\n", cookie, systemAddress);
				bsOut.Write(cookie);
				// Write my public key
				bsOut.WriteAlignedBytes((const unsigned char *)rakPeer->my_public_key,
			...
			for (i = 0; i < rakPeer->pluginListNTS.Size(); i++)
				rakPeer->pluginListNTS[i]->OnDirectSocketSend(
					(const char *)bsOut.GetData(), bsOut.GetNumberOfBitsUsed(), systemAddress);
			...
			if (rakPeer->_using_security) {
				...
				if (rakPeer->_cookie_jar->Verify(&systemAddress.address,
												 sizeof(systemAddress.address), cookie) == false) {
					return true;
				}
				...
				if (clientWroteChallenge) {
					bs.ReadAlignedBytes((unsigned char *)remoteHandshakeChallenge,
										cat::EasyHandshake::CHALLENGE_BYTES);
#ifdef CAT_AUDIT
			...
			if (IPAddrInUse & GUIDInUse) {
				if (rssFromSA == rssFromGuid &&
					rssFromSA->connectMode == RakPeer::RemoteSystemStruct::UNVERIFIED_SENDER) {
					// ID_OPEN_CONNECTION_REPLY if they are the same
					outcome = 1;

					// Note to self: If REQUESTED_CONNECTION, this means two systems attempted to
					// connect to each other at the same time, and one finished first. Returns
					// ID)_CONNECTION_REQUEST_ACCEPTED to one system, and ID_ALREADY_CONNECTED
					// followed by ID_NEW_INCOMING_CONNECTION to another
				} else {
					// ID_ALREADY_CONNECTED (restarted raknet, connected again from same ip, plus
					// someone else took this guid)
					outcome = 2;
				}
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void ProcessNetworkPacket(SystemAddress systemAddress, const char *data, const int length,
...
void ProcessNetworkPacket(SystemAddress systemAddress, const char *data, const int length,
	...
	if (ProcessOfflineNetworkPacket(systemAddress, data, length, rakPeer, rakNetSocket,
...
unsigned int RakPeer::GenerateSeedFromGuid(void) {
	/*
	// Construct a random seed based on the initial guid value, and the last digits of the
	difference to each subsequent number
...
bool RakPeer::RunRecvFromOnce( RakNetSocket *s )
{
	...
	{
		recvFromStruct->s=s;
		SocketLayer::RecvFromBlocking(s, this, recvFromStruct->data, &recvFromStruct->bytesRead,
&recvFromStruct->systemAddress, &recvFromStruct->timeRead);
...
bool RakPeer::RunUpdateCycle(BitStream &updateBitStream) {
	...
	while ((recvFromStruct = PopBufferedPacket()) != 0) {
		...
		*/
		ProcessNetworkPacket(recvFromStruct->systemAddress, recvFromStruct->data,
							 recvFromStruct->bytesRead, this, recvFromStruct->socket,
	...
	while ((bcs = bufferedCommands.PopInaccurate()) != 0) {
		if (bcs->command == BufferedCommandStruct::BCS_SEND) {
			...
			callerDataAllocationUsed =
				SendImmediate((char *)bcs->data, bcs->numberOfBitsToSend, bcs->priority,
			...
			// Set the new connection state AFTER we call sendImmediate in case we are setting it to
			// a disconnection state, which does not allow further sends
			...
			CloseConnectionInternal(bcs->systemIdentifier, false, true, bcs->orderingChannel,
									bcs->priority);
		...
		while (requestedConnectionQueueIndex < requestedConnectionQueue.Size()) {
			...
			if (rcs->nextRequestTime < timeMS) {
				...
				if (condition1 || condition2) {
...
#if LIBCAT_SECURITY == 1
					CAT_AUDIT_PRINTF(
						"AUDIT: Connection attempt FAILED so deleting rcs->client_handshake object "
						"%x\n",
						rcs->client_handshake);
					RakNet::OP_DELETE(rcs->client_handshake, _FILE_AND_LINE_);
#endif
					...
					// bitStream.GetNumberOfBytesUsed(), rcs->systemAddress, _FILE_AND_LINE_
					// )==-10040)
					{
	...
	// for ( remoteSystemIndex = 0; remoteSystemIndex < remoteSystemListSize; ++remoteSystemIndex )
	{
		...
		// Allow the systemAddress for this remote system list to change.  We don't care if it
		// changes now.
		...
		remoteSystem->reliabilityLayer.Update(
			remoteSystem->rakNetSocket, systemAddress, remoteSystem->MTUSize, timeNS,
		...
		if (remoteSystem->reliabilityLayer.IsDeadConnection() ||
			...
			// DISCONNECT_ASAP_SILENTLY and DISCONNECT_ASAP than for DISCONNECT_ON_NO_ACK The first
			// two mean we called CloseConnection(), the last means the other system sent us
		...
		while (bitSize > 0) {
			...
			if (remoteSystem->connectMode == RemoteSystemStruct::UNVERIFIED_SENDER) {
				if ((unsigned char)(data)[0] == ID_CONNECTION_REQUEST) {
					ParseConnectionRequestPacket(remoteSystem, systemAddress, (const char *)data,
												 byteSize);
				...
				// However, if we are connected we still take a connection request in case both
				// systems are trying to connect to each other at the same time
				if ((unsigned char)(data)[0] == ID_CONNECTION_REQUEST) {
					...
					// connections from connected will close the connection due to security at
					...
					if (remoteSystem->connectMode == RemoteSystemStruct::REQUESTED_CONNECTION) {
						ParseConnectionRequestPacket(remoteSystem, systemAddress,
													 (const char *)data, byteSize);
					} else {
						RakNet::BitStream bs((unsigned char *)data, byteSize, false);
						bs.IgnoreBytes(sizeof(MessageID));
						bs.IgnoreBytes(sizeof(OFFLINE_MESSAGE_DATA_ID));
						...
						// Got a connection request message from someone we are already connected
						// to. Just reply normally. This can happen due to race conditions with the
					...
					if (remoteSystem->connectMode ==
						...
						// tries to send to R, sends to loopback because R==firstExternalID Correct
						// fix is to specify in Connect() if target is through a proxy. However, in
						...
						// Send to game even if already connected. This could happen when connecting
						// to 127.0.0.1 Ignore, already connected
						//	rakFree_Ex(data, _FILE_AND_LINE_ );
					}
					...
					SendImmediate((char *)outBitStream.GetData(),
								  outBitStream.GetNumberOfBitsUsed(), IMMEDIATE_PRIORITY,
								  UNRELIABLE, 0, systemAddress, false, false, RakNet::GetTimeUS(),
								  0);
					...
					//	AddPacketToProducer(packet);
				} else if ((unsigned char)(data)[0] == ID_DETECT_LOST_CONNECTIONS &&
					...
					if (byteSize > sizeof(MessageID) + sizeof(unsigned int) +
						...
						if (allowConnection) {
							...
							// Bug: If A connects to B through R, A's firstExternalID is set to R.
							// If A tries to send to R, sends to loopback because R==firstExternalID
							// Correct fix is to specify in Connect() if target is through a proxy.
							// However, in practice you have to connect to something else first
							// anyway to know about the proxy. So setting once only is good enough
							if (firstExternalID == UNASSIGNED_SYSTEM_ADDRESS) {
							...
							SendImmediate((char *)outBitStream.GetData(),
										  outBitStream.GetNumberOfBitsUsed(), IMMEDIATE_PRIORITY,
										  RELIABLE_ORDERED, 0, systemAddress, false, false,
										  RakNet::GetTimeUS(), 0);
...
void RakPeer::CallPluginCallbacks(DataStructures::List<PluginInterface2 *> &pluginList,
								  Packet *packet) {
	for (unsigned int i = 0; i < pluginList.Size(); i++) {
		switch (packet->data[0]) {
			case ID_DISCONNECTION_NOTIFICATION:
				pluginList[i]->OnClosedConnection(packet->systemAddress, packet->guid,
			...
			case ID_REMOTE_SYSTEM_REQUIRES_PUBLIC_KEY:
				pluginList[i]->OnFailedConnectionAttempt(packet,
...
void RakPeer::FillIPList(void) {
	...
	while (startingIdx < MAXIMUM_NUMBER_OF_INTERNAL_IDS - 1 &&
		...
		for (int curIdx = startingIdx + 1; curIdx < MAXIMUM_NUMBER_OF_INTERNAL_IDS - 1 &&
