...
bool TwoWayAuthentication::NonceGenerator::GetNonceById(
	char nonce[TWO_WAY_AUTHENTICATION_NONCE_LENGTH], unsigned short requestId,
...
void TwoWayAuthentication::Update(void) {
	...
	if (GreaterThan(curTime - CHALLENGE_MINIMUM_TIMEOUT, whenLastTimeoutCheck)) {
		while (outgoingChallenges.Size() &&
			...
			PushToUser(ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_TIMEOUT, pc.identifier,
					   pc.remoteSystem);
		}
...
void TwoWayAuthentication::OnClosedConnection(const SystemAddress &systemAddress,
	...
	for (i = 0; i < outgoingChallenges.Size(); i++) {
		if (outgoingChallenges[i].remoteSystem == aog && outgoingChallenges[i].sentHash == false) {
			...
			if (skhi.IsInvalid() == false) {
				...
				bsOut.Write(requestId);
				bsOut.Write(outgoingChallenges[i].identifier); // Identifier helps the other system
															   // lookup the password quickly.
				bsOut.WriteAlignedBytes((const unsigned char *)hashedNonceAndPw,
...
PluginReceiveResult TwoWayAuthentication::OnHashedNonceAndPassword(Packet *packet) {
	...
	if (skhi.IsInvalid() == false) {
		...
		if (memcmp(hashedThisNonceAndPw, remoteHashedNonceAndPw, HASHED_NONCE_AND_PW_LENGTH) == 0) {
			...
			bsOut.WriteAlignedBytes((const unsigned char *)usedNonce,
									TWO_WAY_AUTHENTICATION_NONCE_LENGTH);
			bsOut.WriteAlignedBytes((const unsigned char *)remoteHashedNonceAndPw,
									HASHED_NONCE_AND_PW_LENGTH);
			bsOut.Write(passwordIdentifier);
			SendUnified(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, packet, false);

			// Incoming success, modify packet header to tell user
			PushToUser(ID_TWO_WAY_AUTHENTICATION_INCOMING_CHALLENGE_SUCCESS, passwordIdentifier,
					   packet);
...
void TwoWayAuthentication::Hash(char thierNonce[TWO_WAY_AUTHENTICATION_NONCE_LENGTH],
