...
class RAK_DLL_EXPORT HTTPConnection2 : public PluginInterface2 {
   public:
	...
	/// connect to \param[in] port The port to connect to \param[in] useSSL If to use SSL to
	/// connect. OPEN_SSL_CLIENT_SUPPORT must be defined to 1 in RakNetDefines.h or
	/// RakNetDefinesOverrides.h \param[in] ipVersion 4 for IPV4, 6 for IPV6 \param[in] useAddress
	/// Assume we are connected to this address and send to it, rather than do a lookup \param[in]
	/// userData \return false if host is not a valid IP address or domain name
	bool TransmitRequest(const char *stringToTransmit, const char *host, unsigned short port = 80,
						 bool useSSL = false, int ipVersion = 4,
	...
	/// responseReceived. \param[out] userData Whatever you passed to TransmitRequest \return true
	/// if there was a response. false if not.
	bool GetResponse(RakString &stringTransmitted, RakString &hostTransmitted,
					 RakString &responseReceived, SystemAddress &hostReceived, int &contentOffset,
					 void **userData);
	bool GetResponse(RakString &stringTransmitted, RakString &hostTransmitted,
					 RakString &responseReceived, SystemAddress &hostReceived, int &contentOffset);
