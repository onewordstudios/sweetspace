...
void NatTypeDetectionServer::Startup(const char *nonRakNetIP2, const char *nonRakNetIP3,
...
void NatTypeDetectionServer::Update(void) {
	...
	s3p4 while (len>0 && data[0]==NAT_TYPE_PORT_RESTRICTED)
	{
		...
		if (readSuccess)
		{
			...
			{
		//		RakAssert("i==0 in Update when looking up GUID in NatTypeDetectionServer.cpp. Either
	a bug or a late resend" && 0);
			}
		}
		else
		{
		...
		if (time > natDetectionAttempts[i].nextStateTime) {
			...
			switch (natDetectionAttempts[i].detectionState) {
...
#endif
					// S4P5 sends to C2. If arrived, no NAT. Done. (Else S4P5 potentially banned, do
					// not use again).
				...
				case STATE_TESTING_FULL_CONE_2:
					...
					// S2P3 sends to C1 (Different address, different port, to previously used port
					// on client). If received, Full-cone nat. Done.  (Else S2P3 potentially banned,
					// do not use again).
					saOut = natDetectionAttempts[i].systemAddress;
				...
				case STATE_TESTING_PORT_RESTRICTED_2:
					// C1 sends to S3P4. If address of C1 as seen by S3P4 is the same as the address
					// of C1 as seen by S1P1, then port-restricted cone nat. Done
				...
				default:
					...
					printf(
						...
						"that client would have sent NAT_TYPE_PORT_RESTRICTED on s3p4.\nDefaulting "
						"to Symmetric\n");
#endif
					bs.Write((unsigned char)ID_NAT_TYPE_DETECTION_RESULT);
					bs.Write((unsigned char)NAT_TYPE_SYMMETRIC);
...
void NatTypeDetectionServer::OnClosedConnection(const SystemAddress &systemAddress,
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void NatTypeDetectionServer::DeallocRNS2RecvStruct(RNS2RecvStruct *s, const char *file,
												   unsigned int line) {
